...
export async function buildSearchIndex(index: StaticIndex): Promise<SearchIndex> {
  ...
  // id. ADR-047.
  //
  // **ADR-171 gives every GMB route a `region`, and deliberately does not put it in this key.** It
  ...
  for (const meta of index.routeMeta.values()) {
    ...
    byNumber.set(key, {
      ...
      // (ADR-171). `undefined` is dropped by `JSON.stringify`, so a KMB row costs no bytes for it.
      region: meta.region,
