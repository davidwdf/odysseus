...
public enum NextBusTokens {
    ...
    public enum Opacity {
        ...
        /// board had aged was faded to 45%. The cue is now a muted `~` before the figure, declared
        /// in the component specs (`stop-row`, `place-row`, `route-detail`) and drawn by both
        /// renderers, and NO renderer applies this token any more. It is kept rather than deleted
        /// because this file's readers include a hand-written iOS/Android client (ADR-067/075) that
        /// will meet the same design problem, and the answer to it is not obvious: a fade is
        /// *noticed* rather than read — a rider with one reading on screen has nothing to compare
        /// it against — and it dims the one number they are trying to read. Deleting the token
        /// would leave that decision nowhere a porter looks, and the next 0.45 would be
        /// re-invented. Retiring it in place puts the warning in the file they are reading, because
        /// this description is emitted verbatim into `NextBusTokens.swift` and `.kt`. If you are
        /// implementing a third renderer: draw the mark, not the fade. `check-tokens-current`
        /// counts tokens; it cannot see that one has no consumer, so this sentence is the
        /// mechanism.
