...
object NextBusTokens {
     ...
     * arithmetic, not taste: white on the gold is 2.16:1 and on the yellow 1.60:1, where WCAG
     * asks 4.5:1 of a 14 px bold chip; the dark ink gives 8.28:1 and 11.13:1.
     * `apps/web/test/search-contrast.test.ts` measures every pair, so a fifth operator cannot
     * ship an unreadable chip.
     */
    object OperatorTextColor {
        val kmb: Color = Color(0xFFFFFFFF)
        val lwb: Color = Color(0xFF0F172A)
    ...
    object Opacity {
        /**
         * RETIRED — do not apply this to anything, and do not replace it with a per-reading
         * marker either. It was the staleness cue: an ETA reading whose board had aged was
         * faded to 45%. Two treatments have now been withdrawn (ADR-123) — this fade, and a
         * muted `~` before the figure — and the reason is the same for both and is worth a
         * porter's time: staleness is a property of the BOARD, not of each figure. `isStale`
         * reads one `dataTimestamp` per board, so a per-figure cue draws one fact once per
         * reading (78 times on one route screen), and a rider can act on "the screen has
         * stopped updating" but not on "this particular number is two minutes old". The cue
         * belongs at screen level, as one line. If you are implementing a third renderer: do
         * not dim the number, do not mark it — say when the screen was last updated.
         * `check-tokens-current` counts tokens; it cannot see that one has no consumer, so
