...
export interface IndexRouteMeta extends IndexRouteRef {
  ...
  gtfsId?: string
  /**
   * GMB only: the region the public route number is unique within (ADR-174).
   *
  ...
  for (const entry of Object.values(data.routeList)) {
    for (const co of entry.co) {
      ...
      routeMeta.set(routeId, {
        ...
        // The region that makes a GMB number mean something (ADR-174). Keyed on `gmbId` rather than
