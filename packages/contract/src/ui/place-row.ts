...
export const PLACE_ROW_SPEC: ComponentSpec = {
  ...
  states: {
    ...
    stale: {
      must: 'Nothing on the row — the screen says it once (ADR-133).',
      ...
      enforcement: {
        unenforced:
          'Deliberately nothing to enforce here since ADR-123: the row draws no staleness cue at all, because staleness is the board’s and a per-figure mark repeats one fact once per reading. The screen’s `feedNotice` line carries it (ADR-133), and Place detail draws that line since ADR-150 — `place-detail.spec.json` projects it, so the claim is checked one level up, which is where the fact lives.',
      },
    },
    offline: {
