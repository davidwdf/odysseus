...
export const PLACE_ROW_SPEC: ComponentSpec = {
  ...
  states: {
    ...
    stale: {
      ...
      enforcement: {
        unenforced:
          'Deliberately nothing to enforce here since ADR-123: the row draws no staleness cue at all, because staleness is the board’s and a per-figure mark repeats one fact once per reading. The screen’s `feedNotice` line carries it (ADR-133), and Place detail has yet to be wired for it.',
      },
    },
    offline: {
