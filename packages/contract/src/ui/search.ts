...
/**
 * The tag that says **which `1` this is** (ADR-175).
 *
...
const REGION_TAG: SlotNode = {
  ...
  text: { field: 'region' },
  when: 'region',
  why: 'Absent for every operator whose numbers are already unique — and absent for a minibus route the region table has not met yet, because an untagged row is honest where a guessed one is not (ADR-175).',
  invariant:
...
export const SEARCH_SPEC: ComponentSpec = {
  ...
  states: {
     ...
     * and until ADR-175 Search drew them as two identical chips whose only difference was the pair of
    ...
    minibusRegions: {
      ...
      why: 'The identity quirk is ADR-047’s (`route_code` is unique per region, `route_id` globally), and it had been carried as a follow-up ever since: the canonical id disambiguates, the *screen* did not. ADR-175 closes it.',
      enforcement: {
        shows: [
          CHIPS,
          {
