...
export const STOP_ROW_SPEC: ComponentSpec = {
  ...
  states: {
    ...
    stale: {
      ...
      enforcement: {
        unenforced:
          'Nothing on the CARD says this, and since ADR-123 nothing is meant to: two per-reading treatments (a 45% fade, then a muted `~`) were built and withdrawn because staleness is a property of the BOARD, so a per-figure cue draws one fact once per reading. The sentence is the screen’s — ADR-133’s `feedNotice`, held by `eta#feedNotice`’s corpus and wired on Route detail so far. This row stays `unenforced` until it is wired on the screens that draw this card.',
      },
    },
    offline: {
