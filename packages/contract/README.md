...
| --- | --- | --- |
| `packages/contract/openapi.json` | OpenAPI 3.1, v2.0.0 — **9 paths, 40 component schemas** | Generate your models. This is the only artefact you *must* consume. |
| `packages/contract/asyncapi.json` | AsyncAPI 3.0.0 for the `/v1/live` socket — **6 frames, 51 component schemas** | Read it. **Do not plan to generate from it** — there is no AsyncAPI→Swift generator at all, and the Kotlin one cannot serialise. See §7. |
...
| `packages/core/spec/` | **19 corpora, 140 groups, 1168 cases, 3 `knownDefect` rows** | Drive your XCTest/JUnit suite from these bytes. This is the domain-rule half of the port. |
| `packages/contract/ui/` | **12 component spec(s)** — AboutData, Faq, Favourites, Nearby, PlaceDetail, PlaceRow, RouteDetail, Search, Settings, StopRow, WatchCard, WatchPill; each declares its slots and their order, all five states with what each must *not* look like, its interaction targets and its a11y role (4 state(s) marked `knownDefect`) | The **view** half of the port, and the newest thing here — read §7 before you rely on it. Two renderers drive these today; yours would be the third and the first independent one. |
...
| `packages/ui/generated/NextBusTokens.swift` | 134 design tokens — **never compiled** | Compile it. A compile error here is a bug in the emitter, not something to patch in place. |
| `packages/ui/generated/NextBusTokens.kt` | 134 design tokens — **never compiled** | Ditto. |
| `packages/i18n/generated/ios/` | 3 locales × 161 strings + 4 plural messages | `.lproj` bundles — drop in as-is; do not retype a string. |
...
| --- | --- | --: | --: | --: |
| `eta.spec.json` | `packages/core/src/eta.ts` | 26 | 187 | — |
| `favourites.spec.json` | `packages/core/src/favourites.ts` | 5 | 35 | — |
| `geo-snap.spec.json` | `packages/core/src/geo-snap.ts` | 1 | 12 | — |
| `geo.spec.json` | `packages/core/src/geo.ts` | 10 | 87 | — |
| `ids.spec.json` | `packages/core/src/ids.ts` | 10 | 56 | — |
| `live.spec.json` | `packages/core/src/live.ts` | 18 | 208 | — |
| `location-mark.spec.json` | `packages/core/src/location-mark.ts` | 2 | 17 | — |
| `mercator.spec.json` | `packages/core/src/mercator.ts` | 10 | 63 | 1 |
| `policy.spec.json` | `packages/core/src/policy.ts` | 1 | 7 | — |
| `route-detail.spec.json` | `packages/core/src/route-detail.ts` | 10 | 104 | 1 |
| `route-markers.spec.json` | `packages/core/src/route-markers.ts` | 1 | 14 | — |
| `route-path.spec.json` | `packages/core/src/route-path.ts` | 6 | 34 | — |
| `route-position.spec.json` | `packages/core/src/route-position.ts` | 1 | 14 | — |
| `search.spec.json` | `packages/core/src/search.ts` | 16 | 135 | — |
...
| `watch.spec.json` | `packages/core/src/watch.ts` | 2 | 21 | — |
| **total** |  | **140** | **1168** | **3** |
