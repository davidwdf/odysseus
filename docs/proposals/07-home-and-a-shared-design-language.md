...
| ~~H-5b~~ | ~~`home.spec.json`, `/` becomes Home, `/favorites` redirects, the tab bar retires~~ | 2 | M | ✅ **done 2026-09-20** — ADR-181. **Eleven states, down from seventeen**, all driven. The spec caught two defects that would have shipped (`cold` drew a "locating" skeleton with no position to locate; `cold` and `denied` claimed *"No scheduled service"*) and one omission (Home never said the position was remembered). Settings moved tab → lens and the header taxonomy immediately failed it for having no way back |
...
| ~~H-7a~~ | ~~Home gets a map~~ | 3 | M | ✅ **done 2026-09-20** — ADR-183. **Not** `RouteMap` generalised: `MapView` was already the generic half, so `PlacesMap` is a 150-line composition beside a 700-line route screen. Q4 answered (option A). Filled the taxonomy's `root × map` cell as `mapRoot`. 🔴 Found that a device with no WebGL2 lost the **whole screen**, not just the map — latent on Route detail since the map landed |
