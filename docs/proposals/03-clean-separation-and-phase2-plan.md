...
| **WP5-14** | **The frames do not carry `failed`, so a live card cannot say "we could not ask"** *(added 2026-08-03 by WP5-7; ✅ **done 2026-08-03** — [ADR-081](../08-decision-log.md#adr-081--the-frames-carry-failed-and-a-round-whose-failure-set-moved-is-news))* | [ADR-077](../08-decision-log.md#adr-077--a-card-can-say-we-could-not-ask-and-a-failure-list-must-not-outlive-its-round) decision 2 named this as the fix and deferred it in one sentence: *"the frames carry no failure list, deliberately — so once a subscription takes over, its `status: retrying` is the authority and the HTTP-era list must go … the fix is frames that carry `failed`, which is a wire change to make **when a screen renders per-kerb failure**, because that is when the extra precision has a reader."* WP5-7 is that moment: Nearby subscribes, so `StopCardView.incomplete` is now **first-paint-only** there. Most of an outage is still honest without this — `retainFailedPoles` keeps a refusing kerb's readings and they visibly age on the operator's clock (ADR-008) — and the residual is precisely one case: **a pole that has never produced a reading retains nothing**, so a card at a place whose outage was already running when the screen opened reads as a quiet stop. The change is not additive-and-done: `SnapshotFrame` and `DeltaFrame` gain `failed?`, `LiveSession` has to hold it, `applyLiveFrame` has to decide what a delta with no `failed` *means* (unchanged, or empty — an absent optional cannot say both), `EtaHub.sendRound` has to send it, `asyncapi.json` re-emits, ADR-074's corpus grammar needs a column, and only then can `EtaListener` carry it. Both engines in one commit, or they diverge | ~~A Nearby or Favourites card is distinguishable from a quiet stop **on the live path as well as the first paint**, on both engines, with the corpus asserting they agree; and a recovered kerb's marker clears within one round~~ → **met on all three, and the row's own list of work was right about every item except one it did not have.** `SnapshotFrame`/`DeltaFrame` gained `failed?`, `LiveSession` holds it, `EtaHub` and the poll emulator both send it, `EtaListener` carries it as a trailing optional (so no existing listener had to change — and `watch()`'s standing argument against widening the signature was about the *accepted target set*, which is a different kind of fact and still stands). **The decision the row could not have anticipated is the one that makes it correct: a round whose failure set moved is NEWS even when no reading did.** Without it the delta branch is silent for exactly the round an outage produces — a kerb stops answering, `retainFailedPoles` keeps its readings, so nothing changed and nothing is gone — and a recovered kerb's marker would outlive the recovery by a whole cadence, failing the second half of this acceptance. `sameFailures` is that predicate, in the kernel, called by both engines, and it deliberately **ignores the error message** so an upstream that rewords itself does not make every round news. `unionFailures` is the other new rule: a round is N targets and the union has to be built identically on both sides, and it dedupes by pole because a rider can watch a merged place *and* one of its own kerbs at once. An absent `failed` on a frame means **empty**, not unchanged — an optional field cannot say both, and only clearing fails safe (ADR-077 decision 1's direction). Pole failures only: a whole-target failure stays a `status` frame, because `EtaFailure.stopId` is a boarding point and a target may be a place. **One thing was found on a real socket rather than reasoned about:** a re-`subscribe` is answered from stored *readings*, so answering it with `failed: []` paired six real readings with a claim that nothing was refusing and a card went quiet for a cadence — the shard now carries the set forward for the targets that survived, filtered through `memberStopIds`. What that still does not recover is a *reconnect*, which is a new socket with no attachment; stated rather than fixed. **The corpus is the cross-runtime proof:** ADR-074's `settles` grammar gained a `failed=[…]` column, all 11 rows were re-declared, and **the real Durable Object over a real WebSocket in workerd reproduces every one independently**; an anti-vacuous control on each driver requires ≥5 rows to name a kerb and ≥2 to recover. Verified against live data with the KMB upstream unroutable: the batch named 3, 4 and 5 refusing kerbs while the Citybus places kept every reading; the socket produced **`delta changed=0 gone=0 failed=[3 kerbs]`** — the frame that was silence before this row; and in the browser the marker was still on the card *after* the times had advanced, which is the whole acceptance in one screenshot. `CONTRACT_VERSION` unmoved (additive) | M |
