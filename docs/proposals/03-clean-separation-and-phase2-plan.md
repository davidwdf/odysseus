...
| **WP5-11** | **`buildPlaces` keeps one physical pole as two members** *(added 2026-07-31 by WP5-10's measurement)* | Same operator, same name, same printed stop code, coordinates identical or one grid step apart — and clustering keeps them as two members, so the Place screen renders two groups under one heading and (since correction 11) a line boarding at "both" gets two rows. Measured in `d598893de6add2e4`: **331 colliding heading groups sit under 10 m apart, 49 code collisions at exactly 0.0 m.** `MERGE_RADIUS_M` = 30 m already covers the distance; what is missing is a same-operator merge for members that agree on *every* rider-visible field. It is a dataset change, so it moves the build hash, needs its own ADR and wants a check that it does not swallow a genuine two-berth stand (KMB really does print one code on poles 20–35 m apart — TN507, TN581, ND126) | A place has one member per physical boarding point; WP5-10's declines fall to the genuinely-ambiguous few, and no place shows two identical headings | M |
