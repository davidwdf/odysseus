...
> | [06 — Maps & route geometry](./06-maps-and-route-geometry.md) | **Proposal, not built.** Acts on [ADR-151](../08-decision-log.md#adr-151--the-route-line-geometry-we-said-hong-kong-did-not-publish-has-existed-since-2021): the Transport Department **does** publish road-following route lines (CSDI, since 2021), covering **93%** of our route-directions at ~4 KB each. Covers the interactive map and a `MapProvider` seam that keeps LandsD as one option, per-route polylines fetched on demand, the resolver for the two traps in that data (`ROUTE_SEQ` is not a direction; a short-working scores the same as its parent), live user location, a scroll-linked camera, LandsD Streetscape, and **four interaction paradigms for Route detail with clickable [mockups](./mockups/route-detail/) on real KMB 1 geometry** (2026-08-22). |
> | [07 — Watch a stop](./07-watch-a-stop.md) | **Proposal, nothing built.** From a stop's menu on Route detail, **Watch** it: a floating pill above the tab bar then carries that route-at-stop around the app, and a tap expands it into a card. A watch is *a favourite you are using right now* — same `formatFavoriteRouteKey` identity, one narrowed `watch([{ stopId, routeIds }])` target, a different lifetime. **Five pill shapes and three card shapes with a clickable [mockup](./mockups/watch/)** that puts every arm (Due · stale · nothing due · board failed · offline) through all of them, plus the seven decisions that have to be made first — how many watches, how long one lives, what it says on the screen that already shows that board, and what a screen reader hears from a figure that changes every 45 s (2026-09-20). |
