...
| 4 ⬅ next | **Search** | the keypad, chips and recents are pure interaction over a spec'd index; never walked in a browser | Interaction-heavy specs. |
...
| **WP6-5** ⬅ next | Search: spec + port | Walked in a browser for the first time — the visual pass `docs/11` has owed since ADR-037 | M |
