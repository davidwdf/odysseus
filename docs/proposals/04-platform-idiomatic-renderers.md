...
| **WP6-8** ⬅ next | Retire `apps/mobile` — **both WP6-7b blockers are closed** (ADR-098 the favourite affordance, ADR-099 Inter); what remains in `docs/07`'s "WP6-8 blockers" section is ambers and `defect-both` items, none of which retiring `apps/mobile` causes or fixes. | Every spec green on `apps/web`; `expo`, `react-native`, `nativewind`, `reanimated`, `gesture-handler` leave the lockfile; `packages/ui` keeps generating for three platforms | M |
