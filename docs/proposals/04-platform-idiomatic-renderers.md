...
> **Status:** work plan, **in progress — WP6-0 … WP6-7 are done** (2026-08-03/05,
...
| 5 ✅ | **Route detail** | the schematic, the bus tokens, the collapsing header, the auto-scroll | **The motion test** — the first screen where "motion is idiom" is a real claim rather than a slogan. **Half answered by WP6-6a, and the surprise came from the other direction.** This table calls Place detail *"the most domain rules in the app"*; Place detail had nine and this screen had **sixteen** — a bus's position, the origin exception, both header labels, the facts strip and the boarding anchor among them (ADR-093). And the format found a defect **by being unable to look at something**: a spec's vocabulary is text, so the conformance walker cannot see a bus token at all — which is how it came out that the screen's signature graphic had no accessible name and was invisible to a screen reader. *A graphic carrying information a rider acts on needs a name, and naming it is what makes it projectable.* **WP6-6b answered the motion question itself**, and the answer is narrower than the slogan: **motion is idiom; what the motion is about is not** ([ADR-094](../08-decision-log.md#adr-094--motion-is-idiom-what-the-motion-is-about-is-not)). A token's slide, tween and bob are curve and physics; which node it is at is the model's, and it is projected in eight states. The same line settled the collapsing header (idiom in behaviour, identity in content) and the auto-scroll (the DOM screen owns no offset at all). Writing it found three more defects — a readout composed as one animated string, an empty rail whenever a measurement never arrives, and a loop whose collapsed label *is* its destination. |
| 6 ✅ | **Settings · About · FAQ** | mostly chrome and prose | **"Cheap; last" was wrong about the cheap part, and the reason is the format rather than the screens.** These are the only surfaces in the app with no `DataSource` call, and the spec format requires all five canonical states — which were designed as branches over a fetch. Answering that honestly meant **inverting the `slots`/`shows` split**: put the whole screen in `slots`, and `shows: []` becomes a claim that *everything* is there on the first frame (ADR-096). Two more things fell out. A preference screen's content is a **choice**, and *which* option is lit is a rule with two traps in it, neither expressible as a type. And the walker's blind spot arrived from the other side — it reads presence, not visibility — which rules out `<details>`, catches a `›` glyph diverging from an SVG, and surfaced a **live accessibility defect**: `react-native-web` drops `accessibilityState`, so six controls on the shipping PWA announced nothing (ADR-097). |
...
| **WP6-7** ✅ | Settings · About · FAQ: `settingsView`/`aboutView`/`faqView` + 12 corpus cases, three specs, three `apps/web` screens, six suites, and `Placeholder.tsx`/`ShellPreferences.tsx` **deleted** ([ADR-096](../08-decision-log.md#adr-096--a-screen-with-no-data-still-has-five-states-and-attribution-is-one-of-them) · [ADR-097](../08-decision-log.md#adr-097--the-conformance-walker-sees-presence-not-visibility--and-an-aria-state-it-cannot-see-is-one-a-rider-may-not-be-getting-either)) | Ported; the stale `faqOfflineA` strings refreshed — **and four more that were staler**, including one describing a rule ADR-042 deleted. `apps/web` has **zero** unported destinations | S→M |
| **WP6-7b** ✅ | *(slipped in ahead of WP6-8, by request)* The parity audit: walk `apps/web` screen by screen against the `apps/mobile` it replaces, and separate identity gaps from capability gaps from intended idiom | A written gap list with owners, and the cheap gaps closed. ADR-075 makes idiom free to differ, so the audit's job is to find what a rider **loses**, not what looks different | M |
| **WP6-8** ⏸ *deferred* | **Retire `apps/mobile` — deferred by [ADR-113](../08-decision-log.md#adr-113--appsmobile-is-not-retired-at-the-end-of-wave-6-it-becomes-the-reference), at the owner's request.** Its precondition is met and both WP6-7b blockers are closed (ADR-098, ADR-099); what defers it is that the RN app is wanted as the **reference** for unported UI/UX decisions, and that keeping it declines the one-renderer risk this document already flags below. The specs still bind it; idiom does not. | Unchanged when it does happen. Until then: `apps/mobile` keeps passing every spec and the shared corpus, and is owed no idiom. | M |
| **WP6-9** ⬅ next | The first native repo consumes `contract/ui/` | One screen in SwiftUI passing the same specs — **the honest test of the whole thesis**, and the first time the Swift/Kotlin token artefacts are compiled at all | L |
