...
| `mapRoot` | Home | A **context card** over a full-bleed map, a sheet that owns the scroll, and **no back control** — a root is switched to, never pushed ([ADR-183](./08-decision-log.md#adr-183--home-becomes-map-backed-and-the-taxonomys-empty-cell-is-filled-rather-than-stretched)). Its own kind rather than a widened `map`, because `map`'s card *encloses* the back control (ADR-170) and a kind whose defining feature is absent is a kind that has been stretched. This card does not collapse: two lines and a control have nothing to reclaim. |
| `map` | Route detail | A **context card** floating over a full-bleed map ([ADR-156](./08-decision-log.md)). Since [ADR-170](./08-decision-log.md#adr-170--the-route-header-ships-one-enclosing-card-a-numbered-rail-and-an-island-for-the-collapse) the expanded card **encloses** the back control rather than sitting beside it — one `BackButton`, wearing a `flat` prop while it stands on the card's pane — and the collapsed state is a **centred island**, a separate element rather than the same one transitioned. The map is the screen; everything else floats over it. |

**The two remaining empty cells are not gaps to fill.** A collapse exists to keep a back control reachable
while the title gets out of the way, so a root screen has nothing to collapse *for*.

> 🟢 **`mapRoot` is the cell this section said was worth revisiting, filled** (ADR-183). It arrived exactly
> as the note asked — a deliberate edit to a declared set rather than a fifth undeclared answer — and
> `header-taxonomy.test.tsx` failed the moment Home changed shape and before the taxonomy caught up, which
> is the whole reason it is data.
