...
> **Index** — all 152 decisions, in numeric order. The entries below are in the order
...
| [049](#adr-049--the-basemap-is-the-hk-lands-departments-self-cached-with-labels-as-a-per-locale-overlay) | The basemap is the HK Lands Department's, self-cached, with labels as a per-locale overlay | Amended by ADR-051 |
...
| [154](#adr-154--the-gallery-covers-every-component-in-the-renderer-and-the-directory-is-what-says-so) | The gallery covers every component in the renderer, and the directory is what says so | Accepted |
| [155](#adr-155--an-operator-chips-two-colours-are-a-pair-and-the-pair-is-measured) | An operator chip's two colours are a pair, and the pair is measured | Accepted |
...
- **Status:** **Amended by ADR-051** — `TileSource`'s canonical home is `packages/ports`, not the app
...
## ADR-154 — The gallery covers every component in the renderer, and the directory is what says so

- **Status:** **Accepted** — **2026-08-15.** `apps/web/lab/samples.tsx` (23 groups over all 24 exported
  components, in three tiers), its corpus access split out into `apps/web/lab/goldens.ts`, the browser that
  reads them in `apps/web/lab/Components.tsx`, and the new gate
  `apps/web/test/gallery-covers-components.test.ts`. Extends
  [ADR-134](#adr-134--the-design-system-gallery-is-enumerated-from-the-published-specs-not-hand-listed)
  and pays the rest of what
  [ADR-150](#adr-150--the-freshness-notice-reaches-all-four-screens-and-offline-stops-being-a-state-nothing-could-see)'s
  live-samples consequence started.
- **Context.** The owner asked for the whole set — *"find all the components (both primitive components and
  multi-part components) … render each one in each state so I can review individual parts and see the
  complex parts too."* ADR-150 had drawn three: `FeedNotice`, `StopRow` and `PlaceRow`. The other twenty-one
  were listed only as *prose*, if at all: the gallery's listing half is enumerated from
  `packages/contract/ui/*.spec.json`, and **most of what `src/components/` holds has no spec of its own** —
  `BusGlyph`, `MiniMap`, `SlideNumber`, `BottomSheet`, `RailBusToken`, `JourneyLines` are all real components
  with real states and no `.spec.json`. The spec-driven gate could not have noticed any of them missing.
- **Decision 1 — the coverage gate's subject is the source directory, not the published specs.**
  `gallery-covers-specs.test.ts` reads `packages/contract/ui/`; this one reads `apps/web/src/components/` and
  asserts the same relationship in both directions — every exported component has a sample group, and every
  group names a component that exists. That is the only subject that can be complete: a set enumerated from
  the specs is complete only for the components that happen to have one, and a hand-maintained list is
  complete until the third new component. A component counts as an `export function` with a PascalCase name,
  which is the whole convention in that directory; a component written as an arrow `const` fails this gate on
  the file that introduces it, which is the right moment to have that conversation rather than a silent gap.
- **Decision 2 — three tiers, because a reviewer asks three different questions.** A **leaf** is judged as a
  *scale* (are the three urgency tones one family? do the eight compass octants read as one quantisation?),
  a **composed** component as a *projection* (does every arm of the kernel's view get drawn?), and an
  **overlay** as an *interruption*. The tier is a field on the group rather than a section in the page, so a
  new component picks its own and a tier the page does not render is a red build.
- **Decision 3 — an overlay mounts behind a trigger, and this is not laziness.** `BottomSheet` calls
  `showModal()` in a mount effect. Eleven of those open at once is eleven modal dialogs fighting over one
  focus trap and a page nobody can scroll. Behind a trigger the reviewer gets the **real** component — the
  two-stage entrance, the drag-to-dismiss, the rubber band — one at a time, which is also how a rider meets
  it. It keeps the gate honest as a side effect: the sample under test is the trigger, so jsdom is never
  asked for `showModal`, which it does not implement.
- **Decision 4 — a state that draws nothing declares what *kind* of nothing.** On the page, a glyph that
  rendered, a state whose whole correctness is drawing nothing, and a panel that threw are **one empty box**.
  So `Sample.draws` is `'no-text'` or `'nothing'`, and the gate asserts the stronger thing in each case:
  `no-text` must draw an element and no words, `nothing` must draw neither. It was a single boolean for the
  length of one test run, and the run is what killed it — a boolean could not tell a bare `BusGlyph` from the
  `SaveStar` whose `hideWhenEmpty` correctly renders `null`, and demanding an element from the second is how
  that was found.
- **Decision 5 — a sweep is allowed only where a token table or a function owns the axis.** Nearly every
  panel is a corpus golden or the kernel call the screen makes. Three are not: the eight compass octants
  (`bearingOctantDeg`'s own quantisation), the operator liveries (`OPERATOR_ACCENT`'s own keys, so a fifth
  operator appears the day its colours land) and a glyph at three sizes. Each is **read off the thing that
  owns it** rather than typed out, and each says so in its own `how` line. What is never invented here is an
  ETA, a stop name or a fare.
- **Decision 6 — coverage and *reading* are two jobs, and the first one shipped without the second.** The
  set landed as one flat scroll and the owner's verdict was immediate: *"This is helpful, but now it's very
  cluttered."* Ninety-four panels down one page is a listing you can prove things about and cannot use. So
  the sample **data** is untouched — `lab/samples.tsx` is still the one declaration, and the gate still
  reads it — and `lab/Components.tsx` is a browser over it: a sidebar that **selects** rather than only
  scrolls (one component's states, nothing else on screen — the Storybook affordance), a filter over
  component names *and state names* (what you remember is usually the state you saw it in), and a **Notes**
  toggle, because the prose is what makes the panels trustworthy and is also most of the ink. Selection
  lives in the hash (`#components/EtaBadge`) so a review comment can point at a component rather than at a
  scroll position.
- **Decision 7 — the tabs are reordered to what the lab is for, and Components is the default.** Owner's
  ask. It opened on `#rail`, which was true when the rail rig was the only thing here; the order is now
  **Components · Design system · Rail motion · Bus glyphs**. The design-system page keeps `#gallery` — an
  address in three ADRs and the docs is not worth churning — and it stops drawing components entirely. It
  lists **contracts**; one tab over is the picture. Most of `src/components/` has no spec, so the listing
  is the smaller half now, and it says so.
- **Decision 8 — appearance is the *real* preference, and "Light + dark" is a lab-only extra on top.** The
  toggle writes `usePreferences.setAppearance` through the same `resolveMode` rule both apps share, from
  `APPEARANCES` — a second notion of what `auto` means, on the one page whose job is checking that things
  agree, would be the exact mistake this log keeps recording. The consequence is stated rather than hidden:
  flipping it here flips the app on this origin, because **it is the same control**. Side-by-side compare is
  different in kind — it needs *both* themes at once, which the document cannot be — so each pane sets the
  token vars inline from `@nextbus/ui`'s `themes`, which is byte-for-byte what `apps/mobile` hands to
  NativeWind's `vars(themes[mode])`. One declaration, now three callers, and **no `.light` class invented in
  the lab**.
- **What writing it found.**
  - 🟠 **`StopName`'s `emphasis` is a no-op in light and all but invisible in dark** — and it took the
    side-by-side compare to see the first half. In the light theme `--accent` and `--text` are *the same
    value* (`17 24 39`); in dark they are `226 232 240` against `244 246 250`. So the rider's own boarding
    stop is emphasised in three places and the third — the one `StopName` was given a prop for, and which
    `apps/web` was written up as *missing* until WP6-7b — contributes nothing on either appearance. This is
    the Ink identity working as designed (ADR-029 makes the accent the *ink*), which is exactly why it needs
    deciding rather than fixing: either the boarding stop gets an emphasis token of its own, or the rule
    stops claiming the name is one of its cues. `docs/07`.
  - 🟠 **The lab has never had a prose measure, for the same reason it had no geometry.** `max-w-3xl` is not
    generated — the app has no long-form text, so it has no width for it — and every paragraph on the
    gallery had been running the full width of the window since ADR-134, unnoticed because nobody reads a
    lab page at 1400 px on purpose. It joins `border-r` and the compare panes' light theme in
    `lab/index.html`'s own `<style>`: three rules that cannot be an inline style, in a file `build:web`
    never sees, which is the seam ADR-112 actually leaves open.
  - 🟠 **Indexing into a card is indexing into whatever survived the cap.** The urgency sweep asked for row 8
    of `the-urgency-bands-at-every-boundary`, which has eight rows because `stopCardView` caps at the served
    `maxRows`. `cardRowWhere(name, band)` asks the question the panel is actually about and keeps working
    when the policy that decides the cap moves. The same reasoning replaced three other fixed indices with
    finders (`stopRowWhere`, `stopRowWithArrivals`).
  - 🟠 **A comparison panel that drew the same picture twice.** `BearingArrow`'s "as a card draws it" picked
    two goldens that happen to share a bearing, so a panel whose whole point was the rounding demonstrated it
    with one arrow drawn twice. Three cases with genuinely different bearings now, printed with their
    degrees.
- **Consequences:**
  - 🟢 All 24 exported components are drawn, in 94 panels, every one from a corpus golden or a kernel call,
    with the states of a component side by side at a phone's width — and reachable in one click rather than
    one scroll.
  - 🟢 **A new component with no gallery panel is a red build**, in the tier the reviewer would look for it.
  - 🟠 **`apps/mobile` has 31 components and no equivalent page at all.** The RN app cannot render in this
    gallery — it is a DOM page — and its `/workbench` route lists tokens rather than components. That is the
    ADR-069 asymmetry again, in the one place a reviewer would most notice it, and it is a `docs/07` row
    rather than something this change could close.
  - 🟢 **A component is now a URL.** `#components/RouteStopRow` opens that component and nothing else,
    which is the unit a review comment wants to name.
  - 🟠 **The locale is fixed at `en`.** Every panel is composed in the corpus goldens' own language, so the
    bilingual half of rule 5 is reviewable only where a golden happens to be Chinese (`EtaBadge`'s due arm,
    `RemarkTag`, `StopName`'s CJK case). A locale switch over the whole page is a `docs/07` row; done
    carelessly it would show English data under Chinese chrome, which is a worse picture than none.
  - 🟡 **The `MiniMap` panels need `pnpm dev:edge` running**, because the tiles are proxied by our own
    Worker. Without it they draw the pins on an empty plate, which is a state the app can genuinely be in and
    is therefore not the wrong picture — only a confusing one to meet by accident.

## ADR-155 — An operator chip's two colours are a pair, and the pair is measured

- **Status:** **Accepted** — **2026-08-15.** `packages/ui/tokens.json` (`color.operatorText.LWB` → the dark
  ink) with its seven emitted artefacts, the contrast block in `docs/09` §Operator accents, and the gate in
  `apps/web/test/search-contrast.test.ts`. It changes a **value**, not a rule: this is
  [ADR-015](#adr-015--theme--design-system-token-architecture--livery-themes)'s *"themes are value swaps"* doing
  exactly the job it was written for, and it leaves
  [ADR-029](#adr-029--collapse-to-a-single-ink-theme-lightdarkauto-retire-the-livery-axis)'s retirement of the livery axis
  untouched — the operator accents survived that and still do. What it adds is the measurement
  [ADR-129](#adr-129--two-things-a-conformance-walker-cannot-see-colour-and-geometry) brought to the
  semantic tokens, one layer over — from *which token a string may be drawn in* to *whether a chip's two
  colours may sit on each other*.
- **Context.** The owner, looking at the component lab's livery sweep: *"for the LWB route chip, can we also
  do dark text inside this. Like the CTB route chip, the white text is too light."* They are right, and the
  number is worse than "too light": **white on LWB's `#E8A33D` is 2.16:1**, against the 4.5:1 a 14 px bold
  route number owes. Every Long Win chip in the app has been effectively unreadable since Wave 1.
- **Decision 1 — both *light* accents take the dark ink, and the rule stops naming an example.**
  `tokens.json` said *"The yellow CTB accent always pairs with dark text, never white"*, which is a rule
  stated as an instance — so it was applied as an instance. LWB's gold is barely darker than CTB's yellow
  (relative luminance **0.437** against **0.604**; both are far above the 0.18 or so where white starts to
  work) and it got white anyway. The description now states the *class* — the light accents — and the table
  below carries the ratios rather than the reasoning.
- **Decision 2 — the pairing is a gate, because a colour is not a word.** `conformStates` reads text, so a
  chip drawn in an unreadable colour projects identically to a readable one — the same blind spot ADR-098
  names for interaction destinations and the one
  [ADR-129](#adr-129--two-things-a-conformance-walker-cannot-see-colour-and-geometry) answered for the
  *semantic* tokens with arithmetic — this is the same answer for a colour **pair**. `search-contrast.test.ts` now measures each operator's two
  published colours against each other at **`AA_BODY`, not `AA_LARGE`**: a route number is `text-label`
  (14 px) and bold, two points short of the 18.66 px that would buy the 3:1 threshold, and it is the densest
  string in the app — a rider scanning a column of chips for "969C" is reading, not glancing.
  The control that makes it non-vacuous is `Object.keys(OPERATOR_ACCENT)`: **`searchView` derives its
  operator chips from the dataset index** precisely so a fifth operator lights up the day its adapter lands
  (ADR-037), and the colours are the one part of that which is not automatic. There is a second, injected
  control that asserts the *old* value would have failed — kept rather than run once and deleted, because
  "every livery passes" only reassures if the check can tell.
- **Decision 3 — the fix is in `tokens.json` and nowhere else.** Seven artefacts regenerate from it,
  including `NextBusTokens.swift` and `NextBusTokens.kt`, so the iOS and Android chips are already correct
  and `apps/mobile`'s `RouteChip` changed without being touched. `check-tokens-current.mjs` makes a stale
  copy a red build. This is ADR-015's *"themes are value swaps"* doing exactly the job it was written for.
- **What writing it found.**
  - 🟠 **GMB is at 4.71:1 and KMB at 4.96:1** — both pass, both within a nudge of failing. Published as a
    table in `docs/09` rather than left to be rediscovered, because the next person to lighten that red or
    that green needs to see the margin before they do it, not after.
- **Consequences:**
  - 🟢 Every operator chip clears AA on both renderers and on both native targets, from one edit.
  - 🟢 **A fifth operator cannot ship an unreadable chip** — the gate enumerates the accent table rather
    than a list of four.
  - 🟢 **The lab paid for itself in a day.** Four liveries had never been drawn side by side before
    [ADR-154](#adr-154--the-gallery-covers-every-component-in-the-renderer-and-the-directory-is-what-says-so)'s
    sweep, and the defect was visible in the first minute of looking at it. That is the second finding from
    that page in a day (the first being `StopName`'s inert `emphasis`), and both are of the same shape: a
    *set* is only reviewable when it is shown as a set.
  - 🟡 **The other three colour pairings in the app are still unmeasured.** `--positive`/`--warning` on the
    arrival readouts are a `docs/07` row already; the operator *line* on the route schematic and the map
    pin's core-against-ring are not. The helper is now here for whoever takes them.
