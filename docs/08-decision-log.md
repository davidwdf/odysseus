...
> **Index** — all 132 decisions, in numeric order. The entries below are in the order
...
| [008](#adr-008--etas-are-approximations-no-client-side-fake-countdown) | ETAs are approximations; no client-side fake countdown | Amended by ADR-123 |
...
- **Status:** **Amended by ADR-123** — the staleness *treatment* is now a muted `~` rather than a fade. Otherwise **Accepted** — Golden rule 3, and one of the most-cited decisions in the repo: no client-side
