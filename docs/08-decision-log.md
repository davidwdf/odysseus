...
> **Index** — all 156 decisions, in numeric order. The entries below are in the order
...
| # | Decision | Status |
| --- | --- | --- |
| [001](#adr-001--monorepo-with-pnpm--turborepo) | Monorepo with pnpm + Turborepo | Accepted |
| [002](#adr-002--expo-rn--rn-for-web-pwa-first-native-later-ota) | Expo (RN + RN-for-Web), PWA-first, native later, OTA | Superseded by ADR-075 |
| [003](#adr-003--cloudflare-stack-for-hosting--infra) | Cloudflare stack for hosting & infra | Accepted |
| [004](#adr-004--phased-hybrid-data-layer-behind-a-datasource-interface) | Phased hybrid data layer behind a `DataSource` interface | Accepted |
| [005](#adr-005--canonical-normalized-data-model-gtfs-backbone--per-operator-crawl) | Canonical normalized data model (GTFS-backbone + per-operator crawl) | Accepted |
| [006](#adr-006--v1-operators-kmblwb--citybus-only) | v1 operators: KMB/LWB + Citybus only | Amended by ADR-014, ADR-047 |
| [007](#adr-007--nearby-computed-on-device) | "Nearby" computed on-device | Amended by ADR-016 |
| [008](#adr-008--etas-are-approximations-no-client-side-fake-countdown) | ETAs are approximations; no client-side fake countdown | Amended by ADR-123, ADR-133 |
| [009](#adr-009--nativewind-for-the-cross-platform-design-system) | NativeWind for the cross-platform design system | Amended by ADR-075 |
| [010](#adr-010--client-state-tanstack-query--zustand) | Client state: TanStack Query + Zustand | Accepted |
| [011](#adr-011--maps-via-maplibre-tentative-phase-15) | Maps via MapLibre (tentative, Phase 1.5) | Amended by ADR-049 |
| [012](#adr-012--lintformat-biome) | Lint/format: Biome | Accepted |
| [013](#adr-013--pre-commit-documentation-freshness-check-skill--hook) | Pre-commit documentation-freshness check (skill + hook) | Amended by ADR-078 |
| [014](#adr-014--localize-en--繁體中文--简体中文-from-v1) | Localize EN / 繁體中文 / 简体中文 from v1 | Accepted |
| [015](#adr-015--theme--design-system-token-architecture--livery-themes) | Theme & design system (token architecture + livery themes) | Amended by ADR-029 |
| [016](#adr-016--slice-1-server-side-v1nearby-on-device-index-deferred) | Slice 1: server-side `/v1/nearby` (on-device index deferred) | Amended by ADR-021, ADR-055 |
| [017](#adr-017--design-system-realization-fonts-text-scale-elevation-themed-nav-chrome) | Design-system realization: fonts, `<Text>` scale, elevation, themed nav chrome | Amended by ADR-054, ADR-099 |
| [018](#adr-018--two-axis-theme-livery--appearance-with-persistence) | Two-axis theme (livery × appearance) with persistence | Amended by ADR-029 |
| [019](#adr-019--cjk-use-the-platform-font-do-not-bundle-noto-v1) | CJK: use the platform font; do **not** bundle Noto (v1) | Accepted |
| [020](#adr-020--slice-2-stoproute-detail--favorites--canonical-id-reconciliation) | Slice 2: Stop/Route detail + Favorites + canonical id reconciliation | Accepted |
| [021](#adr-021--citybus-and-kmb-static-data-from-the-hkbus-consolidated-dataset) | Citybus (and KMB) static data from the hkbus consolidated dataset | Amended by ADR-055 |
| [022](#adr-022--same-kerb-stop-merge-our-own-conservative-landmarkdistance-clustering) | Same-kerb stop-merge: our own conservative landmark+distance clustering | Amended by ADR-042 |
| [023](#adr-023--eta-lists-are-de-duplicated-once-server-side-canonical-api) | ETA lists are de-duplicated once, server-side (canonical API) | Amended by ADR-072 |
| [024](#adr-024--stop-card-navigation-stop-vs-route-are-distinct-tap-targets) | Stop-card navigation: stop vs. route are distinct tap targets | Accepted |
| [025](#adr-025--iconography-lucide-via-an-icon-primitive-on-the-token-system) | Iconography: Lucide via an `<Icon>` primitive on the token system | Accepted |
| [026](#adr-026--nearby-is-a-flat-list-not-cards-surface-distance--walk-time) | Nearby is a flat list, not cards; surface distance + walk time | Accepted |
| [027](#adr-027--floating-tab-bar-content-scrolls-underneath) | Floating tab bar; content scrolls underneath | Accepted |
| [028](#adr-028--liquid-glass-material--ink-livery) | Liquid-glass material + Ink livery | Amended by ADR-029 |
| [029](#adr-029--collapse-to-a-single-ink-theme-lightdarkauto-retire-the-livery-axis) | Collapse to a single "Ink" theme (light/dark/auto); retire the livery axis | Accepted |
| [030](#adr-030--route-view-as-a-vertical-schematic-line-strip-with-two-state-bus-tokens) | Route view as a vertical schematic line-strip with two-state bus tokens | Amended by ADR-033, ADR-132 |
| [031](#adr-031--british-english-oxford--ize-spelling-for-all-prose--user-facing-strings) | British English (Oxford `-ize` spelling) for all prose & user-facing strings | Accepted |
| [032](#adr-032--favourites-are-route-at-stop-pairs-not-bare-routes) | Favourites are **route-at-stop** pairs, not bare routes | Amended by ADR-042 |
| [033](#adr-033--route-header-no-bar-background-title-morphs-into-a-pill-beside-the-back-lens) | Route header: no bar background; title morphs into a pill beside the back lens | Accepted |
| [034](#adr-034--nearby-shows--destination-per-route-one-stopname-for-title-cased-names-app-wide) | Nearby shows "→ destination" per route; one `StopName` for title-cased names app-wide | Accepted |
| [035](#adr-035--elevation-is-two-channels-opaque-shadowlighten-and-glass-defocus-led) | Elevation is two channels: opaque (shadow↔lighten) and glass (defocus-led) | Accepted |
| [036](#adr-036--surface-fares-frequency-journey-time--eta-remarks-from-data-we-already-fetch) | Surface fares, frequency, journey time & ETA remarks from data we already fetch | Accepted |
| [037](#adr-037--search-on-device-index-a-smart-route-keypad-and-extensible-filter-chips) | Search: on-device index, a smart route keypad, and extensible filter chips | Amended by ADR-063 |
| [038](#adr-038--about-the-data-screen-open-data-attribution--honesty-notes) | "About the data" screen: open-data attribution & honesty notes | Accepted |
| [039](#adr-039--one-back-button-everywhere-the-floating-glass-backbutton) | One back button everywhere: the floating glass `BackButton` | Accepted |
| [040](#adr-040--dont-scrape-app1933-for-live-electric--occupancy-curate-or-crowd-source-instead) | Don't scrape App1933 for live electric / occupancy; curate or crowd-source instead | Accepted |
| [041](#adr-041--stop-detail-a-collapsing-header-shared-with-route-a-keyless-static-mini-map-and-an-enriched-summary) | Stop detail: a collapsing header (shared with Route), a keyless static mini-map, and an enriched summary | Amended by ADR-045, ADR-049 |
| [042](#adr-042--direction-aware-same-kerb-clustering-n-member-places-supersedes-adr-022s-pair-merge--invariant) | Direction-aware same-kerb clustering (N-member places); supersedes ADR-022's pair-merge + invariant | Amended by ADR-071, ADR-072 |
| [043](#adr-043--a-core-navigation-animation-system-cross-fade-tabs-slide-and-reveal-stack-web-swipe-back) | A core navigation-animation system: cross-fade tabs, slide-and-reveal stack, web swipe-back | Accepted |
| [044](#adr-044--route-badges-are-tap-to-expand-fare-stage-timeline-per-band-frequency--hours-concession-estimates) | Route badges are tap-to-expand: fare-stage timeline, per-band frequency & hours, concession estimates | Accepted |
| [045](#adr-045--stop-detail-mini-map-pinned-with-brand-coloured-labelled-dots-and-a-scroll-linked-pole-highlight) | Stop detail mini-map: pinned, with brand-coloured labelled dots and a scroll-linked pole highlight | Amended by ADR-049 |
| [046](#adr-046--route-detail-direction-toggle-server-resolved-reverse-an-in-card-fromto-header-and-a-circular-route-treatment) | Route detail direction toggle: server-resolved reverse, an in-card from/to header, and a circular-route treatment | Accepted |
| [047](#adr-047--green-minibus-gmb-a-third-operator-keyed-on-gtfsid-with-per-arrival-livescheduled-honesty) | Green Minibus (GMB): a third operator, keyed on `gtfsId`, with per-arrival live/scheduled honesty | Accepted |
| [048](#adr-048--pwa-install-metadata-web-app-manifest--ios-apple-touch-icon-via-a-custom-html) | PWA install metadata: web app manifest + iOS `apple-touch-icon` via a custom `+html` | Accepted |
| [049](#adr-049--the-basemap-is-the-hk-lands-departments-self-cached-with-labels-as-a-per-locale-overlay) | The basemap is the HK Lands Department's, self-cached, with labels as a per-locale overlay | Amended by ADR-051, ADR-154 |
| [050](#adr-050--stop-imagery-google-street-view-deep-link-now-hk-streetscape-360-as-the-inline-target) | Stop imagery: Google Street View deep link now, HK Streetscape 360 as the inline target | Accepted |
| [051](#adr-051--layered-package-boundaries-packagesports-is-declaration-only-and-imports-nothing) | Layered package boundaries; `packages/ports` is declaration-only and imports nothing | Accepted |
| [052](#adr-052--the-wire-contract-zod-is-the-single-declaration-types-erase-and-the-schema-stays-additive-safe) | The wire contract: Zod is the single declaration, types erase, and the schema stays additive-safe | Accepted |
| [053](#adr-053--the-line-the-server-owns-content-order-counts-and-text-the-client-owns-layout-colour-and-motion) | The line: the server owns content, order, counts and text; the client owns layout, colour and motion | Accepted |
| [054](#adr-054--design-tokens-and-i18n-as-generated-cross-platform-artefacts) | Design tokens and i18n as generated cross-platform artefacts | Accepted |
| [055](#adr-055--content-addressed-precompute-to-kvr2-the-dataset-leaves-the-request-path) | Content-addressed precompute to KV/R2: the dataset leaves the request path | Accepted |
| [056](#adr-056--the-live-protocol-frames-a-sharded-hibernating-etahub-and-what-we-could-not-verify) | The live protocol: frames, a sharded hibernating `EtaHub`, and what we could not verify | Accepted |
| [057](#adr-057--live-eta-ttl-is-30-s-and-every-upstream-call-is-coalesced-per-pole) | Live ETA TTL is 30 s, and every upstream call is coalesced per pole | Accepted |
| [058](#adr-058--offline-is-a-service-worker-a-persisted-query-cache-and-a-remembered-fix--not-a-new-data-tier) | Offline is a service worker, a persisted query cache and a remembered fix — not a new data tier | Amended by ADR-124 |
| [059](#adr-059--the-id-grammar-one-parser-in-core-the-spec-and-corpus-in-contract) | The id grammar: one parser in `core`, the spec and corpus in `contract` | Accepted |
| [060](#adr-060--the-fixture-corpus-is-the-equivalence-mechanism-for-domain-rules) | The fixture corpus is the equivalence mechanism for domain rules | Accepted |
| [061](#adr-061--environments-and-configuration-topology-local--production-ephemeral-previews-and-no-staging-tier) | Environments and configuration topology: local + production, ephemeral previews, and no staging tier | Accepted |
| [062](#adr-062--the-favourite-key-is-the-member-pole-and-the-scheme-is-versioned) | The favourite key is the member pole, and the scheme is versioned | Amended by ADR-071 |
| [063](#adr-063--the-search-indexs-order-is-data-a-precomputed-sortkey-range-scans-a-content-hash-version-and-an-etag) | The search index's order is data: a precomputed `sortKey`, range scans, a content-hash version and an ETag | Accepted |
| [064](#adr-064--the-error-taxonomy-the-status-code-and-the-code-are-one-decision) | The error taxonomy: the status code and the code are one decision | Accepted |
| [065](#adr-065--routeservice-is-two-named-schemas-not-one-optional-field-the-fidelity-tier-is-in-the-type) | `Route.service` is two named schemas, not one optional field: the fidelity tier is in the type | Accepted |
| [066](#adr-066--the-colo-cache-key-carries-the-build-hash-so-a-dataset-flip-invalidates-by-construction) | The colo cache key carries the build hash, so a dataset flip invalidates by construction | Accepted |
| [067](#adr-067--the-contract-is-published-for-native-consumers-and-every-part-we-cannot-verify-says-so) | The contract is published for native consumers, and every part we cannot verify says so | Accepted |
| [068](#adr-068--the-clients-derived-view-is-kernel-logic-so-a-second-renderer-calls-it-rather-than-reading-the-jsx) | The client's derived view is kernel logic, so a second renderer calls it rather than reading the JSX | Accepted |
| [069](#adr-069--a-second-renderer-and-what-it-caught-in-the-first) | A second renderer, and what it caught in the first | Accepted |
| [070](#adr-070--a-turbo-tasks-hash-must-include-everything-it-reads-and-says-so) | A turbo task's hash must include everything it reads, and says so | Accepted |
| [071](#adr-071--what-counts-as-one-boarding-point-and-what-a-rider-is-told-about-two) | What counts as one boarding point, and what a rider is told about two | Accepted |
| [072](#adr-072--an-arrival-is-a-line-at-a-kerb-not-a-line-at-a-place) | An arrival is a line at a kerb, not a line at a place | Accepted |
| [073](#adr-073--a-failed-board-is-not-an-empty-board-per-pole-eta-failure-on-the-wire) | A failed board is not an empty board: per-pole ETA failure on the wire | Accepted |
| [074](#adr-074--the-live-rounds-corpus-one-table-two-runtimes-and-the-rule-that-binds-two-engines) | The live rounds corpus: one table, two runtimes, and the rule that binds two engines | Accepted |
| [075](#adr-075--three-renderers-one-executable-spec-and-drift-defined-on-the-spec-rather-than-the-pixels) | Three renderers, one executable spec, and drift defined on the spec rather than the pixels | Amended by ADR-100 |
| [076](#adr-076--the-live-engine-is-selected-by-the-environment-and-the-default-stays-poll) | The live engine is selected by the environment, and the default stays `poll` | Amended by ADR-121 |
| [077](#adr-077--a-card-can-say-we-could-not-ask-and-a-failure-list-must-not-outlive-its-round) | A card can say "we could not ask", and a failure list must not outlive its round | Accepted |
| [078](#adr-078--rule-7-is-enforced-per-commit-over-a-range-and-an-empty-range-is-a-failure) | Rule 7 is enforced per commit over a range, and an empty range is a failure | Accepted |
| [079](#adr-079--one-request-per-round-the-batch-eta-endpoint-and-nearby-as-a-live-adopter) | One request per round: the batch ETA endpoint, and Nearby as a live adopter | Amended by ADR-124 |
| [080](#adr-080--what-tells-two-boarding-points-apart-in-the-order-the-data-can-support-it) | What tells two boarding points apart, in the order the data can support it | Accepted |
| [081](#adr-081--the-frames-carry-failed-and-a-round-whose-failure-set-moved-is-news) | The frames carry `failed`, and a round whose failure set moved is news | Accepted |
| [082](#adr-082--the-web-shell-before-the-web-screens-a-router-over-a-declared-destination-set-and-one-pwa-policy-for-two-apps) | The web shell before the web screens: a router over a declared destination set, and one PWA policy for two apps | Amended by ADR-125 |
| [083](#adr-083--a-component-spec-is-data-with-five-words-and-the-projection-is-what-pins-it) | A component spec is data with five words, and the projection is what pins it | Accepted |
| [084](#adr-084--a-screen-spec-a-state-that-declares-what-it-shows-and-a-slot-that-references-another-spec) | A screen spec: a state that declares what it shows, and a slot that references another spec | Accepted |
| [085](#adr-085--the-place-screens-composition-is-a-kernel-function-and-the-words-it-joins-are-injected) | The Place screen's composition is a kernel function, and the words it joins are injected | Accepted |
| [086](#adr-086--two-readings-that-were-honest-and-useless-a-walk-in-hours-and-poles-at-one-coordinate) | Two readings that were honest and useless: a walk in hours, and poles at one coordinate | Accepted |
| [087](#adr-087--the-maps-pins-are-content-and-the-dots-label-is-the-headings-own-code) | The map's pins are content, and the dot's label is the heading's own code | Accepted |
| [088](#adr-088--place-details-spec-its-dom-port-and-the-gate-that-finally-reads-both-renderers) | Place detail's spec, its DOM port, and the gate that finally reads both renderers | Accepted |
| [089](#adr-089--a-favourite-is-a-riders-own-data-so-its-migration-is-a-shared-rule-rather-than-a-stores-private-business) | A favourite is a rider's own data, so its migration is a shared rule rather than a store's private business | Amended by ADR-125 |
| [090](#adr-090--a-mustnot-a-component-cannot-satisfy-is-a-statement-about-its-producer) | A `mustNot` a component cannot satisfy is a statement about its producer | Accepted |
| [091](#adr-091--the-keypad-and-the-result-list-are-one-filtered-set-and-a-chip-set-is-the-indexs-answer) | The keypad and the result list are one filtered set, and a chip set is the index's answer | Accepted |
| [092](#adr-092--a-spec-cannot-hold-an-interaction-but-it-can-hold-what-a-rider-infers-from-one) | A spec cannot hold an interaction, but it can hold what a rider infers from one | Accepted |
| [093](#adr-093--which-node-a-bus-is-at-is-content-where-that-node-is-on-screen-is-geometry) | Which node a bus is at is content; where that node is on screen is geometry | Accepted |
| [094](#adr-094--motion-is-idiom-what-the-motion-is-about-is-not) | Motion is idiom; what the motion is about is not | Amended by ADR-100 |
| [095](#adr-095--the-estimate-mark-is-content-and-so-is-the-separator-between-two-day-names) | The estimate mark is content, and so is the separator between two day names | Amended by ADR-100 |
| [096](#adr-096--a-screen-with-no-data-still-has-five-states-and-attribution-is-one-of-them) | A screen with no data still has five states, and attribution is one of them | Accepted |
| [097](#adr-097--the-conformance-walker-sees-presence-not-visibility--and-an-aria-state-it-cannot-see-is-one-a-rider-may-not-be-getting-either) | The conformance walker sees presence, not visibility — and an ARIA state it cannot see is one a rider may not be getting either | Accepted |
| [098](#adr-098--a-spec-can-declare-an-interaction-and-no-gate-will-check-where-it-goes) | A spec can declare an interaction and no gate will check where it goes | Accepted |
| [099](#adr-099--the-same-font-declaration-four-static-cuts-on-native-and-one-variable-file-on-the-web) | The same font declaration, four static cuts on native and one variable file on the web | Accepted |
| [100](#adr-100--the-apps-signature-motion-and-material-are-identity-platform-conventional-detail-is-idiom) | The app's signature motion and material are identity; platform-conventional detail is idiom | Accepted |
| [101](#adr-101--a-data-router-because-the-point-of-view-transitions-is-the-shared-element-rather-than-the-fade) | A data router, because the point of View Transitions is the shared element rather than the fade | Accepted |
| [102](#adr-102--searchs-query-lives-in-the-url-and-replace-is-what-makes-that-safe) | Search's query lives in the URL, and `replace` is what makes that safe | Accepted |
| [103](#adr-103--the-bottom-sheet-keeps-every-property-the-dialog-was-chosen-for) | The bottom sheet keeps every property the `<dialog>` was chosen for | Accepted |
| [104](#adr-104--a-url-change-is-not-an-excuse-for-no-motion) | A URL change is not an excuse for no motion | Accepted |
| [105](#adr-105--check-no-derivation-was-blind-inside-every-template-literal) | `check-no-derivation` was blind inside every template literal | Accepted |
| [106](#adr-106--the-scroll-spys-line-is-the-maps-own-edge-and-fixed-chrome-outranks-sticky-content) | The scroll-spy's line is the map's own edge, and fixed chrome outranks sticky content | Accepted |
| [107](#adr-107--a-bounce-has-to-be-centred-on-the-thing-it-bounces-on) | A bounce has to be centred on the thing it bounces on | Accepted |
| [108](#adr-108--the-rail-overlays-re-measure-never-attached-and-a-hidden-tab-hid-it) | The rail overlay's re-measure never attached, and a hidden tab hid it | Accepted |
| [109](#adr-109--scrollrestoration-restores-the-window-and-the-list-that-loses-its-place-is-not-the-window) | `<ScrollRestoration>` restores the window, and the list that loses its place is not the window | Accepted |
| [110](#adr-110--the-rails-resting-place-is-css-only-its-travel-is-measured) | The rail's resting place is CSS; only its travel is measured | Accepted |
| [111](#adr-111--an-ordinal-is-a-slot-not-a-bus-the-rail-gains-an-entrance-and-an-exit) | An ordinal is a slot, not a bus: the rail gains an entrance and an exit | Accepted |
| [112](#adr-112--a-dev-page-lives-in-the-app-and-a-gate-keeps-it-out-of-the-app) | A dev page lives in the app, and a gate keeps it out of the app | Amended by ADR-134 |
| [113](#adr-113--appsmobile-is-not-retired-at-the-end-of-wave-6-it-becomes-the-reference) | `apps/mobile` is not retired at the end of Wave 6; it becomes the reference | Accepted |
| [114](#adr-114--eta-null-on-every-stop-meant-three-different-things-and-the-route-view-could-not-say-which) | `eta: null` on every stop meant three different things, and the route view could not say which | Accepted |
| [115](#adr-115--the-sheet-a-rider-already-opens-is-where-one-stops-times-go) | The sheet a rider already opens is where one stop's times go | Accepted |
| [116](#adr-116--a-route-nobody-can-ask-about-in-bulk-gets-one-durable-object-of-its-own) | A route nobody can ask about in bulk gets one Durable Object of its own | Accepted |
| [117](#adr-117--a-narrowed-read-narrows-the-questions-not-just-the-answers) | A narrowed read narrows the questions, not just the answers | Accepted |
| [118](#adr-118--a-route-watch-polls-on-the-operators-clock-and-the-counter-that-proves-a-round-happened) | A route watch polls on the operator's clock, and the counter that proves a round happened | Accepted |
| [119](#adr-119--the-client-can-watch-a-route-and-the-two-engines-get-there-differently-on-purpose) | The client can watch a route, and the two engines get there differently on purpose | Accepted |
...
| [121](#adr-121--the-socket-is-the-default-engine-because-polling-a-route-costs-19-what-it-should) | The socket is the default engine, because polling a route costs 19× what it should | Amended by ADR-135 |
...
| [123](#adr-123--staleness-is-a-board-level-fact-so-a-per-reading-cue-is-the-wrong-unit) | Staleness is a board-level fact, so a per-reading cue is the wrong unit | Amended by ADR-133 |
...
| [127](#adr-127--an-adr-number-is-an-address-so-the-log-is-indexed-rather-than-consolidated) | An ADR number is an address, so the log is indexed rather than consolidated | Accepted |
| [128](#adr-128--a-fare-sheet-with-nothing-in-it-is-the-strip-contradicting-itself) | A fare sheet with nothing in it is the strip contradicting itself | Accepted |
| [129](#adr-129--two-things-a-conformance-walker-cannot-see-colour-and-geometry) | Two things a conformance walker cannot see: colour and geometry | Accepted |
| [130](#adr-130--one-declaration-two-apps-the-deep-link-fallback-and-an-enforcement-claim-nobody-evaluates) | One declaration, two apps: the deep-link fallback, and an enforcement claim nobody evaluates | Accepted |
| [131](#adr-131--a-react-19-ref-callback-that-both-registers-and-observes-must-return-one-cleanup) | A React 19 ref callback that both registers and observes must return one cleanup | Accepted |
| [132](#adr-132--the-bus-glyphs-two-vehicles-and-the-three-rules-that-had-never-been-written-down) | The bus glyphs: two vehicles, and the three rules that had never been written down | Accepted |
| [133](#adr-133--a-screen-says-once-that-it-has-stopped-being-fed-and-never-a-fourth-sentence) | A screen says once that it has stopped being fed, and never a fourth sentence | Accepted |
| [134](#adr-134--the-design-system-gallery-is-enumerated-from-the-published-specs-not-hand-listed) | The design-system gallery is enumerated from the published specs, not hand-listed | Accepted |
| [135](#adr-135--the-live-path-hardened-against-the-networks-own-failure-modes-wp6-8b) | The live path hardened against the network's own failure modes (WP6-8b) | Accepted |
| [136](#adr-136--the-batch-endpoint-learns-the-route-question-v1etasroute) | The batch endpoint learns the route question: `/v1/etas?route=` | Accepted |
| [137](#adr-137--every-edge-request-carries-a-deadline-a-hung-fetch-is-a-failure-on-time) | Every edge request carries a deadline: a hung fetch is a failure, on time | Accepted |
| [138](#adr-138--the-dataset-download-carries-its-own-deadline-because-its-memo-only-clears-on-rejection) | The dataset download carries its own deadline, because its memo only clears on rejection | Accepted |
| [139](#adr-139--the-tile-proxy-joins-fetchupstream-the-workers-last-bare-fetch-gets-the-deadline) | The tile proxy joins `fetchUpstream`: the Worker's last bare fetch gets the deadline | Accepted |
| [140](#adr-140--a-round-that-answered-nothing-records-nothing-the-fabricated-polled-empty-row) | A round that answered nothing records nothing: the fabricated "polled, empty" row | Accepted |
| [141](#adr-141--a-route-watchs-pole-resolution-is-retried-on-the-poll-cadence-not-fired-once) | A route watch's pole resolution is retried on the poll cadence, not fired once | Accepted |
| [142](#adr-142--an-arrival-we-cannot-read-is-departed-not-nan-min) | An arrival we cannot read is departed, not "NaN min" | Accepted |
| [143](#adr-143--a-corrupt-preferences-blob-is-quarantined-not-a-splash-screen-held-for-ever) | A corrupt preferences blob is quarantined, not a splash screen held for ever | Accepted |
| [144](#adr-144--soonest-first-means-the-readings-a-departed-or-empty-board-is-a-dash-and-dashes-go-last) | "Soonest first" means the readings: a departed or empty board is a dash, and dashes go last | Accepted |
| [145](#adr-145--a-writer-that-has-never-persisted-cannot-have-deleted-anything-the-null-base-list-ancestor-is-empty) | A writer that has never persisted cannot have deleted anything: the null-base list ancestor is empty | Accepted |
| [146](#adr-146--three-small-honesty-repairs-at-the-edge-the-live-doors-500-the-frame-caps-units-the-kmb-bucket-key) | Three small honesty repairs at the edge: the live door's 500, the frame cap's units, the KMB bucket key | Accepted |
| [147](#adr-147--a-pulled-forward-round-has-a-floor-one-upstream-round-per-gap-whatever-the-reconnect-rate) | A pulled-forward round has a floor: one upstream round per gap, whatever the reconnect rate | Accepted |
| [148](#adr-148--the-tile-hang-detector-bounds-the-connection-not-the-riders-download) | The tile hang detector bounds the connection, not the rider's download | Accepted |
| [149](#adr-149--three-alignments-the-review-surfaced-the-mid-session-quarantine-the-kmb-co-the-retry-cadence) | Three alignments the review surfaced: the mid-session quarantine, the KMB `co`, the retry cadence | Accepted |
| [150](#adr-150--the-freshness-notice-reaches-all-four-screens-and-offline-stops-being-a-state-nothing-could-see) | The freshness notice reaches all four screens, and `offline` stops being a state nothing could see | Accepted |
| [151](#adr-151--the-route-line-geometry-we-said-hong-kong-did-not-publish-has-existed-since-2021) | The route line geometry we said Hong Kong did not publish has existed since 2021 | Accepted |
| [152](#adr-152--route-geometry-comes-from-csdi-joined-on-gtfsid-resolved-at-the-edge) | Route geometry comes from CSDI, joined on `gtfsId`, resolved at the edge | Accepted |
| [153](#adr-153--a-route-line-is-chosen-by-coverage-oriented-by-the-riders-stops-and-trimmed-to-their-terminals) | A route line is chosen by coverage, oriented by the rider's stops, and trimmed to their terminals | Accepted |
| [154](#adr-154--mapprovider-widens-the-basemap-seam-and-the-baselabel-zoom-split-becomes-a-kernel-rule) | `MapProvider` widens the basemap seam, and the base/label zoom split becomes a kernel rule | Accepted |
| [155](#adr-155--route-detail-draws-its-line-and-the-specs-subject-is-the-sentence-under-it) | Route detail draws its line, and the spec's subject is the sentence under it | Accepted |
| [156](#adr-156--route-detail-becomes-a-map-with-a-sheet-over-it-assembled-from-parts) | Route detail becomes a map with a sheet over it, assembled from parts | Accepted |

<!-- END ADR INDEX -->

## ADR-001 — Monorepo with pnpm + Turborepo
- **Status:** **Accepted** — The shape of the repo, unchanged since day one — pnpm workspaces + Turborepo,
  internal packages source-only with no build step (golden rule 1).
...
- **Status:** **Superseded by ADR-075** — [ADR-075](#adr-075--three-renderers-one-executable-spec-and-drift-defined-on-the-spec-rather-than-the-pixels)
  took the one-codebase premise apart: native will be hand-written against the contract, and the shipping
  web renderer is `apps/web` ([ADR-069](#adr-069--a-second-renderer-and-what-it-caught-in-the-first)), not
  react-native-web. The Expo PWA still ships until WP6-8 and
  [ADR-113](#adr-113--appsmobile-is-not-retired-at-the-end-of-wave-6-it-becomes-the-reference) keeps
  `apps/mobile` in the repo afterwards as the reference, so the code outlives the decision.
...
- **Status:** **Accepted** — Workers, KV, R2 and Durable Objects are what the edge is actually built on;
  [ADR-055](#adr-055--content-addressed-precompute-to-kvr2-the-dataset-leaves-the-request-path) (dataset
  shards in KV/R2) and
  [ADR-056](#adr-056--the-live-protocol-frames-a-sharded-hibernating-etahub-and-what-we-could-not-verify)
  (the hibernating `EtaHub`) are this decision cashed in.
...
- **Status:** **Accepted** — The `DataSource` seam is golden rule 2, and two gates enforce it rather than
  one: `pnpm boundaries` on the import graph and `check-view-transport-free` on the source
  ([ADR-056](#adr-056--the-live-protocol-frames-a-sharded-hibernating-etahub-and-what-we-could-not-verify)).
...
- **Status:** **Accepted** — The canonical model is `packages/contract`'s Zod schemas
  ([ADR-052](#adr-052--the-wire-contract-zod-is-the-single-declaration-types-erase-and-the-schema-stays-additive-safe))
  with the id grammar of
  [ADR-059](#adr-059--the-id-grammar-one-parser-in-core-the-spec-and-corpus-in-contract); normalization
  stays out of the request path
  ([ADR-055](#adr-055--content-addressed-precompute-to-kvr2-the-dataset-leaves-the-request-path)). The GTFS
  backbone was investigated and rejected as a *substitute* source in
  [ADR-021](#adr-021--citybus-and-kmb-static-data-from-the-hkbus-consolidated-dataset).
...
- **Status:** **Amended by ADR-014, ADR-047** — the v1 *language* line here was overtaken by
  [ADR-014](#adr-014--localize-en--繁體中文--简体中文-from-v1) (which says so in its own **Why**), and GMB
  shipped as a third operator in
  [ADR-047](#adr-047--green-minibus-gmb-a-third-operator-keyed-on-gtfsid-with-per-arrival-livescheduled-honesty)
  — so "KMB/LWB + Citybus only" no longer describes v1. The rest of the operator list is still tracked in
  [`docs/07`](./07-backlog.md).
...
- **Status:** **Amended by ADR-016** — On-device nearby was deferred the moment Slice 1 shipped:
  `/v1/nearby` is computed in the Worker
  ([ADR-016](#adr-016--slice-1-server-side-v1nearby-on-device-index-deferred)), now off precomputed shards
  ([ADR-055](#adr-055--content-addressed-precompute-to-kvr2-the-dataset-leaves-the-request-path)). The
  on-device index is still the stated target and still unbuilt.
...
- **Status:** **Amended by ADR-123, ADR-133** — the staleness *treatment* is now a muted `~` rather than a fade. Otherwise **Accepted** — Golden rule 3, and one of the most-cited decisions in the repo: no client-side
  countdown, tabular figures, "Arriving/Due" under a minute, honest staleness.
  [ADR-122](#adr-122--the-stale-cue-fired-on-healthy-data-because-90-s-is-shorter-than-a-citybus-readings-age)
  is the most recent correction to *how* staleness is judged, not to the rule.
...
- **Status:** **Amended by ADR-075** — NativeWind is still the design system in `apps/mobile`, but it is no
  longer the *cross-platform* one: `apps/web` consumes the same generated preset as plain Tailwind 3.4
  ([ADR-054](#adr-054--design-tokens-and-i18n-as-generated-cross-platform-artefacts),
  [ADR-075](#adr-075--three-renderers-one-executable-spec-and-drift-defined-on-the-spec-rather-than-the-pixels),
  golden rule 4). react-native-reusables and the Reanimated/Moti motion stack stand where RN ships.
...
- **Status:** **Accepted** — TanStack Query for server state and Zustand for local state in both renderers;
  the web shell adds a persisted query cache
  ([ADR-058](#adr-058--offline-is-a-service-worker-a-persisted-query-cache-and-a-remembered-fix--not-a-new-data-tier),
  [ADR-082](#adr-082--the-web-shell-before-the-web-screens-a-router-over-a-declared-destination-set-and-one-pwa-policy-for-two-apps)).
...
- **Why:** Avoids Google Maps fees/licensing; works RN + Web. Nearby launches as a **list** first;
  map follows.
- **Status:** **Amended by ADR-049** — the tile-provider question is settled, and not by MapTiler:
  [ADR-049](#adr-049--the-basemap-is-the-hk-lands-departments-self-cached-with-labels-as-a-per-locale-overlay)
  chose the keyless Lands Department basemap behind the `TileSource` seam. MapLibre itself is still unbuilt
  and still the roadmap's route to an interactive map. *(As written, this said: "Tentative — confirm tile
  provider + cost before committing.")*

## ADR-012 — Lint/format: Biome
- **Status:** **Accepted** — Biome is the one lint/format tool — `pnpm lint` is `biome check .`, `pnpm
  format` writes.
...
- **Status:** **Amended by ADR-078** — The `check-docs` skill and the `PreToolUse` hook both still exist,
  but the rule is no longer only a local reminder:
  [ADR-078](#adr-078--rule-7-is-enforced-per-commit-over-a-range-and-an-empty-range-is-a-failure) made
  `scripts/precommit-docs-check.mjs` hold it **once** and CI apply it per commit over a pull request's
  range, where `[docs-ok]` is the only bypass and an empty range fails.
...
- **Status:** **Accepted** — Three locales ship (`en` / `zh-Hant` / `zh-Hans`) and bilingual is golden rule
  5. Data names are `I18nText`; UI strings live in `@nextbus/i18n`.
...
- **Status:** **Amended by ADR-029** — The three-layer token architecture stands and is now *generated*
  ([ADR-054](#adr-054--design-tokens-and-i18n-as-generated-cross-platform-artefacts)). The **livery** half
  does not: [ADR-029](#adr-029--collapse-to-a-single-ink-theme-lightdarkauto-retire-the-livery-axis) retired
  the livery axis for a single Ink theme in light/dark.
...
- **Status:** **Amended by ADR-021, ADR-055** — `/v1/nearby` is still computed server-side, but neither half
  of the mechanism survived. The KMB-only memoized index gave way to the multi-operator `static-index.ts`
  ([ADR-021](#adr-021--citybus-and-kmb-static-data-from-the-hkbus-consolidated-dataset)), and the
  per-isolate memo gave way to precomputed KV/R2 shards
  ([ADR-055](#adr-055--content-addressed-precompute-to-kvr2-the-dataset-leaves-the-request-path)) — which is
  what `"dataset":"kv"` in `/v1/health` reports.
...
- **Status:** **Amended by ADR-054, ADR-099** — The `<Text>` scale, the elevation tokens and the themed nav
  chrome stand. `TYPE_SCALE` moved into the generated tokens
  ([ADR-054](#adr-054--design-tokens-and-i18n-as-generated-cross-platform-artefacts), noted inline in
  decision 2 below), and how Inter is *loaded* is now per-renderer
  ([ADR-099](#adr-099--the-same-font-declaration-four-static-cuts-on-native-and-one-variable-file-on-the-web)).
...
- **Status:** **Amended by ADR-029** — The **appearance** axis and its persistence stand and are still what
  Settings exposes. The **livery** axis was retired by
  [ADR-029](#adr-029--collapse-to-a-single-ink-theme-lightdarkauto-retire-the-livery-axis).
...
- **Status:** **Accepted** — Still no bundled CJK webfont; CJK renders in the platform face.
  [ADR-099](#adr-099--the-same-font-declaration-four-static-cuts-on-native-and-one-variable-file-on-the-web)
  adds the web's Inter and says so explicitly — *"Latin only — ADR-019 is untouched"*.
...
- **Status:** **Accepted** — Slice 2 shipped. Its endpoints have been reshaped many times since, but the
  rule it established is the one that stands: **the app never speaks operator-native ids**, which is now the
  id grammar of [ADR-059](#adr-059--the-id-grammar-one-parser-in-core-the-spec-and-corpus-in-contract) and
  the `P:`/pole vocabulary of
  [ADR-042](#adr-042--direction-aware-same-kerb-clustering-n-member-places-supersedes-adr-022s-pair-merge--invariant)/[ADR-071](#adr-071--what-counts-as-one-boarding-point-and-what-a-rider-is-told-about-two).
...
- **Status:** **Amended by ADR-055** — The hkbus consolidated dataset is still the static source, but it is
  no longer fetched on the request path:
  [ADR-055](#adr-055--content-addressed-precompute-to-kvr2-the-dataset-leaves-the-request-path) precomputes
  it into content-addressed KV/R2 shards. The corrected subrequest-cap figure in the Context below was fixed
  in place (2026-08-10) rather than in a new decision.
...
- **Status:** **Amended by ADR-042** — **Superseded in part by [ADR-042](#adr-042--direction-aware-same-kerb-clustering-n-member-places-supersedes-adr-022s-pair-merge--invariant)** — the cross-operator *pair-only* merge and the "≤ 1 member per operator"
...
- **Status:** **Amended by ADR-072** — **Amended by [ADR-072](#adr-072--an-arrival-is-a-line-at-a-kerb-not-a-line-at-a-place)
...
- **Status:** **Accepted** — The stop name and each route row are still **sibling** tap targets, in both
  renderers, and the route-at-stop view is still what a route row opens.
...
- **Status:** **Accepted** — Lucide through one `<Icon>` primitive on the token system; the tone-to-token
  mapping is still how an icon re-skins with the theme.
...
- **Status:** **Accepted** — Nearby is still a flat list of rows with distance and walk time, and the
  honesty helpers (`formatDistance`, `walkMinutes`, `formatWalk`) still live in `@nextbus/core/geo`.
...
- **Status:** **Accepted** — The floating tab bar, and the one place its geometry is derived
  (`useTabBarLayout`), are still `apps/mobile`'s shell. `apps/web` reaches the same layered result its own
  way
  ([ADR-082](#adr-082--the-web-shell-before-the-web-screens-a-router-over-a-declared-destination-set-and-one-pwa-policy-for-two-apps),
  [ADR-100](#adr-100--the-apps-signature-motion-and-material-are-identity-platform-conventional-detail-is-idiom)).
...
- **Status:** **Amended by ADR-029** — The **glass material** stands — `GlassView`, the SVG-refraction port
  and the Chromium-only caveat with it. The **Ink livery** it introduced was folded into the single Ink
  theme by [ADR-029](#adr-029--collapse-to-a-single-ink-theme-lightdarkauto-retire-the-livery-axis).
...
- **Status:** **Accepted** — One Ink theme in light/dark, chosen by the appearance preference. The livery
  axis has not come back; re-introducing it is still a localized change.
...
- **Status:** **Amended by ADR-033, ADR-132** — **Implemented** for KMB/LWB (verified end-to-end in-browser against live data). Built on the
...
- **Status:** **Accepted** — **Implemented** (KMB/LWB, verified in-browser). Refines the collapsing header from
...
- **Status:** **Amended by ADR-042** — the stop half of the key is the raw **member pole** id, never a `P:`
  place id
  ([ADR-042](#adr-042--direction-aware-same-kerb-clustering-n-member-places-supersedes-adr-022s-pair-merge--invariant)
  point 1, and the marked paragraph below). **Framework built; save UI pending.** The store + Favourites tab
  were migrated to the
...
- **Status:** **Accepted** — Golden rule 5's spelling half — British English with Oxford `-ize`, code
  exempt. Applies to every doc in this repo, this line included.
...
- **Status:** **Accepted** — **Implemented** (KMB/LWB, verified in-browser).
...
- **Status:** **Accepted** — **Implemented** (verified in-browser, both modes). Documents shipped behaviour (`ELEVATION` +
...
- **Status:** **Accepted** — **Implemented** (verified against the live worker: `/v1/route/KMB:1:outbound:1` →
...
- **Status:** **Amended by ADR-063** — the index's ordering is data now — a precomputed `sortKey` and a
  content-hash version in place of the trie and the size-pair version
  ([ADR-063](#adr-063--the-search-indexs-order-is-data-a-precomputed-sortkey-range-scans-a-content-hash-version-and-an-etag)).
  **Implemented** (verified against the live worker: `/v1/index` → 2002 routes + 8126 stops
...
- **Status:** **Accepted** — **Implemented** (proposals [`00`](../docs/proposals/00-fast-and-fun-wins.md) P10; typecheck 7/7,
...
- **Status:** **Accepted** — **Implemented** (typecheck 7/7; Biome clean for the touched files).
...
- **Status:** **Accepted** — **Ruled out** (decision only; no code). Scopes [`docs/proposals/00`](../docs/proposals/00-fast-and-fun-wins.md) P15.
...
- **Status:** **Amended by ADR-045, ADR-049** — the mini-map's tiles are no longer OpenStreetMap's
  ([ADR-049](#adr-049--the-basemap-is-the-hk-lands-departments-self-cached-with-labels-as-a-per-locale-overlay))
  and its hero no longer scrolls away
  ([ADR-045](#adr-045--stop-detail-mini-map-pinned-with-brand-coloured-labelled-dots-and-a-scroll-linked-pole-highlight)).
  **Built & in-app** (2026-06-10). Refines the Stop-detail screen (`apps/mobile/app/stop/[id].tsx`).
...
- **Status:** **Amended by ADR-071, ADR-072** — **Amended by [ADR-071](#adr-071--what-counts-as-one-boarding-point-and-what-a-rider-is-told-about-two)
...
- **Status:** **Accepted** — **Partially implemented (web PWA)** — typecheck 7/7, Biome clean. **Shipped on web:** tab cross-fade,
...
- **Status:** **Accepted** — **Implemented** — typecheck 7/7, Biome clean, web bundles (3251 modules). Verified against the
...
- **Status:** **Amended by ADR-049** — the tiles under it are the Lands Department's now
  ([ADR-049](#adr-049--the-basemap-is-the-hk-lands-departments-self-cached-with-labels-as-a-per-locale-overlay)).
  **Built & verified on web** (2026-07-03). Extends
  [ADR-041](#adr-041--stop-detail-a-collapsing-header-shared-with-route-a-keyless-static-mini-map-and-an-enriched-summary)'s
...
- **Status:** **Accepted** — **Built & verified on web** (2026-07-04). Touches the `DataSource` seam (`@nextbus/core`), the edge
...
- **Status:** **Accepted** — **Built & verified end-to-end on the edge** (2026-07-09). Widens `OperatorId` in `@nextbus/core`; adds
...
- **Status:** **Accepted** — **Icons/manifest/head built (2026-07-14); on-device install not yet verified.** Adds
...
- **Status:** **Amended by ADR-051, ADR-154** — `TileSource`'s canonical home is `packages/ports`, not the app
  ([ADR-051](#adr-051--layered-package-boundaries-packagesports-is-declaration-only-and-imports-nothing)).
  **Decided 2026-07-26, implemented 2026-07-27** (WP0-2). `apps/edge/src/tiles.ts` proxies and
...
- **Status:** **Accepted** — **Decided 2026-07-26, not yet implemented.** Two-step: the deep link is unblocked; the inline
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-27/28** — ports (WP1-3) *and* the enforcement engine (WP1-4).
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-27** (WP1-1 of
...
- **Status:** **Accepted** — **Decided and partially implemented 2026-07-29** (WP3-4 of
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-29** (WP3-1 and WP3-2 of
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-27** (WP0-1 of
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-30** (Wave 5: WP5-0 … WP5-3), **extended 2026-07-31** with
...
- **Context:** [ADR-004](#adr-004--phased-hybrid-data-layer-behind-a-datasource-interface) has promised
  ...
  edits a screen while running a test. That is [WP4-0](#adr-068--the-clients-derived-view-is-kernel-logic-so-a-second-renderer-calls-it-rather-than-reading-the-jsx)'s situation again — an acceptance
...
- **Decision:**
  ...
  2. **`delta` can say `gone`, and the client must honour it.** Polling replaces the whole payload, so a
     ...
     [ADR-008](#adr-008--etas-are-approximations-no-client-side-fake-countdown) forbids, so `gone` is not an
  ...
  **Decisions 13–19 were forced by an adversarial review over the finished diff (2026-07-31).** Six
  ...
  16. **The hook that replaces a screen's `refetchInterval` returns the screen's clock.** `refetchInterval`
     ...
     [ADR-008](#adr-008--etas-are-approximations-no-client-side-fake-countdown) failure the whole honesty design
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-27** (WP0-4). Revises the 8 s/10 s TTLs chosen in
...
- **Status:** **Amended by ADR-124** — an offline query now resolves to `error` rather than parking in `pending`. Otherwise **Accepted** — **Decided and implemented 2026-07-27** (WP0-3). Closes the offline claim `docs/03` had been
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-28** (WP1-2). Implementation: `packages/core/src/ids.ts`;
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-28** (WP1-5). Implementation: `packages/core/spec/*.spec.json`,
...
- **Status:** **Accepted** — **Decided 2026-07-28.** The disarming half is implemented; the rest is guidance that WP0-5
...
- **Status:** **Amended by ADR-071** — [ADR-071](#adr-071--what-counts-as-one-boarding-point-and-what-a-rider-is-told-about-two)
  closes the orphaning hazard structurally — a cluster's poles fold onto boarding points — rather than by
  migration. **Decided and implemented 2026-07-28** (WP2-5). Implementation:
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-28** (WP2-7 of
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-28** (WP2-8 of
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-28** (WP2-9 of
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-28**, closing the one defect Wave 2's verification pass found.
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-29** (WP3-3). Implementation: `packages/contract/README.md`,
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-29** (WP4-0, the prerequisite Wave 4's plan row did not
...
- **Decision:**
  ...
  3. **`core` owns the rule; `i18n` owns the word** ([ADR-054](#adr-054--design-tokens-and-i18n-as-generated-cross-platform-artefacts)). `remaining` is a number and
...
- **The bug this found before any second renderer existed:** `EtaBadge` decided imminence with a literal
  ...
  [ADR-053](#adr-053--the-line-the-server-owns-content-order-counts-and-text-the-client-owns-layout-colour-and-motion)'s three-way arrival-cap disagreement one field over, and the **seventh** instance in
...
- **Consequences, including what we are accepting:**
  ...
  - **`remarkView` accepts `null` as well as `undefined`, and the type is wider than `Eta.remark`'s.**
    The app does no runtime validation ([ADR-052](#adr-052--the-wire-contract-zod-is-the-single-declaration-types-erase-and-the-schema-stays-additive-safe) decision 2), so an explicit `"remark": null`
  ...
  - **Two of the four gate scopes for `apps/web` are armed but unexercised until the package is real.**
    ...
    two-net design of [ADR-051](#adr-051--layered-package-boundaries-packagesports-is-declaration-only-and-imports-nothing) covers the gap is the reason it has two nets, and it is worth
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-29** (WP4-1, completing Wave 4). Implementation:
...
- **Context:** The plan calls WP4-1 *"the cheapest empirical test of the whole thesis"* — everything else
  ...
  [ADR-068](#adr-068--the-clients-derived-view-is-kernel-logic-so-a-second-renderer-calls-it-rather-than-reading-the-jsx) had to come first, because the acceptance presupposed a derived view that did not
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-29.** Implementation: `turbo.json` (root),
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-31** (WP5-10 + WP5-11, on `wave5-followups-v1`).
...
- **Status:** **Accepted** — **Decided and implemented 2026-07-31** (WP5-9, four commits on `wave5-followups-v1`). Closes
  [ADR-056](#adr-056--the-live-protocol-frames-a-sharded-hibernating-etahub-and-what-we-could-not-verify)
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-03** (WP5-4). Implementation: `apps/edge/src/eta-cache.ts`
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-03** (WP5-5). Implementation:
...
- **Status:** **Amended by ADR-100** — the invariant/idiom table is amended by
  [ADR-100](#adr-100--the-apps-signature-motion-and-material-are-identity-platform-conventional-detail-is-idiom):
  the signature motion and material are **identity**, platform-conventional detail is idiom. **Decided
  2026-08-03, not implemented.** Owner's decision. **Supersedes
...
- **Status:** **Amended by ADR-121** — the shipped default is the socket now, not `poll`
  ([ADR-121](#adr-121--the-socket-is-the-default-engine-because-polling-a-route-costs-19-what-it-should)) —
  the reasoning below is untouched, only the choice. **Decided and implemented 2026-08-03** (WP5-6).
  Implementation:
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-03** (WP5-13). Implementation:
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-03** (WP5-8). Implementation:
...
- **Status:** **Amended by ADR-124** — its `status === 'error'` refetch predicate could never fire until a parked query could reach `error`. Otherwise **Accepted** — **Decided and implemented 2026-08-03** (WP5-7). Implementation:
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-03** (WP5-12). Implementation:
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-03** (WP5-14). Implementation:
...
- **Status:** **Amended by ADR-125** — decision 5's two-app hazard is settled by a three-way merge, and its `scripts/pwa` one-declaration rule now covers the host-side deep-link fallback too (ADR-130). Otherwise **Accepted** — **Decided and implemented 2026-08-03** (WP6-0, the first row of Wave 6 —
...
- **Decisions:**
  ...
  3. **The tabs are a layout route, so the pushed destinations have no tab bar and do have a way back.**
     ...
     is the identical shape. Getting it right is what preserves [ADR-037](#adr-037--search-on-device-index-a-smart-route-keypad-and-extensible-filter-chips)'s
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-03** (WP6-1, the second row of Wave 6 —
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-03** (WP6-2, the third row of Wave 6). Implementation:
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-03** (WP6-3a — the hoist half of WP6-3). Implementation:
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-04.** Both found by opening the rewired Place screen at
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-04** as the first half of **WP6-3b**. Implementation:
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-05** — **WP6-3b**, which closes WP6-3. Implementation:
  ...
  [ADR-069](#adr-069--a-second-renderer-and-what-it-caught-in-the-first) recorded.
...
- **Status:** **Amended by ADR-125** — the blob is merged rather than overwritten. Otherwise **Accepted** — **Decided and implemented 2026-08-05** as the first half of **WP6-4**. Implementation:
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-05** — **WP6-4b**, which closes WP6-4. Implementation:
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-05** as the first half of **WP6-5**. Implementation:
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-05** — **WP6-5b**, which closes WP6-5. Implementation:
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-05** as the first half of **WP6-6**. Implementation:
...
- **Status:** **Amended by ADR-100** — the boundary it drew moved: signature motion is back on the identity
  side
  ([ADR-100](#adr-100--the-apps-signature-motion-and-material-are-identity-platform-conventional-detail-is-idiom)).
  **Decided and implemented 2026-08-05** as the second half of **WP6-6**, which closes it.
...
- **Status:** **Amended by ADR-100** — decision 8 is amended by
  [ADR-100](#adr-100--the-apps-signature-motion-and-material-are-identity-platform-conventional-detail-is-idiom).
  **Decided and implemented 2026-08-05** as **WP6-6c**, which closes **WP6-6** entirely.
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-07** as **WP6-7**, which closes the last unported
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-07**, alongside
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-08**, closing WP6-8's first blocker. Implementation:
...
- **Status:** **Accepted** — **Decided and implemented 2026-08-08**, closing WP6-8's second blocker. Implementation:
...
- **Status:** **Accepted** — **Decided by the owner 2026-08-08**, and it **amends ADR-075's invariant/idiom table, ADR-094,
...
- **Status:** **Accepted** — **Decided 2026-08-08** with the owner, implemented in `apps/web/src/shell/App.tsx`.
...
- **Status:** **Accepted** — **Decided 2026-08-08** with the owner, implemented in `apps/web/src/screens/Search.tsx` and
...
- **Status:** **Accepted** — **Decided 2026-08-08**, implemented in `apps/web/src/components/BottomSheet.tsx` (with
...
- **Status:** **Accepted** — **Decided 2026-08-08**, implemented in `apps/web/src/components/JourneyLines.tsx`,
...
- **Status:** **Accepted** — **Fixed 2026-08-08** in `scripts/check-no-derivation.mjs`.
...
- **Status:** **Accepted** — **Decided 2026-08-08**, implemented in `apps/web/src/screens/PlaceDetail.tsx` and the three
...
- **Status:** **Accepted** — **Fixed 2026-08-08** in `apps/web/src/index.css` and `apps/web/src/components/RailBusToken.tsx`,
...
- **Status:** **Accepted** — **Fixed 2026-08-08** in `apps/web/src/screens/RouteDetail.tsx`, after the owner sent a
...
- **Status:** **Amended by ADR-134** — **2026-08-09** at the owner's request — *"happy to keep it in the repo as a separate
...
- **Status:** **Amended by ADR-135** — decision 3's "there is no socket→poll fallback" stopped being true
  on 2026-08-12: a supervised, whole-subscription fallback now exists (see ADR-135 decision 4), which
  closes the cost decision 3 names. Everything else stands. Originally **2026-08-11** in
  `packages/api-client/src/live/select.ts` (`DEFAULT_LIVE_ENGINE`),
...
- **Status:** **Amended by ADR-133** — **2026-08-12.** Both per-reading treatments are removed from all four
...
- **Status:** **Accepted** — **2026-08-11**, `networkMode: 'always'` in both apps' `QueryProvider` and
  both Nearby screens branching on `isPending`. Amends
  [ADR-058](#adr-058--offline-is-a-service-worker-a-persisted-query-cache-and-a-remembered-fix--not-a-new-data-tier)
  (what offline *does*) and completes
  [ADR-079](#adr-079--one-request-per-round-the-batch-eta-endpoint-and-nearby-as-a-live-adopter)'s
  permanently-dead-screen fix, whose `status === 'error'` predicate could never fire.
- **Context.** Two 🔴 rows in `docs/07`, filed months apart against different screens, **were the same
  defect**: an offline paused fetch rendering *"No scheduled service"* on Nearby, and a failed `getStop`
  settling at `status: 'pending'`, `fetchStatus: 'idle'` on Place detail. Both are TanStack Query
  **parking** a fetch and a screen reading a parked query as an answer.
- **The mechanism, measured three ways** — a harness mounting the app's real `QueryProvider` (real
  `PersistQueryClientProvider`, real `onlineManager`/`focusManager`, real `retry: 1`), a live
  `pnpm dev:edge` + `pnpm dev:dom` driven in Chrome, and a read of `@tanstack/query-core@5.101.4`:
  - `networkMode` defaults to `'online'`, so while `onlineManager.isOnline()` is false the query function
    is **never invoked** — no data, no error, `fetchFailureCount: 0`, indefinitely.
  - `retryer.canContinue()` ANDs `focusManager.isFocused()`, so a retry scheduled while
    `document.visibilityState === 'hidden'` is parked until `visibilitychange`.
  - Both are `isError === false` **and** `isLoading === false`, because `isLoading` is
    `isPending && isFetching` and nothing is fetching. Nearby's
    `loc.loading || isLoading ? skeleton : isError ? error : list` therefore fell straight through to the
    list, where `nearbyView([])` is `[]` and the screen printed `noService`.
  - **And the `fetchStatus: 'idle'` that hid this for two waves** is what a parked query becomes when its
    last observer detaches: `Query.removeObserver` → `#isInitialPausedFetch()` →
    `retryer.cancel({ revert: true })`, restoring the pre-fetch state with the failure count **erased**.
    `<StrictMode>` and any navigation away both trigger it — which is why it looked like a query nobody
    had ever asked.
- **Decision, in two halves.**
  1. **`networkMode: 'always'` in both apps.** *A request we cannot make must fail, not vanish.* Three
     reasons it is right here specifically and not just convenient: this app **has a service worker that
     can answer**, so "offline" is not the same as "no possible response"; `navigator.onLine` lies behind
     a captive portal, which is the ordinary Hong Kong MTR-station case; and only `status === 'error'`
     lets ADR-079's error-path `refetchInterval` heal the screen when the network returns.
  2. **A screen's "we have no answer" arm branches on `isPending` (status alone), never on `isLoading`**,
     and its error arm is guarded by `query.data === undefined` so a *failed refresh* shows the last known
     list rather than the reason.
  The focus gate is deliberately **left in place**: parking a retry in a tab nobody is looking at is
  correct behaviour, and it resumes on `visibilitychange`.
- **Why this is ADR-073's rule and not a new one.** *"A failed board is not an empty board."* ADR-073 and
  ADR-077 fixed that conflation on the arrivals path — a pole that refused to answer stopped rendering as
  a quiet stop. This is the identical conflation one layer up: **our own silence, rendered as a claim
  about Hong Kong.** Nothing new was invented to say it; the existing vocabulary was reached.
- **Consequences:**
  - 🟢 Verified end to end against a killed and restarted Worker: a hidden tab shows the skeleton (was
    *"No scheduled service"*), a visible tab shows the failure, and **the screen heals itself when the
    Worker returns** — which it could never do before, because a parked query never reaches `error`.
  - 🟢 One fix closes two 🔴 rows and, per its own reasoning, the same shape on **every** screen: the
    `isLoading` branch was the house style.
  - 🔴 **Favourites carried the identical defect and it was the worst instance of the three.** With
    favourites saved, a hidden document and a rejecting `getStop`, both queries sat at
    `pending`/`paused`/`fetchFailureCount: 1` — one attempt made, the retry parked behind the focus gate
    this ADR deliberately keeps — so `loading` and `failure` were both falsy and **the rider saw the
    title and nothing else, on the one screen whose contents they curated by hand.** That is
    `favourites.spec.json`'s own `failed` `mustNot` ("a heading with an empty list, which reads as *you
    have nothing saved*"), reached through a door the file's comment was watching but not guarding.
    Fixed on both renderers, so the consequence above is now **measured on two screens** rather than
    asserted about all of them.
  - **The aggregate form of the rule, for a screen that fans out N queries:** branch on
    `nothingToShow && results.some(r => r.isPending)`, and **order the arms so the wait precedes the
    reason** — with one query per saved pole, a pole that has exhausted its retries must not answer for
    one that is still trying. That reordering was not cosmetic: it was a second, quieter defect, where
    one failed pole printed its error over the whole screen while another was still loading. `some`
    rather than `every`, because the spec already answers it — a card that has arrived is drawn
    immediately rather than held for its siblings, so "some parked, some answered" stays a partial list.
  - 🟠 **Neither renderer's Favourites can say "some poles answered and some failed".** The partial list
    is drawn with no marker, so a rider cannot tell a short list from a complete one.
    `StopCardView.incomplete` says this per *place* (ADR-077); nothing says it per *screen*.
  - 🟠 The failure arm prints the thrown error's message verbatim, as `nearby.spec.json` requires, so an
    offline rider reads *"Failed to fetch"* (Chrome) or *"Load failed"* (Safari) — **untranslated browser
    English.** Inventing a sixth `ErrorCode` for "the fetch never reached a server" was declined; the
    honest place to fix the wording is the four-sentence taxonomy in `docs/07`, not a fifth catalogue
    string.
  - 🟢 **The taxonomy now has its signal, which was the bonus asked for:** `error` with
    `data !== undefined` is *stale but trying*; `error` with `data === undefined` and a non-`EdgeRequestError`
    (a `TypeError` from fetch) is *our edge unreachable*; an `EdgeRequestError` carries the Worker's own
    `code`/`retryable` and is *upstream refused*. Nothing built here has to be torn out to add it.
  - ⚠️ **A suite that configures away the environment cannot see an environmental defect.**
    `place-detail-states.test.tsx` builds its own `QueryClient` with `retry: false` against a visible
    jsdom document, so **neither gate is reachable there** — which is exactly why the bug report said
    "the identical rejection lands on `error` in the test". Worth remembering before trusting a green
    suite about anything to do with network state.
  - ⚠️ An offline cold start now resolves to `error` rather than hanging in `pending`. That is the
    intent, but it means the offline path is `error` + persisted cache rather than `pending` + persisted
    cache, and ADR-058's four cache numbers are what keep the rider's last list on screen through it.

## ADR-125 — Preferences are merged, not overwritten, and the ancestor moves with the write

- **Status:** **Accepted** — **2026-08-11**, `mergePreferences`/`mergeSavedKeys` in
  `packages/core/src/favourites.ts` with 15 corpus rows, and a `PersistStorage` per app.
  Amends [ADR-089](#adr-089--a-favourite-is-a-riders-own-data-so-its-migration-is-a-shared-rule-rather-than-a-stores-private-business)
  (one blob, one key) and settles the hazard
  [ADR-082](#adr-082--the-web-shell-before-the-web-screens-a-router-over-a-declared-destination-set-and-one-pwa-policy-for-two-apps)
  decision 5 identified between two *apps*, which had arrived between two *tabs* of one.
- **Context.** Two tabs of the PWA silently overwrote each other's preferences — **including a rider's
  favourites, the only data in this app they make by hand.** Two writers share the `nextbus.preferences`
  key on one origin; zustand's `persist` reads it once at load and writes `partialize`'s output as the
  **whole** blob. So a second writer held a stale copy from the moment it loaded, and its next write did
  not merely fail to see the other's change — **it deleted it.** Verified rather than assumed:
  `AsyncStorage`'s web build writes `window.localStorage` under the **raw** key with no namespace of its
  own, so the Expo PWA and `apps/web` are the same two writers.
- **`docs/07` named the fix and the name was incomplete.** It said *"one `storage` listener per store that
  re-reads and merges"*. A listener cannot help a writer that was never told: a bfcached tab is **frozen**
  and `storage` events are not queued for it, which is the widest stale window there is (minutes to hours).
  So the merge has to sit on the **write** path; the listener is only what makes the other tab's change
  visible without a reload, and a `pageshow`-with-`persisted` re-read covers the window no event announces.
- **Decision, four parts.**
  1. **A three-way merge in the kernel** — `mergePreferences` / `mergeSavedKeys` — whose **ancestor is
     this writer's own previous persisted state**. Per-field last-writer-wins for the scalars; for the
     saved-key list, `theirs ∪ (mine \ ancestor) \ (ancestor \ theirs)`.
  2. **Read-modify-write on every save**, not just on an event.
  3. **The ancestor moves only *with* a write, and in exactly one place per store.** It means *"the last
     state this writer put on disk"*, so the `theirs` read in the same critical section is always a
     snapshot taken after it. The adopt path therefore deliberately does **not** advance it — nothing has
     been written at that point, and the `setState` it makes is what causes the write that does.
  4. **Read, merge, write and adopt happen in one critical section.** On web that is free, because
     `localStorage` is synchronous — so the merged state is applied *synchronously* rather than through a
     microtask. On native it costs a module-level promise queue plus re-reading `mine` when the write's
     turn comes, because AsyncStorage is asynchronous and `persist` never awaits a write.
- **Why set-union was rejected, and it matters:** a union is not merely lossy, it is **a worse bug than
  the one being fixed** — with a union, un-starring a route in one tab is undone by the other tab's stale
  copy, so a rider with two tabs open cannot delete a favourite at all.
- **Why the two stores diverge on purpose.** `apps/web` gets no queue: an async write would lose a tap
  made just before an unload, and the whole point of synchronous storage there is that the appearance is
  known before the first paint. They already diverge by storage (`getItem`'s guard, the `hydrated` flag);
  each file now says so and points at its twin. On **native** the merge and the listener are both gated
  on a web feature test — there is no second writer on a device, so behaviour is byte-identical to before.
- 🔴 **The first implementation of this ADR caused the data loss it was written to prevent, and that is
  the most useful thing in it.** The read-modify-write was unserialised, so the ancestor could advance past
  the snapshot a concurrent write was holding and `mergeSavedKeys` read `base \ theirs` as a remote
  deletion. Three manifestations, all reproduced before being fixed and **none needing a second tab**:
  two stars in one task erased the first; a `storage` event in the same task as a star erased the star;
  and an un-star plus a star in one task **resurrected the un-starred key**. Part 3 of the decision above
  is the half that fixes it and the half an obvious "just serialise the writes" fix does not.
- **The sharp edge cannot be closed by a signature, and that is written down rather than omitted.**
  `mergeSavedKeys`' precondition is about *the order two I/O operations happened in*, not about values.
  The only in-band way to detect a stale snapshot is a per-writer counter or a tombstone **on the blob**,
  and the envelope is frozen by a shipped app that reads it (ADR-082 d5). Deriving the remote base from
  "the last `theirs` we saw" was evaluated and breaks legitimate remote-deletion detection. So the
  precondition is in the function's contract and pinned by
  `mergeSavedKeys:an-ancestor-ahead-of-theirs-reads-an-addition-as-a-deletion`, a corpus row whose `why`
  says the arithmetic is **correct and must not be softened** — and which is in `check-spec-coverage`'s
  `REQUIRED_ROWS`, so it cannot be deleted to make a build green.
- 🔴 **And a second adversarial pass over the corrected fix found three more, one of them worse than the
  first.** Sixteen interleavings were run against the real stores; five findings, all now closed:
  - **BLOCKER — the web store advanced the ancestor past a write the disk refused.** `safeLocalStorage`
    swallows a `QuotaExceededError`, and the origin is genuinely fillable because the TanStack query cache
    persists through the same wrapper (ADR-058). So the ancestor named bytes the disk never took, and the
    next merge read this tab's own addition as the other writer's deletion. **No second tab, no second
    app** — a full disk was enough. `safeLocalStorage.setItem` returns `boolean` now, and both the
    ancestor advance and the catch-up `setState` are conditional on it. *Before the merge existed, a
    refused write only meant "not persisted"; the merge is what turned it into data loss.*
  - **MAJOR — memory adopted the merge on the wrong side of the `await`.** `setState(merged)` applies a
    value computed from `mine`, so it must land before any suspension point; after the write, a mutation
    arriving mid-flight was overwritten by a snapshot predating it. Reproduced at microtask depth 3 with
    no mocks — which is what any `async` handler that awaits before mutating looks like. The ancestor
    stays *after* the write, because it means something else. **Two adjacent lines, two different clocks.**
  - **A blob missing `favoriteRoutes` was destructive where an unparseable one was safe.** `readDisk`
    defaulted the absent field to `[]`, turning *"this writer did not say"* into *"this writer says none"*
    — every key a remote deletion. Both `readDisk`s now return `null` for it, which the kernel already
    reads as "nobody to merge with". **The more nearly-valid corruption was the dangerous one.**
  - **A hung write wedged the session.** `then(job, job)` continues from a *settlement*, and a promise
    that never settles is not one — so a wedged storage layer queued every later write for the rest of the
    session, silently. Each job is bounded now. (A **rejection** was already safe, and verified so: the
    chain drains, the ancestor never advanced, and no unhandled rejection escapes.)
  - 🟠 **One residual that is not fixable in the merge, stated rather than closed:** for the whole rollout
    window the *other* writer is a build without this commit — the installed, service-worker-cached Expo
    PWA that ADR-082 d5 names as a writer of this key. It writes the whole blob from a stale copy, and our
    handler correctly reads the difference as deletions. The soundness argument for trusting a deletion
    holds only once **every** writer on the origin merges. Never trusting one is the worse bug.
- **Consequences:**
  - 🟢 The on-disk envelope is unchanged (`{"state":…,"version":…}`, same key, same version), which is
    what keeps the two apps mutually readable.
  - 🟢 **Both second-round defects have regression tests that were watched failing** against the corrected
    code, and one of those tests was itself wrong first: `vi.spyOn(window.localStorage, 'setItem')` is a
    silent no-op, because jsdom's `localStorage` is a proxy whose own properties do not take an assignment
    — so the write went through and the interesting assertion was never reached. Spy on
    `Storage.prototype` instead. Worth knowing before stubbing storage anywhere else here.
  - ⚠️ **The pattern across both rounds is the lesson.** Every one of the six defects the merge introduced
    was a *sequencing* error around correct arithmetic — the kernel needed no change at either round. When
    a fix's correctness depends on the order two I/O operations happened in, the tests that matter are
    interleavings, and no type system will stand in for them.
  - 🟢 `settings.spec.json`'s `stale` `knownDefect` is closed — and classified **honestly** as
    `unenforced` rather than pointed at a slot that cannot observe it. See ADR-130's decision 2.
  - 🟠 **A write that fails silently still advances the ancestor**, so a swallowed quota error (Safari
    private browsing) is also a *forgotten deletion* — the other writer will resurrect the key. That
    makes `docs/07`'s open *"a preference that could not be saved is not reported to the rider"* row
    worth more than it looked: surfacing the failure would let the ancestor stay put and close this too.
  - 🟡 The query cache persists to the same origin under `nextbus.query.v1` with the same two-writer
    shape. Benign — it is a cache, not rider-authored data — but nothing has examined it.
  - ⚠️ Not walked by hand in two real tabs. Everything here is asserted in jsdom against a real
    `localStorage` and real `StorageEvent`s.

## ADR-126 — The navigation moment, and the document's language

- **Status:** **Accepted** — **2026-08-11**, `useNavigationMoment` in `apps/web` and a `lang` effect in
  both apps' `LocaleProvider`.
- **Context.** Three omissions in the web shell, filed separately, that turn out to be one instant and one
  unrelated attribute. (a) `<html lang>` was written once at build time — `apps/web`'s in a static
  `index.html`, `apps/mobile`'s by Expo substituting `%LANG_ISO_CODE%` at export — so **neither renderer
  ever knew the rider's language**, which mis-announces every word on the page and breaks the browser's
  own font selection for Chinese. (b) react-router does no scroll management, so a client-side push leaves
  `window.scrollY` where the previous screen left it. (c) After a client-side navigation the activated
  control is unmounted, focus falls back to `<body>`, and no live region exists — so a screen-reader rider
  is told nothing at all.
- **Decision 1 — scroll, focus and the announcement are ONE hook**, `useNavigationMoment`, because they
  are the same instant and three separate effects would be three definitions of *"a navigation happened"*.
  One definition: a changed `useLocation().key`, never the first commit, never a `REPLACE`. On an arrival
  it (1) scrolls the window to the top **only on a PUSH**, in a layout effect; (2) focuses the new
  screen's `<main>` (`tabIndex = -1`, `preventScroll`) **only when the navigation orphaned focus**, so a
  tab link keeps focus and Search's autofocused field keeps the keyboard; (3) sets a persistent, empty-at-
  load `role="status" aria-live="polite"` region to the destination's catalogue name, read off the matched
  route's `handle` — so no second path→name table exists.
- **`<ScrollRestoration>` is explicitly declined, and it is the second time it was the wrong answer.**
  It sets `history.scrollRestoration = 'manual'` and replaces the browser's back-restore with a single
  `scrollTo(0, y)` on the first commit of the POP — which on these screens is a **skeleton**. So it would
  trade away the direction that already works to fix the one that is broken, and it would also fire on
  Search's per-keystroke `replace`. `docs/07` recommended it in both rows; ADR-109 was the first refusal,
  for a different reason (Search does not scroll the window).
- **Decision 2 — a POP is not a scroll problem, and it *is* an arrival.** The scroll reset is push-only;
  the focus move and the announcement are not. A back navigation changes the page and nothing else says
  so — a screen reader announces a document *load*, not a client-side history entry — and react-router
  restores no focus, so the cursor sits on `<body>`. `preventScroll` is what makes the focus move free in
  that direction.
- **Decision 3 — `<html lang>` is written by the component that already knows the answer**: an effect in
  each app's `LocaleProvider` (a `DocumentLanguage` component on the RN side, web-only, rendering
  nothing). **No mapping table was added or needed** — the `Locale` union (`en` / `zh-Hant` / `zh-Hans`,
  the contract's `LocaleSchema`) is already the BCP-47 tag set. `index.html` keeps `lang="en"` as a
  documented pre-bundle default rather than gaining an inline script, which would be the second
  declaration of a storage key that the light-flash row already refuses to make.
- **Consequences:**
  - 🟠 **The scroll reset fires on a tab switch**, because a tab link is a PUSH — so each tab now starts
    at the top rather than remembering where it was. `apps/mobile` keeps all three tabs mounted and
    preserves them. A real behavioural difference between the renderers; it follows directly from the
    acceptance as written, and per-tab memory would be `useScrollRestoration`'s shape applied to the
    window.
  - 🟠 **The two destinations whose name is bus data announce nothing.** `/stop/:id` and `/route/:id`
    clear the live region rather than say a wrong thing, so a rider tapping a Nearby card gets the focus
    move and no words. One generic key each would fix it and is not written.
  - 🟠 **Neither collapsing header was measured.** jsdom has no `IntersectionObserver`, so no suite in
    this repo can see a collapse; the claim that the reset does not disturb them is **reasoned from where
    it lands in the frame**, and the code says so. Route detail's `scrollIntoView` reveal is argued the
    same way — it is a passive effect against a layout effect in the same commit, so it has the last word.
  - 🟡 `useScrollToTop.ts` exports `useNavigationMoment`; the filename is the brief's and the export is
    the truth. Renaming the file is free and touches two imports.
  - 🟡 Both apps' web manifests still pin `"lang": "en"`.

## ADR-127 — An ADR number is an address, so the log is indexed rather than consolidated

- **Status:** **Accepted** — **2026-08-11**. `scripts/check-adr-index.mjs`, in the `boundaries` chain.
- **Context.** The owner asked whether ADRs ever get consolidated and cleaned up. The log is 122 decisions,
  8,389 lines, 780 KB, **one flat file with no table of contents**, so every reader — human or agent —
  greps to find out what is still law.
- **Decision: never consolidate; index instead.** There are **2,068 `ADR-NNN` references outside
  `docs/08`** — 195 in `route-detail.spec.json` alone, plus the i18n catalogue, `packages/core/src/live.ts`,
  `openapi.json`, corpus fixtures and code comments. **An ADR number is a permanent address**, like a
  commit hash. Renumbering, merging or deleting invalidates all of it and nothing would catch a dangling
  one. Splitting into per-ADR files is also declined: 710 in-file anchors depend on `#adr-NNN--slug`, and
  the single-file grep is load-bearing.
- **Three things instead.**
  1. **A generated index** between the intro and ADR-001 — number, title, status, each row linking to its
     own anchor — emitted and diffed by the gate, exactly like `openapi.json` and the component specs.
  2. **A three-word status vocabulary** as the first bold token of the `Status` line: `Accepted`,
     `Amended by ADR-NNN`, `Superseded by ADR-NNN`. The token answers only *"can I still rely on this?"*;
     build state ("Framework built; save UI pending") is orthogonal and stays in the freeform prose after
     it. All 95 pre-existing lines keep their prose byte-for-byte behind the new token; 27 ADRs that had
     no `Status` at all were read individually rather than stamped.
  3. **A gate with six rules:** stale/missing index · missing status · unknown status · duplicate number ·
     **supersede reciprocity** · **dangling reference / broken anchor** over every tracked file. Plus a
     14-scenario `--selftest`, because a gate nobody has watched fail is not known to work.
- **Why reciprocity is the rule that earns this.** *"It is in an ADR" is not "the owner chose it."* The
  WP6-7b parity audit reclassified findings as intended idiom because a choice was written down somewhere,
  and ADR-100 later showed several of those were wrong. The mechanism behind that failure is now named:
  **an amendment recorded only in the amending ADR.** ADR-100's own body names three things it changed —
  the invariant/idiom table, and two later decisions that rested on it — and **not one of the three
  carried a Status line saying so**, so a reader landing on any of them read settled law that had been
  changed out from under it. **16 of the 20 ADR-to-ADR relations in the log were one-sided in exactly this
  way**; only four had ever been written on both sides. All 16 are now reciprocal.
- **Consequences:**
  - 🟢 It caught its author's own forward references within minutes of being wired up, and 11 previously
    broken anchors in the existing log.
  - 🟡 The reciprocity rule is one-directional by design: it fires when B's prose *actively* claims to
    change A. A passive `Amended by` written only on A's side is legal and unchecked — making it
    bidirectional means editing the *body* of the amending ADR.
  - 🟡 `ADR-NNN` survives as a placeholder only because it is spelled with letters — a numeric
    placeholder (all-zeroes, or any number nobody has used) is flagged, which is the rule working as
    intended rather than an edge case. This ADR's first draft tripped it.
  - 🟡 The gate does not validate links *out* of `docs/08`, so `docs/09` §5/§6's superseded banners —
    promised in ADR-084 and still prose-only — remain outside any rule's reach.

## ADR-128 — A fare sheet with nothing in it is the strip contradicting itself

- **Status:** **Accepted** — **2026-08-11**, `wholeRouteStage` in `packages/core/src/route-detail.ts`.
- **Context.** A route whose per-stop fares are not numbers opened an **entirely blank** fare sheet while
  the pill that opened it showed `$13.4`. Pinned as a `knownDefect` corpus row since WP6-6a.
- **The root cause is a shape, not a bug.** `fareStages` and `fareRange` reject an *identical* set of
  values (`null`, `''`, anything `Number()` reads as `NaN`), and two surfaces disagreed about what to do
  next: the strip fell through the rejection to `service.fareFull`, the sheet had no fallback at all. So
  this was never a defect in either guard — it was **the missing second half of one decision**, which
  existed for the pill and nowhere for the sheet.
- **Decision.** When `fareStages` yields nothing, the timeline falls back to **one stage spanning
  `1 … stops.length`** priced at `service.fareFull`, boarding at the origin row, and priced for
  concessions through the **existing** `concessionFigures` helper — so ADR-107's
  `min(adult, max($2, 20%))` cap is applied once rather than copied onto a second path. It says nothing
  about where the sections change: no invented boundaries, no caveat sentence, and it is **not
  distinguishable in the view from an ordinary stage**, because *"we cannot describe this route's
  sections"* is not something a rider can act on. Where there is no whole-route fare either the timeline
  stays empty; where there is no stop to name as a boarding point, there is no stage.
- **Why it is sound rather than a guess:** the identity of the two guards means *no stages implies no
  span*, which is exactly when the pill is showing `formatFare(service.fareFull)` and nothing else. The
  sheet re-reads the pill's own datum.
- **Consequences:**
  ...
  - 🟢 The corpus row was **renamed, not merely re-expected** —
    `…-opens-an-entirely-blank-fare-sheet` → `…-opens-with-the-fare-the-pill-showed`. The property it
    pins is unchanged; the answer is not. (ADR-122's rule: a row whose *name* is the property must keep
    testing that property.)
  - 🟠 `fareSectionalNote` — *"Fares are sectional — you pay less boarding further along the route"* — is
    still printed unconditionally above the timeline. Over a single fallback stage it is a general
    statement about HK fares rather than a claim about this route, so it is not false, but it is the one
    place a rider could read a sectional promise the data does not support. A conditional variant needs a
    catalogue key in three locales **and** a discriminant on the view, which there deliberately is not.
  - 🟡 `fareRange` and `fareStages` still spell the same rejection twice. Nothing depends on them staying
    in step except this fallback; one `isPriceable(fare)` predicate would make it structural.

## ADR-129 — Two things a conformance walker cannot see: colour and geometry

- **Status:** **Accepted** — **2026-08-11**, two test techniques in `apps/web/test/`.
- **Context.** `conformStates` reads **words**. ADR-098 already recorded that it cannot see an interaction
  *destination* and ADR-106 that it cannot see the scroll spy's geometry. Two more instances arrived
  together, and both were port-time regressions no gate could have caught.
- **Decision 1 — a text token is classified by contrast, and a renderer is measured against the
  classification.** Three Search strings had dropped from `--text-muted` to `--text-subtle` in the port.
  Measuring rather than trusting the report found the bigger fact: **`--text-subtle` fails AA for body
  text in five of six pairings** (3.90 / 3.55 / 3.12 : 1 dark, 4.76 / 4.55 / 4.34 light), not one.
  So: `search-contrast.test.ts` classifies **every** `--text*` token arithmetically from
  `@nextbus/ui`'s emitted `THEME_VARS` — with a control that fails if a new one is neither classified —
  and `search-states.test.tsx` asserts which token each of the three strings actually renders in. The two
  halves are worth little apart. **Token *values* stay a design decision in `docs/09`**, and the
  `--text-subtle` assertion is a deliberate **tripwire**: it goes red if the value is ever raised, so the
  rule gets retired rather than quietly outlived.
- **Decision 2 — a geometry claim is tested by modelling a layout from the screen's own exported
  expressions.** jsdom lays nothing out. Give the test three numbers (the chrome above the first kerb, a
  kerb's height, the viewport), stub `getBoundingClientRect` for the sections and the docked card,
  dispatch a real scroll, and read the answer off the real `MiniMap`'s lit dot. **Everything else is
  derived from the screen's own output** — the page height from what `tailRoom()` returns, the maximum
  scroll from that, the spy's line from where the card reports itself. The test never restates the
  formula. `SNAP_BASE` became `CARD_DOCKED_BOTTOM + MAP_GAP` so the constant a suite places the card at
  and the line the spy measures cannot drift apart.
- **Consequences:**
  - 🟢 The last-kerb defect `docs/07` carried turned out to have been **fixed in `ec7053c` already** —
    what was missing was evidence, because both existing assertions parsed the tail *expression* and
    neither would have caught a page that still could not scroll far enough.
  - 🟠 **Two token-value changes are now measured and owed to the owner:** `--text-subtle`'s dark value
    would need to rise from `107 114 128` to about `139 146 161` to be usable for body prose, and
    light-mode `--positive` / `--warning` need darkening for the 16 px arrival readouts. Both are design
    decisions and neither was made here.
  - 🟡 `ClearButton` is `text-subtle` where the RN screen uses `muted` — a fourth port-time drop, left
    alone because it is a non-text mark and passes 1.4.11's 3:1.
  - 🟡 The token classification is deliberately app-local for now. It belongs in `packages/ui`, where both
    renderers and any future one would inherit it — but the second half of the rule (which token each
    *string* uses) can only be asserted by rendering.

## ADR-130 — One declaration, two apps: the deep-link fallback, and an enforcement claim nobody evaluates

- **Status:** **Accepted** — **2026-08-11**, `scripts/pwa/redirects.mjs`.
- **Decision 1 — the host-side SPA fallback is declared once and emitted by both builds.** A shared
  `/route/…` link 404s on a **first** visit, because `navigateFallback` is a service-worker route and a
  first visit has no service worker. The first fix put a `_redirects` in `apps/web/public/` only —
  reintroducing, one layer down at the host, exactly the split ADR-082 removed from the Workbox policy so
  that two PWAs cannot disagree about what a rider sees with no network. **And the app that actually
  ships was the one left broken:** `expo export -p web` writes its dynamic routes as literal
  `dist/route/[id].html` and `dist/stop/[id].html`, so no host matches a real route id.
  The rule now lives beside `workbox.config.mjs` and takes its target from a newly exported
  `NAVIGATE_FALLBACK` there, so **the worker's fallback and the host's rewrite are one value** — a
  disagreeing pair would fail only on a first visit to a deep link, which is the case nobody tests
  locally. A **200 rewrite, not a redirect**: redirecting to `/` would open the app at Nearby and silently
  drop the place, route or query that was shared. `writeRedirects` refuses a `dist/` with no
  `index.html`, because a rewrite pointing at a file the exporter never produced is a 404 with extra steps.
- **Decision 2 — an enforcement that names a slot which cannot observe the state is worse than an honest
  gap.** Settings' `stale` `knownDefect` was cleared by ADR-125 and pointed at `by: 'languageOptions'` —
  a slot that is already `empty`'s `by`, and `conformStates` does not project a `by` state at all. So the
  published `knownDefect` count fell 5 → 4 on a claim **no harness evaluates**: delete the `storage`
  listener tomorrow and every conformance run on both renderers stays green. It is now `unenforced`,
  naming what does hold it — two kernel corpus groups and three suites. **The decisive argument was not
  the RN driver's mock** but that `apps/mobile/lib/preferences.ts` gates the merge and the listener on a
  web feature test: `stale` is a state the native surface **cannot enter by design**, and `conformStates`
  treats a projected state a renderer cannot reach as a finding — so a `shows` projection would have
  obliged the RN driver to fake a state its surface does not have.
- **Consequences:**
  - 🟢 Verified by running both builds end to end: `_redirects` present in both `dist/`, absent from both
    precache manifests, `sw.js` otherwise byte-identical.
  - 🟠 **WP0-5 must curl a deep link on the real origin once.** Cloudflare Pages and Netlify read
    `_redirects`; Workers static assets do too but with different precedence rules. And if the API is ever
    moved behind the app's origin, a rule must go **above** the catch-all or `/v1/…` starts being answered
    with the app shell — a JSON parse error in the client rather than a network failure it can report.
  - 🟡 Settings' `failed` is now that spec's only `knownDefect`, still owned by `docs/07`'s hardening list.

## ADR-131 — A React 19 ref callback that both registers and observes must return one cleanup

- **Status:** **Accepted** — **2026-08-11**, `apps/web/src/screens/PlaceDetail.tsx`.
- **Context.** Place detail's kerb-section ref callback did `return measureTail(el)` and nothing else.
  **React 19 calls a returned cleanup *instead of* calling the ref back with `null`** — so for the
  bottom-most kerb, the only one that gets a `ResizeObserver`, `registerSection(poleId, null)` never ran.
  Its `<section>` stayed in the registry after leaving the document, one entry per detach, and the scroll
  spy went on measuring it.
- **It is not inert, and the symptom is the surprising part.** A detached node reports a zero rect, which
  clears the spy's line at every offset, so the stale kerb wins the "last heading to clear the line"
  comparison and `activePole` becomes a pole on no pin of this place. `MiniMap` dims every pin the moment
  it has an `activeId` and emphasises only the one containing it — so **the map dims whole and lights
  nothing.** Driven for real: place A (3 kerbs) → place B (2 kerbs) leaves 3 entries and zero lit dots.
- **Decision.** One combined cleanup, returned unconditionally: `registerSection(poleId, null)` then
  `stopMeasuring?.()`. **The bug was the mixture of two idioms in one callback** — this file's
  `measureTail` is itself a cleanup-returning ref, while `RouteStopRow`'s `ref={(el) => registerRow(i, el)}`
  correctly relies on the null-call because it returns nothing. Either is fine; one callback doing both is
  not.
- **Why no suite could have caught it, which is the reusable half.** `measureTail` returns a cleanup only
  when `ResizeObserver` exists — and **jsdom has none**, so every Place-detail suite in this repo runs in
  the one environment where the bug is impossible. The new harness stubs `ResizeObserver`, which is what
  makes it a browser rather than a rebuttal, and it reads registry **membership** rather than a rendering,
  because the spy measures every member.
- **Consequences:**
  - 🟠 **How reachable it is, honestly:** only while the screen stays mounted, and nothing on the shipping
    app taps place → place. The live path is the persisted query cache plus the daily dataset rebuild
    handing a mounted screen an answer with one kerb fewer. Every other exit unmounts the screen. So on
    the common path it is a leak with no symptom — written into the test file rather than sold as a crash.
  - 🟠 **`apps/mobile/app/stop/[id].tsx` has the same class of defect and worse:** `recordSection` only
    ever adds or replaces, because an unmounted `<View>` fires no `onLayout` — so the RN screen keeps
    **every** kerb it has ever laid out. Not fixed; it retires at WP6-8.
  - 🟡 `activePole` is `null` until the rider's first scroll: the spy's effect runs at commit, before the
    query answers, and deliberately does not re-run when data arrives. A place a rider never scrolls
    shows no kerb emphasis at all. That wants a spec view on what *"a dot is always lit"* means.

## ADR-132 — The bus glyphs: two vehicles, and the three rules that had never been written down

- **Status:** **Accepted** — **2026-08-12.** `routeVehicle` in `packages/core/src/route-detail.ts` with a
  6-case corpus, `BusGlyph` rewritten in both renderers, and the rules recorded in
  [`docs/09`](./09-theme.md) §8 for the first time. Amends
  [ADR-030](#adr-030--route-view-as-a-vertical-schematic-line-strip-with-two-state-bus-tokens)'s glyph.
- **Context.** Now that every stop on a GMB route has times (ADR-116–121), the rail draws a bus token for
  minibus routes too — and `BusGlyph` drew one silhouette for every operator. The owner asked for a
  front-facing minibus and for the decker to be distinct enough to tell apart.
- **Decision 1 — which vehicle is the KERNEL's call.** `routeVehicle(operator)` names a `RouteVehicle`:
  `bus` or `minibus`. Each renderer maps that word to a drawing, exactly as `etaUrgency` names a band and
  each renderer owns its colour for it (ADR-053). **This is not stylistic**: deciding it in a view is
  precisely what two gates forbid — `check-no-derivation` bans selecting on data,
  `check-no-adhoc-id-parsing` bans reaching into an id for the operator. **Unknown operators degrade to
  `bus`** (ADR-051's shape-not-vocabulary rule): the day a fifth operator ships, a client compiled today
  must draw *something* rather than leave a blank rail.
  - ⚠️ **Two words, and one is an overstatement recorded rather than fixed.** NLB and MTR Bus run
    single-deckers, and the app draws a double-decker for *"a bus"*. A third word is not justifiable from
    the feed, and the decker has been the app's bus since Wave 1. The corpus row
    `nlb-is-called-a-bus-though-it-runs-single-deckers` is where that changes if it ever should.
- **Decision 2 — the drawings.** Shared body width (14), window width (8) and ground line; the difference
  is **height** (17.0 against 12.6) and **glass** (two 3.6 bands against one 4.4 pane), plus the minibus's
  filled roof sign box. A real light bus is narrower and equal width is a deliberate departure: the token
  is a fixed 24 px circle, and a narrower glyph inside it reads as a *smaller drawing* rather than a
  smaller vehicle. The decker's rhythm is **derived** — three equal gaps, `(17 − 2·band) / 3`.
- **Decision 3 — the three rules `docs/09` §8 never had.**
  1. **Radii are Lucide's two values**: body `rx=2`, inner details `rx=1`. Counted in the installed set —
     `rx="2"` 260 times, `rx="1"` 111 — so it is a two-value convention, not a formula. The shipping body
     was `2.5`, which was never a decision.
  2. **The tyre pill is path `1.6 × 2.6`, painted `3.6 × 4.6`.** Head-on you see a tyre's *tread*, so it
     must read taller than wide. The previous value painted `4.4 × 4.6` — essentially square and 31 % of
     the body's width, which is why it read as a foot.
  3. **Painted ≠ path.** Painted = path + 2 wherever a shape is stroked, which is everywhere here —
     including the *filled* tyres and sign box, which carry both a `fill` and the shared stroke.
- **What was drawn and rejected, and why each one is worth knowing:**
  - **Headlights**, as Lucide draws them (`M8 15h.01` — a zero-length path a round cap paints as a dot one
    stroke across). They **fit the minibus and not the decker**: a dot needs 2.00 of clear inner face, the
    minibus has 3.20 and the decker 1.27. Rejected on looks, and on a family argument — a detail on one
    vehicle only becomes a *distinguishing* mark, competing with the difference already doing that work.
  - **A partial-width bumper line** in the minibus's empty lower face. More visible at token size than the
    arithmetic predicted — and it makes the minibus read as a **two-band** vehicle, destroying exactly the
    one-pane-against-two-slots difference. **The empty face is the reason the pair works**, not a gap in it.
  - **A concentric radius rule** (inner = outer − padding), which the owner proposed. Correct arithmetic
    (`2 − 3 → 0`, square) and **unobservable**: the whole sweep from square to a full pill is identical at
    token size, because a round linejoin on a 2 px stroke rounds a square corner anyway. **Below about
    `rx=1` the stroke decides the look, not the path** — so the radius was settled on the rule, because
    nothing visible could settle it.
  - **Lucide's stroke wheels** (`M6 19v2`, painted 4 tall by its caps). More Lucide-consistent and reads
    spindly — thin legs rather than tyres. Confirms by looking what §8 had already argued.
  - **Dropping the stroke from the filled pill**, which looked like pure padding. Wrong twice: it shrinks
    **both** axes, and **the round join is what carries the bottom of the wheel** — `rx` cannot, being
    capped at half of a 2.6-high rect. *The stroke is the shape.*
- **Consequences:**
  - 🟢 Seven rounds of candidates are **deleted** from `apps/web/lab/`, leaving only the pre-ADR glyph for
    comparison. A lab full of rejected drawings rots — the next reader cannot tell a live option from a
    dead one — so the reasoning lives here and in §8, and the geometry lives in the shipped component.
  - 🟠 **Three design errors in this ADR's own process came from quoting a path value as a painted one**:
    "2.4 ships today" (the ink is 4.4), a fill-only variant labelled "same silhouette" that matched in one
    axis only and came out a horizontal pill, and a token-size difference computed without the stroke. §8
    now carries the distinction as a rule, and the habit that catches it is **rasterising and scanning the
    ink in both axes** rather than reading attributes.
  - 🟠 **A lab whose defaults drift is worse than no lab.** The proposed pill width was set as the component
    *default*, so every glyph on the page silently adopted it — including cells labelled "settled". Studies
    pass their values explicitly now.
  - 🟡 The minibus's roof sign overlaps the body's top stroke by ~1.8 units, so it reads as a pod fused to
    the roofline rather than a box on it. Left as-is; it is close to the "roof pod" variant that was liked.
  - 🟡 `apps/mobile` gets both glyphs even though it retires at WP6-8, because the twins are asserted
    against each other and a one-sided change would fail that.

## ADR-133 — A screen says once that it has stopped being fed, and never a fourth sentence

- **Status:** **Accepted** — **2026-08-12.** `feedNotice` in `packages/core/src/eta.ts` with a 9-case
  corpus, `lastUpdatedIso` on `RouteDetailView`, `FeedNotice` + `useOnline` in `apps/web`, three catalogue
  keys in three locales. Pays the debt [ADR-123](#adr-123--staleness-is-a-board-level-fact-so-a-per-reading-cue-is-the-wrong-unit)
  deliberately took, and amends [ADR-008](#adr-008--etas-are-approximations-no-client-side-fake-countdown)'s
  staleness requirement by moving where it is satisfied.
- **Context.** ADR-123 removed both per-reading staleness cues on the finding that staleness is a property of
  the **board**, not of each figure — leaving the app with **no staleness indication at all**, against a
  golden rule. The owner had already found the underlying gap with the local Worker down: every time on
  screen kept counting down and nothing said the data had stopped arriving.
- **Decision 1 — the kernel decides which state a screen is in.** `feedNotice` takes the newest board on
  screen, the clock, whether the platform has a network, how the last attempt failed, and the served
  `staleAfterMs`; it returns one of `none` / `lastUpdated` / `offline` / `unreachable` plus the clock time to
  say it with. **Not four `isError ? … : isPaused ? …` ladders in four screens** — that is the shape
  ADR-124 found wrong twice, and a rule with a corpus is what stops the fifth screen guessing differently.
- **Decision 2 — precedence, which is the whole of the logic.** `offline` → `unreachable` → `lastUpdated` →
  `none`. **Each earlier state explains the later ones**: a rider with no network does not also need telling
  their data is old, and neither sentence is improved by adding the other. One line, one message.
- **Decision 3 — there is deliberately no fourth sentence.** An **upstream board refusing** already has
  vocabulary: `StopCardView.incomplete`, `RouteStopRowView.incomplete` and `liveArrivals`
  (ADR-073/077/081/114). `feedNotice` is not given upstream failures and has no arm for them, because a
  second sentence **could disagree with the first**: a live round asks each pole separately, so one kerb can
  refuse while forty answer, and no screen-level line can say that without being wrong about most of the
  route. Route detail therefore draws *two* lines in the worst case, and that is correct — they answer
  different questions, and a rider acts on them differently.
- **Decision 4 — the time is absolute.** `at` is Hong Kong wall-clock from the existing corpus-pinned
  `formatClock`. ADR-008 prefers that to a fabricated relative age: *"2 minutes ago"* ages while nothing
  re-renders, which is the same dishonesty as a client-side countdown. `updatedAgo`, in the catalogue and
  uncalled since Wave 1, stays uncalled.
- **Why `navigator.onLine` is trusted here and nowhere else.** It is a claim about a *link*, not about
  reachability — `true` behind a captive portal, the ordinary MTR-station case. ADR-124 refused it as a
  reason not to fetch for exactly that reason. It is safe for choosing a **sentence** because the asymmetry
  runs the right way: `false` is reliable evidence of no network, `true` is merely the absence of evidence,
  and the `unreachable` arm covers the portal.
- **Consequences:**
  - 🟢 ADR-008 is satisfied again, at the grain the fact actually has.
  - 🟢 The notice is silent in the ordinary case. ADR-122's lesson: a cue that fires while everything works
    is one riders learn to ignore before the day it matters.
  - 🟢 **The wording is accepted as the default** (owner, 2026-08-12): *"Last updated {time}"*, *"You're
    offline — showing the last times we had"*, *"Can't reach NextBus — retrying"*. It joins the app's other
    error and placeholder texts in a `docs/07` row to be reviewed **as a set**, because that is the only way
    the register stays consistent — not one sentence at a time.
  - 🟠 **Wired on Route detail only.** Nearby, Place detail and Favourites each reach their readings through a
    different hook, so each needs its own `lastUpdatedIso` and its own `trouble`; the component and the rule
    are shared and ready. Route detail is first because it is where the owner hit it. `docs/07` carries the
    remainder — **until it lands, three screens still say nothing.**
  - 🟠 **A reading from yesterday reads as today.** `formatClock` gives `HH:MM` with no date, so a cache
    replayed after midnight says *"Last updated 23:58"* with no hint it is a day stale. Reachable only
    through the persisted query cache; the honest fix needs a date-aware, locale-aware format.
  - 🟡 `trouble` is derived from `query.isError` on the screen, which collapses *"the Worker said no"* and
    *"the fetch never arrived"* into one sentence. ADR-124 established that the two are distinguishable — an
    `EdgeRequestError` carries the Worker's own `code`/`retryable`, a bare `TypeError` does not — so a fourth
    arm is available when it earns its own sentence.

## ADR-134 — The design-system gallery is enumerated from the published specs, not hand-listed

- **Status:** **Accepted** — **2026-08-12.** `apps/web/lab/Gallery.tsx` at `/lab/#gallery`, its input in
  `lab/specIndex.ts`, gated by `apps/web/test/gallery-covers-specs.test.ts`. Extends
  [ADR-112](#adr-112--a-dev-page-lives-in-the-app-and-a-gate-keeps-it-out-of-the-app)'s dev page.
- **Context.** The owner asked for *"a listing of our design system components … as that will make it easier
  to build a corresponding page for our eventual native apps"*, and extended it to *"some of the motions to
  help understand the underlying principles about how it's designed and translate that to the appropriate
  styles for the operating system it's on."*
- **Decision 1 — the page is built from `packages/contract/ui/*.spec.json`.** The same ten files a native
  repo vendors. A hand-maintained gallery drifts, and **a drifted gallery is worse than none** — a porter
  would check against it confidently and be wrong. Each component's slots and states are printed from the
  spec itself, including **what enforces each state**, which is the field a porter must read before copying
  anything: `projected` is checked by the walker, `unenforced` is honest prose, and `knownDefect` is a target
  no renderer meets yet and must never be copied as behaviour.
- **Decision 2 — the list of specs is explicit, and the gate is bidirectional.** `import.meta.glob` was the
  obvious choice and is the wrong one: it would pick a new spec up automatically and **assert nothing about
  it**, which is the exact failure the gate exists to prevent. So `specIndex.ts` names all ten, and the test
  fails if a published spec is missing *or* if an entry names a spec that no longer exists.
- **Decision 3 — it does not render every component from a fixture.** Ten components take ten different sets
  of props, so a generic renderer is either a pile of adapters or a lie — and the **conformance suites already
  render each one against its corpus**, which is a stronger check than a picture. The spec is the listing; the
  screen itself is one click away.
- **Decision 4 — motions lead with the occasion, not the keyframe.** A keyframe name and a duration port to
  nothing: Compose and SwiftUI have neither. What ports is *what moves, on what occasion, and how fast*. So
  each row names the occasion in words first — *"a bus token exists on the rail; idle, continuous, never
  triggered"*, *"an arrival figure changes value; never on mount"* — and the principle second.
- **Consequences:**
  - 🟢 **It found a live drift on its first run.** `stop-row`, `place-row` and `favourites` still declared the
    `opacity.etaStale` fade in their `stale` state, three days after ADR-123 removed it from every renderer —
    a spec asking for something no code does, invisible because the state was `unenforced` and so nothing
    read it. All three now describe the screen-level sentence and say which screens are still unwired. **A
    gallery that renders prose is a gate on prose**, which no other check here is.
  - 🟠 It lists the components; it does not review them. The **error and placeholder texts** the owner wants
    to look at later — `feedNotice`'s three sentences, `etaStaleMark`'s retired label, `etasUnavailable`,
    the search and FAQ prose — are not surfaced as their own section yet. `docs/07` carries that.
  - 🟡 A component's **slots are printed, not drawn.** For the leaf components (`StopRow`, `PlaceRow`) a live
    sample from a corpus case would be cheap and is the obvious next step; for whole screens it is not.

## ADR-135 — The live path hardened against the network's own failure modes (WP6-8b)

- **Status:** **Accepted** — **2026-08-12**, across `packages/data-normalize` (the timeout),
  `apps/edge/src/eta-hub.ts` (the deferred snapshot, the slimmed attachment), `packages/api-client`
  (liveness, the fallback, the round watermark, the listener-door gate) and `packages/core`
  (`LIVE_ROUTE_RETRY_MS`). **Amends ADR-121** (decision 3: "there is no socket→poll fallback" — there is
  now, supervised) — ADR-076's "no `auto` spelling" is untouched and still asserted by a test.
- **Context.** The owner asked for a reliability review of the socket system, the CTB/GMB per-pole
  fan-out and the coalescing. The review found nine defects, all of one family: **the code assumed the
  network behaves** — fetches resolve, sockets that die say so, rounds finish before the next one starts,
  upstream errors are small, and a WebSocket, once the default, connects. Each assumption fails in an
  ordinary way (a wedged connection, a phone changing networks, a slow cell link, a Zod dump, an office
  proxy), and each failure was invisible in tests because jsdom/workerd networks behave. Response times
  were measured before choosing the timeout (2026-08-12 23:10 HKT, off-peak, 8 samples per feed:
  154–621 ms worst; all three feeds are CDN-fronted, so latency tracks cache state rather than passenger
  volume and rush hour grows payloads, not waits).
- **Decisions:**
  1. **Every upstream ETA call carries `AbortSignal.timeout(10 s)`** — `fetchUpstream` in
     `packages/data-normalize/src/normalize.ts`, one helper so the next adapter cannot forget it. Ten
     seconds is a hang detector, ~16–60× the measured worst case, not a latency budget. This finally makes
     `upstream_timeout` reachable: `codeFor` has mapped `TimeoutError` since ADR-064 and nothing ever
     produced one, while a single wedged connection held an `EtaHub` round — and the alarm its `finally`
     reschedules — for as long as the runtime took to notice, occupied one of the six connection slots
     every cap's arithmetic is written in, and parked its hung promise in the coalescer for a whole TTL.
  2. **A cold target's connect snapshot is deferred to the first round** (`Session.snapshotPending`).
     `subscribe()` used to answer every subscription immediately from storage; for a never-polled target
     that was `snapshot { etas: [] }` — the frame for *"no buses due"* — and it blanked the arrivals the
     screen had just painted from its own HTTP fetch until the round landed. The ADR-073/087 blanking
     shape, from the connect path, and a real divergence from the poll emulator (whose first snapshot has
     waited for a round that answered since WP5-4). Sent immediately only when every accepted target has
     a stored row (even an empty one — "polled, nothing due" is knowledge); the `live` transition defers
     with it, so data-first ordering holds by postponement. A round that told the pending session nothing
     sends nothing, exactly as the emulator's "a failed first round is not an empty world". Client side,
     `LiveEtaUpdate` now carries `seq` and `EdgeClient`'s listener door drops updates at `seq === 0`, so
     a pre-data `status` transition can never push the placeholder empty list into a query cache.
  3. **A client socket now checks liveness instead of assuming it.** The keepalive pinged and never
     looked at what came back: a phone that changes networks (or a NAT entry that dies without an RST)
     leaves the platform `WebSocket` OPEN for minutes, `send()` buffering silently, the screen labelled
     live over readings that stopped moving. Two quiet keepalive intervals (`KEEPALIVE_MISSED_LIMIT`,
     ~60–90 s) now tear the connection down and reconnect on the ordinary schedule — the auto-response
     pong is the guaranteed once-per-interval inbound frame, so a healthy connection can never trip it.
     A handshake that has not completed within one interval fails the same way, because a blackholed
     upgrade (a swallowing middlebox) otherwise sits CONNECTING for the platform's own long timeout,
     delivering nothing and retrying nothing.
  4. **A socket subscription that never delivers falls back to the poll engine, supervised**
     (`SOCKET_FALLBACK_AFTER_FAILURES = 3`, in `EdgeClient.watch`/`watchRoute`). This is the amendment.
     ADR-121 decision 3 stated the cost plainly: a rider behind a WebSocket-hostile proxy "gets *nothing*
     where before they got slow-but-working data" — and a browser exposes neither status nor body of a
     refused upgrade, so nothing can even say why. What ADR-076/121 actually protected — a *transport*
     quietly becoming another transport, making "which engine is driving" unanswerable — is preserved:
     the swap is a whole fresh subscription on a fresh poll transport, with a stated trigger, one level
     above the transports. Three consecutive `retrying` updates with no data frame ever received
     (`seq === 0`) is the trigger — ~10 s at the reconnect schedule, inside one poll cadence. A socket
     that has ever delivered a frame has proved the path and never falls back; a terminal
     `closed`/`retryable: false` never counts, because the server answered. `watchRoute`'s fallback is
  ...
  5. **A poll round that completes after a younger round is discarded whole** (a per-subscription
     ...
     finishing last used to fold its older data into the state and publish it as a fresh delta, stepping
     every time on screen backwards for a cadence.
  6. **No thrown message reaches the wire unbounded, and the socket attachment stores no messages at
     all.** `wireErrorOf` caps at 200 chars (`boundedMessage`): a `ZodError.message` is a serialized
     issue list that runs to kilobytes when an upstream changes shape, and it travelled verbatim into
     every `status` frame, every `failed` entry, and the session attachment — whose hard platform cap is
     16,384 bytes for a session that also carries up to 64 route-watch targets. The attachment now stores
     `{stopId, code, retryable}` only (`compactFailures`; `sameFailures` never compared messages), and
     `round()` guards `sendRound` per socket so one connection's bookkeeping throw cannot cost the other
     subscribers their frames or the round its storage writes.
  7. **`LIVE_ROUTE_RETRY_MS` is derived, 33 s, because the edge's own coalescer is the binding floor.**
     The 15 s blind retry reasoned only about the upstream CDN's 45 s entry and forgot our own layer:
     every board a route round reads goes through the coalescer (TTL = `refreshAfterMs` = 30 s), so a
     retry inside that window re-reads our own cache *by construction* — a billed Durable Object wake
     spent learning nothing, every time the arm fired. Now `refreshAfterMs + LIVE_ROUTE_PUBLISH_MARGIN_MS`,
     so the coupling is visible at the definition (the same argument `ETA_TTL_SEC` makes about the same
     number). Also recorded: **`nextRouteRoundMs`'s `cacheAgeSec` arm has no production caller** — the
     adapters never surface the `age` header — so every not-advanced round takes the blind arm. The
     arithmetic stays corpus-pinned; the wiring is a `docs/07` item, not silently claimed.
  8. **The coalescer's scope is written down: one isolate, and a Durable Object is its own.** Comments in
     `eta-hub.ts` implied the shard's rounds share fetches with the HTTP path; they do not — the map is
     module state, and what shares it is what shares a heap. Within one round the dedup is real (shared
     poles cost one call) and reconnect storms are still absorbed, but socket + HTTP for one stop is two
     fetches, and each of the eight shards and every route watch keeps its own window. No cap moves — the
     shard's cost model already priced its rounds standalone — but a future argument must not assume
     cross-context dedup. Genuine sharing would be a Cache API / short-TTL-KV design, deliberately not
     built pre-deploy.
- **Consequences:**
  - 🟢 `pnpm test` green across the workspace; the new behaviours each carry their own suite
    (`upstream-timeout`, `poll-overlap`, `live-fallback`, three liveness cases in `live-socket`), and the
    eta-hub suite now asserts the deferral instead of the empty connect snapshot it used to pin.
  - 🟢 The failure a rider was most likely to meet — first paint on a hostile network — now converges on
    working data three ways: the fallback (no socket), the liveness check (dead socket), the deferral (no
    blanking while either happens).
  - 🟠 The two engines' `seq` numbering now differs by one on a cold connect (the hub consumes a seq at
    subscribe that the emulator does not). `applyLiveFrame` applies a snapshot at any `seq` by design, and
    no suite asserts cross-engine seq equality — recorded so nobody discovers it as a surprise.
  - 🟠 A subscription's *first* snapshot now waits for a round that answers, so a rider on a target whose
    every board is refusing sees `connecting` + `retrying` (with their HTTP-painted times intact) rather
    than an instant empty snapshot — the honest direction, but a visible behaviour change.
  - 🟡 The route watch still cannot use the `ttl − age` retry arm (decision 7); wiring the `age` header
    through an adapter return shape is backlogged in `docs/07`.

## ADR-136 — The batch endpoint learns the route question: `/v1/etas?route=`

- **Status:** **Accepted** — **2026-08-13** in `packages/contract/src/wire/responses.ts` (the parameter),
  `apps/edge/src/stop-route.ts` (`routeEtasBatch`), `apps/edge/src/index.ts` (the branch), and
  `packages/api-client` (`getEtasRoute`, the poll engine's route mode). Additive-optional, so
  `CONTRACT_VERSION` stays 2.0.0 by that constant's own rule. Closes the `docs/07` row ADR-121 demoted.
- **Context.** `/v1/etas?ids=…` carries no per-id route list — `,` is a legal `idchar`, so a nested list
  has no safe delimiter — which meant a *route* round through it asked every pole about **every** route
  calling there. That is the ~19× ADR-121 measured and the reason the socket became the default; the poll
  path kept the cost, tolerable while it was opt-in. ADR-135 changed who stands on it: the supervised
  fallback now routes any rider whose network cannot carry WebSockets onto exactly this path, on the
  operators (Citybus, GMB) whose routes have no bulk feed. Measured 2026-08-13 against the live feed: one
  un-narrowed chunk of 12 of Citybus 182's poles took **10.0 s** (bounded there by ADR-135's new upstream
  timeout — four boards tripped it; ADR-121 measured the same chunk at 19.9 s before timeouts existed),
  where the identical poles narrowed to route 182 took **0.25 s**.
- **Decisions:**
  1. **The route is the parameter, not a per-id list.** `?route=<canonical route id>`, mutually exclusive
     with `ids` (both or neither is a 400). The server resolves the poles from the same route document
     `/v1/route/{id}` serves and answers one `EtaBatchEntry` per pole in the route's own stop order,
     deduplicated — the same three-step resolution `/v1/live?route=` performs (parse → 400, look up → 404,
     stops are the set), so the polled route watch and the socket one cannot disagree about which kerbs
     exist. A per-id `routes=` grammar (the delimiter dance the `docs/07` row costed) would have been more
     general and served no caller: the only narrowed multi-id question anything asks is "one route at its
     own poles".
  2. **The narrowing goes in, not just out.** `routeEtasBatch` threads `[routeId]` through `stopEtasBatch`
     into `stopEtas`, so `boardsFor` bounds the upstream questions exactly as `EtaHub`'s route watch does
     (ADR-117). That closes all three of the `docs/07` row's consequences at once, because they were one
     cause: the fan-out is per-pole; a sibling route's board failure cannot mark this route's kerb
     (`failed` is computed from calls that are now only this route's); and `LIVE_CTB_BUDGET` counts only
     the watched route's own (pole, route) pairs, so it can no longer silently drop the watched route at a
     busy interchange.
  3. **The poll engine uses it whenever the subscription is a route.** `PollTransportDeps` gains `route`
     and `getEtasRoute`; a round becomes one request with no chunking. The targets still come from the
     client's own route-document read (`watchRoutePolled`) — the per-target bookkeeping (retention, drops,
     failure ordering) is written in targets, and the batch answers per pole, so the two meet by id.
     Client-side `narrowEtasToRoutes` still runs per target; against the now-narrowed producer it is
     idempotent, and it is what keeps a scripted un-narrowed test producer from leaking another line's
     times — the same belt the socket path wears.
  4. **Not capped, and cached like its sibling.** The pole set is our own dataset's, the same list
     `/v1/route/{id}` already serves uncapped, and a request runs once — the DO's `LIVE_ROUTE_MAX_POLES`
     bounds a *recurring* alarm, which is a different argument. The response is edge-cached at
     `ETA_TTL_SEC` on a key rebuilt from the one deciding parameter, mirroring the `ids` branch.
- **Consequences:**
  - 🟢 A poll-emulated route watch — including every ADR-135 fallback rider — went from ~3 chunks ×
    10–20 s and ~130 upstream calls each to **one request, one narrowed call per pole** (~31 for Citybus
    182), verified against the live feed at ~0.5 s.
  - 🟢 `ids` in the OpenAPI document is `required: false` now, with the exclusivity stated in prose —
    an OpenAPI parameter table cannot express "exactly one of", recorded rather than pretended otherwise.
  - 🟡 The socket's `?route=` and this parameter resolve poles identically but share no code path with
    each other beyond `stopEtas` — a divergence in resolution (a cap, an ordering) would surface as the
    two engines watching different kerbs. `etas-batch.test.ts` pins this endpoint's set to the route
    document; the DO's own suite pins the socket's.

## ADR-137 — Every edge request carries a deadline: a hung fetch is a failure, on time

- **Status:** **Accepted** — **2026-08-13** in `packages/api-client` (`REQUEST_DEADLINE_MS`, `getJson`).
- **Context.** A reliability audit found the one hole left in the ADR-135 family: the client layer's own
  fetches had no deadline. Every failure arm in the package — the poll round's `requestError`, a screen's
  query retry, the supervisor's fallback — needs a *rejection* to fire, and a blackholed connection (a
  network switch mid-ride, a NAT entry that expired without an RST) never produces one. On the poll
  engine — which is what the ADR-135 fallback puts a rider on precisely *because* their network is
  hostile — a hung `/v1/etas` request meant rounds stacking behind it against the browser's per-origin
  connection pool, no `retrying` status ever emitted, and a screen labelled live over ageing readings:
  the silent-dead-pipe defect ADR-135 closed for the socket (connect watch, keepalive), alive on the
  engine that exists for exactly that network.
- **Decisions:**
  1. **`getJson` wraps every request in an `AbortController` whose timer spans headers *and* body** — a
     server that answers headers and then wedges the stream is the same dead pipe. The abort is rethrown
     as an `Error` naming the path and the wait ("`/v1/etas → no response in 15000 ms`"), because the
     platform's own rejection ("The operation was aborted") names neither, and `wireErrorOf` carries the
     message onto a `status` frame. Classification is unchanged: `internal`, `retryable: true` — the
     ADR-064 fallback for "the fetch never reached a server", deliberately not a sixth code.
  2. **15 s is a derivation, not a guess.** The Worker's own upstream deadline is 10 s (ADR-135
     decision 1), so the slowest *truthful* answer clears the edge inside ~12 s; 15 adds network slack
     and still ends a hung request before the next 30 s poll round starts (`refreshAfterMs`), so a dead
     connection costs exactly one in-flight request at a time, never a stack.
  3. **A manual `AbortController` + timer, not `AbortSignal.timeout()`.** This package runs on Hermes and
     in older WebViews where the static is not dependable, and the timer has to be cleared on settle and
     out-live the header response anyway — the composed form writes itself.
- **Consequences:**
...
- **Status:** **Accepted** — **2026-08-13** in `packages/data-normalize/src/dataset.ts`
  (`DATASET_TIMEOUT_MS`, `fetchConsolidatedIndex`).
- **Context.** ADR-135 gave every upstream *ETA* call a 10 s deadline through `fetchUpstream`; the
  8.3 MB consolidated-dataset fetch stayed a bare `fetch()`, and it is the one call where a hang is
  not merely slow but **permanent**: `getInlineIndex` in `apps/edge/src/dataset.ts` memoizes the
  pending promise per isolate and clears the memo only in `.catch` — a rejection self-heals (that was
  the deliberate "don't cache a rejected build" fix), but a fetch that never settles is cached for the
  isolate's lifetime. The reachable shape: a KV outage puts the fleet on the inline fallback — the
  designated degrade-to-slow path — and `data.hkbus.app` accepts the connection but never sends
  headers; every request in the isolate then awaits one wedged promise forever, and the isolate never
  recovers without being recycled.
- **Decisions:**
  1. **`fetchConsolidatedIndex` passes `AbortSignal.timeout(DATASET_TIMEOUT_MS)`** — 60 s, not
     `UPSTREAM_TIMEOUT_MS`, because 10 s is a hang detector sized for a ~1 kB ETA board and this body
     is 8.3 MB. Sixty is still a hang detector rather than a latency budget: the Worker pulls the file
     from a CDN in single-digit seconds, and the slow consumer — `pnpm dataset:build` on a home
     connection — clears it with an order of magnitude to spare. The signal spans the body read, so
     headers-then-wedge is caught too, and the existing memo-clearing `.catch` turns the abort into
     the retry-next-request behaviour it was already written for.
  2. **The deadline is asserted where it matters**: the inline-fallback case in
     `apps/edge/test/dataset-kv.test.ts` now checks the dataset fetch carried an `AbortSignal` —
     inside workerd, which doubles as proof that `AbortSignal.timeout` exists on that runtime path.
- **Consequences:**
  - 🟡 `fetchUpstream` and this call are now two spellings of "a fetch with a deadline". Left as two
    on purpose: folding a parameter into `fetchUpstream` would invite the next adapter to pick its own
    number, and the whole point of that helper is that it has no knob.

## ADR-139 — The tile proxy joins `fetchUpstream`: the Worker's last bare fetch gets the deadline

- **Status:** **Accepted** — **2026-08-13** in `apps/edge/src/tiles.ts`.
- **Context.** ADR-135 put a 10 s deadline on every upstream *ETA* call and ADR-138 on the dataset
  download; `fetchTile` was the one upstream call left with none. A wedged LandsD connection held one
  of the runtime's six simultaneous outgoing connections — the number every fan-out cap's arithmetic
  is written in terms of, as `normalize.ts`'s own docblock argues — for as long as the platform took
  to notice, while the rider's tile request hung with it. Separately, the non-ok branch built an
  error envelope and dropped `res.body` unread, leaving the upstream stream half-open.
- **Decisions:**
  1. **`fetchTile` calls `fetchUpstream`** rather than growing its own `signal:` — the helper exists
     so the deadline cannot be forgotten, and a tile is a ~10 kB PNG for which the 10 s hang detector
     is generously sized. No new classification: the route handler's `errorResponse` already maps the
     abort (`codeFor`: `TimeoutError` → `upstream_timeout`), and LandsD's own 504 was mapped before.
  2. **The non-ok branch cancels `res.body`** before answering with the envelope, releasing the
...
- **Consequences:**
  - 🟡 The 10 s signal now spans the *success* path's body too, and `fetchTile` streams `res.body` at
    ...
    measures it. **Closed by ADR-148**: the timer is disarmed once headers arrive.

## ADR-140 — A round that answered nothing records nothing: the fabricated "polled, empty" row

- **Status:** **Accepted** — **2026-08-13** in `apps/edge/src/eta-hub.ts` (`round()`'s retryable-failure
  arm).
- **Context.** ADR-135 deferred a cold target's connect snapshot behind `snapshotPending` so that "we
  have not asked yet" could never be presented as "no buses due". The same round left one door open: a
  *retryable* target failure carried the previous readings forward with `before.get(id) ?? []`, and for
  a target the shard had **never polled** the `?? []` fabricated an empty list, which `writeReadings`
  stored as a real row. The row says "polled, nothing due" for a round that answered nothing — and
  `subscribe()`'s `stored.has()` check believes it. Concretely: rider 1 subscribes to a cold place
  during a total upstream outage and is correctly deferred; the failed round writes `P → []`; rider 2
  subscribes mid-outage, finds the row, and is answered an immediate `snapshot { etas: [] }` with no
  `failed` (a fresh socket carries none) — the ADR-073/087 blanking shape, from the connect path, for
  exactly the second rider. The asymmetry between the two riders is what shows the row is a false
  record.
- **Decisions:**
  1. **Carry forward only what exists.** The retryable arm now writes `after` only when `before` has a
     row: a fabricated empty list and a missing row are indistinguishable to every frame (`readingsFor`
     flattens both to the same empty list; `writeReadings` deletes only rows `before` holds), so the
     only observable change is the one that was wrong — what the *next* `subscribe` believes the shard
     knows.
  2. Pinned end-to-end in `eta-hub.test.ts`: two riders subscribing to a never-polled place mid-outage
     are **both** deferred, the readings table stays empty, and the round after recovery delivers both
     deferred snapshots carrying real readings.
- **Consequences:**
...
- **Status:** **Accepted** — **2026-08-13** in `packages/api-client/src/index.ts` (`watchRoutePolled`).
- **Context.** The poll-engine route watch resolves the route's poles with one `getRoute` per
  subscription and — deliberately — swallows its rejection, because an unhandled rejection from a
  subscription nobody awaited would take the screen down. The swallowed arm's defence ("the screen is
  already retrying the same fetch") covers the route *document*; the live subscription is keyed on a
  route id that never changes, so nothing upstream ever re-subscribes it, and one rejection left a husk
  that delivered no frame — no snapshot, no status, no retry — for the life of the screen mount. The
  shape is reached offline as a matter of course: with no network, socket attempts fail fast, so the
  WP6-8b fallback lands on this path *during* the outage it exists for, and the one-shot resolve then
  fails on the same dead network the sockets did. Connectivity returning 20 s later revived the socket
  engine's subscriptions and never this one.
- **Decisions:**
  1. **A retryable resolution failure re-runs `resolvePoles` after `pollMs`.** The poll cadence is the
     retry cadence because this path *is* the poll engine — resolution is its round zero; a second
     backoff schedule here would be a third spelling of "how often the poll path asks".
  2. **`retryable: false` stops asking** (`wireErrorOf` reads the server's own instruction, ADR-064):
     a route id that no longer denotes anything is pruned, not polled on the rider's battery for ever.
  3. **The pending retry timer dies with the subscription** — `unsubscribe()` clears it, pinned by a
     test alongside the recovery and stop-asking cases in `edge-client-watch.test.ts`.
- **Consequences:**
  - 🟡 The still-swallowed rejection means a route screen on the poll fallback shows its own query's
    error state with no live-status explanation while retries run — unchanged from before, and the
    door (`etaListenerDoor`) drops pre-snapshot status anyway. The fix is liveness, not vocabulary.

## ADR-142 — An arrival we cannot read is departed, not "NaN min"

- **Status:** **Accepted** — **2026-08-13** in `packages/core/src/eta.ts` (`etaView`), with guard rows
  in `spec/eta.spec.json` across `etaView` / `formatRelative` / `etaLabelParts` / `etaUrgency`.
- **Context.** The wire deliberately does not validate (ADR-052 decision 2): `Eta.arrivals` admits any
  string, and `cleanArrivals` drops only falsy values. A malformed operator timestamp therefore reached
  `etaView`, where `new Date(bad).getTime()` is NaN — every comparison below it is false, so neither
  honesty flag fired, and `formatRelative` printed a literal **"NaN min"** while `etaUrgency` called it
  `normal`. Every readout in the product renders through these functions, so one bad timestamp from
  KMB or Citybus put the badge on stop cards, favourites and route rows alike. The sibling rules
  (`formatClock`, `newestBoard`, `soonestArrivalMs`, `nextRouteRoundMs`) all guard unparseable input
  already — this was the omission, not a decision, and no corpus row pinned it.
- **Decisions:**
  1. **`etaView` treats an unparseable arrival as departed** — the one honest arm every consumer
     already has: `formatRelative`'s dash, `etaLabelParts`' `departed`, `etaUrgency`'s `none`, and
     `upcoming`'s filter, which now also drops the unreadable reading from route rows. The returned
     figures are a defined `0`/`0` sentinel rather than NaN because the corpus is JSON (which cannot
     hold NaN) and no caller reads the figures once `hasDeparted` is true.
  2. **Four corpus guard rows** pin the rule and its three rendered halves, so the Swift and Kotlin
     ports inherit the guard the day they run the corpus rather than rediscovering the badge.
- **Consequences:**
  - 🟡 "Departed" and "unreadable" are now the same rendered state (the dash). Distinguishing them
    would need a new `EtaLabelParts` arm across two renderers and two future ports for a state no
    operator has been observed to produce; the dash is honest either way — no figure exists.

## ADR-143 — A corrupt preferences blob is quarantined, not a splash screen held for ever

- **Status:** **Accepted** — **2026-08-13** in `apps/mobile/lib/preferences.ts` (`mergingStorage.getItem`,
  `PREFERENCES_QUARANTINE_KEY`).
- **Context.** The mobile storage adapter was deliberately unguarded — its own comment called the
  asymmetry with the DOM twin "a safety property rather than an oversight": a blob that would not parse
  rejected in `getItem`, `persist` abandoned hydration, `hydrated` never flipped, and `_layout.tsx`
  held the splash so the rider could not write over data that was still on the device. The intent was
  right; the failure mode was unbounded and invisible. Nothing ever repaired the blob (the app never
  reaches a write), so one truncated flush on a full device — or, on the shared web origin, any other
  script putting garbage under `nextbus.preferences` — was a **permanent** blank screen on every
  launch. The only exit was clearing app data, which deletes the very bytes the hold was protecting:
  the safety mechanism converged on the destructive outcome it existed to prevent.
- **Decisions:**
  1. **Quarantine, then defaults.** An unparseable blob is copied verbatim to
     `nextbus.preferences.quarantine` *before* `getItem` returns `null`, so the store's first write —
     which overwrites the corrupt main key — destroys nothing, and the app opens. The key is written
     only by this catch arm and read by nothing; it is a support/recovery artefact.
  2. **The splash-hold keeps its narrow job.** A quarantine write that itself fails rejects as before
     (the bytes genuinely could not be preserved), and a *read* rejection — the storage layer failing —
     still rejects too: there are no bytes to quarantine and starting on defaults there would let the
     first write clobber a blob that may be perfectly healthy.
  3. Pinned through the **real adapter** in `preferences.quarantine.test.ts` — the migration suite swaps
     the storage out, which is precisely why it could never see a defect that lived in the adapter.
- **Consequences:**
  - 🟡 The rider's list does not come back by itself: recovery from the quarantine key is a future
    support path, not a shipped behaviour. What changed is that the data survives and the app opens.
  - 🟡 `apps/web` repairs in place without quarantining first, so its first write after a corrupt read
    still overwrites the bytes. Same-shaped gap, smaller blast radius (the app was never bricked);
    worth folding into the same key if it ever bites.

## ADR-144 — "Soonest first" means the readings: a departed or empty board is a dash, and dashes go last

- **Status:** **Accepted** — **2026-08-13** in `packages/core/src/favourites.ts` (`readoutRank`,
  `compareDue`), with a guard row in `spec/favourites.spec.json`.
- **Context.** The Favourites sort documents its own rule — *"soonest first, and the ones with no
  reading last"* — and then contradicted it: `readoutRank` named `headway` (1) and `none` (2) and let
  everything else fall through to 0, which put `departed` in the same tier as a live arrival. A saved
  route whose board answered with no arrivals — common on KMB, where `eta: null` rows clean to an empty
  list — rendered the departed dash **at the very top of the card**, above a bus due in five minutes,
  because its absent `due` compared before every ISO string. The tiebreak was also a `localeCompare`,
  which this package bans (the host's ICU decides its answer — see `live.ts`'s comparator note); this
  was the last live use.
- **Decisions:**
  1. **Rank 0 is exactly the kinds that carry a figure or "Due"** (`mins`, `due`); `headway` stays 1;
     `departed` joins `none` at 2 — both render the dash, and there is no bus to catch either way.
  2. **The tiebreak is code-point order** (`compareDue`), correct because arrivals are ISO-8601 with a
     fixed offset, so lexical order is chronological — the corpus's own convention 3.
  3. The guard row saves the empty board *first* so the expectation only holds if the sort demotes it,
     and carries a third, later reading so both comparison arms are exercised.
- **Consequences:**
  - 🟡 A departed board with a published headway still shows the dash rather than the timetable
    (`favouriteRow` prefers any `eta` readout over the headway arm). Defensible — the board *answered* —
    but it is a separate judgement from this fix and is recorded here rather than smuggled into it.

## ADR-145 — A writer that has never persisted cannot have deleted anything: the null-base list ancestor is empty

- **Status:** **Accepted** — **2026-08-13** in `packages/core/src/favourites.ts` (`mergePreferences`),
  with a guard row in `spec/favourites.spec.json`.
- **Context.** `mergePreferences` stood in `mine` for a null `base` ("this writer has never persisted,
  so its values are defaults rather than choices"), and its docblock claimed "nothing is subtracted
  from the union". Both statements were true only while `mine`'s lists were still default-empty — the
  one shape the corpus pinned. With `base = null`, `mine = [K]` (a star made before this writer's first
  write), `theirs = [X]`: K is in the ancestor and not in `theirs`, which `mergeSavedKeys` correctly
  reads as *the other writer deleted K* — so the star is erased at the moment of its own first write,
  in memory and on disk. Reachability today is one caller-sequencing slip away (`base` recurs as null
  after `removeItem`, and the web store seeds the ancestor at load) — exactly the class ADR-125 records
  as having bitten twice.
- **Decisions:**
  1. **The list arms take an empty ancestor when `base` is null.** No write has happened, so nothing
     can have been removed since it: the merge over the lists becomes a pure union (theirs-first, for
     convergence), and the docblock's claim becomes arithmetic rather than aspiration. This covers the
     recents too — a session's history bumped before the first write is a local change, not something
     the disk can veto.
  2. **Scalars keep `mine` as the null-base stand-in.** There is no neutral scalar value, so a fresh
     writer's scalars still fall to `theirs` — the documented approximation, unchanged.
  3. The guard row pins the exact erasure shape: null `base`, a starred `mine`, a foreign blob — the
     sibling row (`a-writer-that-loaded-before-the-blob-existed-adopts-it`) keeps pinning the benign
     default-empty half.

## ADR-146 — Three small honesty repairs at the edge: the live door's 500, the frame cap's units, the KMB bucket key

- **Status:** **Accepted** — **2026-08-13** in `apps/edge/src/live.ts`, `apps/edge/src/eta-hub.ts` and
  `packages/data-normalize/src/kmb.ts`. Three fixes, one ADR: each is a declared rule whose
  implementation measured something adjacent to what the rule names.
- **Decisions:**
  1. **`/v1/live?route=`'s dataset read is classified where it fails.** It was the one dataset I/O in
     the Worker outside a classifying try, so a transient KV error rode the top-level catch out as 500
     `internal` — "*our* bug", per `index.ts`'s own vocabulary — where every HTTP sibling answers 502.
     Wrapped, `codeFor` applies; pinned in `dataset-kv.test.ts` by corrupting the stored route document
     (`kv.get(type:'json')` throws deterministically).
  2. **The client-frame cap counts bytes, as documented.** `LIVE_MAX_CLIENT_FRAME_BYTES` exists to
     bound parse CPU before `JSON.parse`, but the check read `message.length` — UTF-16 code units —
     so a frame of ~8,000 CJK characters passed at ~24 kB, 3× the stated bound. Units first (free, and
     it bounds the encode), then an exact `TextEncoder` count. Pinned with a frame inside the unit cap
     and outside the byte cap.
  3. **The KMB bucket key is canonicalised exactly as the id it mints.** `service_type` is
     `number | string` (the dataset mixes both) and `dir` has long and short spellings; keyed raw, a
     response mixing `1` and `"1"` split one board into two buckets minting the same `routeId`, and
     `dedupeEtas` then dropped one bucket's arrivals from the rider's row. The key now goes through
     `toBound` + `String`, the same normalisation `rowsToEta` applies. Pinned in `kmb-grouping.test.ts`
     (in the edge suite — `data-normalize` has no suite of its own).

## ADR-147 — A pulled-forward round has a floor: one upstream round per gap, whatever the reconnect rate

- **Status:** **Accepted** — **2026-08-13** in `apps/edge/src/eta-hub.ts` (`LIVE_POLL_NOW_MIN_GAP_MS`,
  `LAST_ROUND_AT_KEY`, `reschedule`).
- **Context.** The adversarial review over the ADR-137–146 branch found the amplifier ADR-140 exposed:
  with no readings row for a still-failing target, every re-`subscribe` reads as "never polled" and
  pulls the shard's round to *now* — and the comment crediting `coalesce` with bounding this was wrong,
  because the coalescer caches answers, never rejections. A client reconnect loop mid-outage therefore
  re-hit the failing upstream once per reconnect, on the shard's whole union.
- **Decisions:**
  1. **`pollNow` is floored, not absolute**: a pulled-forward round may not start sooner than
     `LIVE_POLL_NOW_MIN_GAP_MS` (10 s — `LIVE_ROUTE_MIN_GAP_MS`'s own "never ask more often than this"
     reason) after the last round *started*, stamped at the top of `round()` so the in-flight window is
     covered. A shard that has not polled within the gap — the WP6-8b first-poll promise — still gets
     its round immediately.
  2. **The stamp survives teardown** (`forgetReadings` keeps it, like `roundsCompleted`), because a
     reconnect loop whose every disconnect empties the shard would otherwise reset the floor through
     the teardown itself. It is in `LIVE_HUB_KV_KEYS`, so a *test* reset clears it — exactly the
     drift that list's docblock predicted for a fifth key.
  3. Pinned inside the ADR-140 outage scenario: the second rider's subscribe arms a round at most one
     gap out and the completed-round counter does not move until it is fired.
- **Consequences:**
  - 🟡 A genuinely new cold target subscribed within 10 s of the shard's last round waits up to the gap
    for its first data (against a 45 s cadence). The deferred snapshot already covers the wait honestly.
  - 🟡 Suite cases that need a *self-fired* round mid-test (the harness cannot drive one while waiting
    on frames — the I/O-context constraint `holdBoards` documents) now delete `LAST_ROUND_AT_KEY`
    first via `allowPollNow`, which is the honest spelling of "this shard has not polled recently".

## ADR-148 — The tile hang detector bounds the connection, not the rider's download

- **Status:** **Accepted** — **2026-08-13** in `apps/edge/src/tiles.ts`. Closes the 🟡 body-truncation
  consequence recorded under ADR-139.
- **Context.** ADR-139 routed `fetchTile` through `fetchUpstream`, whose `AbortSignal.timeout(10 s)`
  spans the whole request — headers *and* body. For an ETA board the distinction is invisible (a ~1 kB
  body we consume ourselves); a tile's body is streamed to the rider at the rider's own pace, so a
  client slower than tile-size ÷ 10 s was handed a truncated image where the pre-ADR-139 stream was
  unbounded. The review over the branch flagged it; theoretical at ~30 kB behind Cloudflare's
  buffering, but the success path had become strictly less tolerant for no benefit.
- **Decisions:**
  1. **A manual controller, disarmed on response.** The 10 s timer aborts a connection that never
     answers — the defect ADR-139 closed, still closed, still classified `upstream_timeout` by
     `codeFor` (`AbortError`) — and `clearTimeout` in the `finally` frees the success path's body to
     stream as long as the rider needs. `fetchUpstream` stays what it was built for: bodies we consume.
  2. **Mid-body wedges are deliberately unbounded** on the success path: with a client still reading,
     a wedged upstream is indistinguishable from a slow rider, and the rider's own disconnect is what
     ends the transfer. The non-ok branch's body cancel (ADR-139) is unchanged.

## ADR-149 — Three alignments the review surfaced: the mid-session quarantine, the KMB `co`, the retry cadence

- **Status:** **Accepted** — **2026-08-13** in `apps/web/src/lib/preferences.ts`,
  `apps/mobile/lib/preferences.ts`, `packages/data-normalize/src/kmb.ts` and
  `packages/api-client/src/index.ts`. Three small fixes, one ADR: each is a rule from an earlier ADR
  applied to the door it had missed.
- **Decisions:**
  1. **ADR-143's quarantine covers the mid-session read, on both platforms.** The launch path
     preserved unreadable bytes; the *other* door did not: `readDisk` (what a write's merge reads)
     caught a corrupt blob, returned `null` — "nobody to merge with" — and the write then replaced the
     bytes. The web `getItem`'s old comment even offered "the next write merges over whatever is still
     there" as the comfort, which was exactly backwards. Both `readDisk`s (and the web `getItem`) now
     copy the bytes to `nextbus.preferences.quarantine` before returning `null`. Mid-session, a failed
     quarantine write is swallowed rather than failing the commit — there is no splash to hold, and
     failing the write would trade a possible loss for a certain one. Pinned in both suites.
  2. **ADR-146's bucket-key canonicalisation covers `co`.** The key normalised `dir` and
     `service_type` and left `co` raw while `rowsToEta` collapses any non-`LWB` value to `KMB` — the
     same split-board shape, one field over. The key now applies the identical collapse.
  3. **ADR-141's retry cadence honours a served `refreshAfterMs`.** The resolution retry was
     `this.pollMs` while the rounds it precedes run at `opts.refreshAfterMs ?? this.pollMs`; an edge
     that slowed the cadence would still have been asked for the route document at the shipped one.
     Both now read the same effective cadence.

## ADR-150 — The freshness notice reaches all four screens, and `offline` stops being a state nothing could see

- **Status:** **Accepted** — **2026-08-13.** `newestNearbyBoard` / `newestPlaceBoard` in
  `packages/core/src/eta.ts` with 8 corpus rows; `FeedNotice` + `useOnline` on **both** renderers; the
  notice declared in all four screen specs as a `oneOf` over `notice.kind`; `stale` and `offline`
  promoted from `unenforced` to projected states. Pays off the 🟠 consequence
  [ADR-133](#adr-133--a-screen-says-once-that-it-has-stopped-being-fed-and-never-a-fourth-sentence)
  recorded — *"wired on Route detail only… until it lands, three screens still say nothing"* — and
  finishes what [ADR-123](#adr-123--staleness-is-a-board-level-fact-so-a-per-reading-cue-is-the-wrong-unit)
  put in debt.
- **Context.** ADR-123 removed both per-reading staleness cues on the finding that staleness is a
  property of the **board**; ADR-133 built the screen-level sentence that replaces them and wired it on
  Route detail, where the owner had hit the problem. Nearby, Place detail and Favourites — the three
  screens a rider spends most of their time on, and the only one whose contents they curated by hand —
  said nothing at all when the feed stopped. `apps/mobile` said nothing anywhere.
- **Decision 1 — the payload goes into the kernel, not a list of timestamps.** ADR-133's plan was for
  each screen to map its payload to timestamps and call `newestBoard`. That puts one domain decision in
  the renderers: **which field is the board's clock.** It is `dataTimestamp`, when the operator
  generated the reading, and never `observedAt`, which our own layer stamps on every fetch — so a
  screen that read `observedAt` would say *"last updated 21:34"* about a feed that stopped publishing
  at lunchtime, silently, and the two renderers could get it wrong differently. `newestNearbyBoard`
  and `newestPlaceBoard` are six lines each over `newestBoard`, and each carries a corpus row that
  fails if anyone swaps the field. Route detail keeps its `RouteDetailView.lastUpdatedIso`: that view
  is handed the whole payload already, and *"whichever pole answered most recently"* is part of what
  the schematic means.
- **Decision 2 — the notice is `oneOf`, and it lives in one file.** `packages/contract/src/ui/feed-notice.ts`
  declares the slot once and all four screen specs reference it, which is ADR-083's rule applied to a
  fragment rather than a spec. It is a **discriminated branch** over `notice.kind` rather than three
  `when`-gated lines because the kernel's precedence — `offline` → `unreachable` → `lastUpdated` →
  `none` — makes the states exclusive, and three optional slots could each be true at once, which would
  make the format say something the rule forbids. `oneOf` is also total: a fifth kind with no case here
  is a hard failure in every suite rather than a line that quietly stops being projected.
- **Decision 3 — the driver supplies the notice, the way it already supplies an error message.** A
  screen's freshness is not a field of any view model and never can be: it joins the newest board on
  screen to two facts the kernel is *handed* — whether the platform has a network, and how the last
  attempt to reach us failed. So each conformance driver puts the notice its renderer computed onto the
  view it hands the walker, exactly as it does with `error` in a `failed` state. What keeps that honest
  is that the value is a kernel call over a kernel-selected timestamp, and the comparison is exact: a
  driver that invented a sentence would have to write a different one than the screen draws.
- **Decision 4 — `offline` becomes a projected state on all four screens, and this is the finding.**
  Every one of the four specs carried the same `unenforced` reason, in the same words: *"textually
  identical to `stale`, so asserting it would be asserting `stale` twice."* That was true, and it was a
  statement about **the gap rather than the design** — the screens had no sentence for *why* they had
  stopped being fed, so a harness could not tell "your network is gone" from "this data is old" because
  a rider could not either. The notice gives them different words, and the four states become
  observable. Driven by redefining `navigator.onLine`, which both renderers' suites can do because both
  run under jsdom.
- **Decision 5 — `stale` is driven by moving the clock, not by an old fixture.** Route detail's `stale`
  fixture was a corpus case whose *one* row carries an old board, chosen when the state was about that
  row dimming. Since ADR-123 there is nothing per-row to observe, so the fixture would have passed while
  asserting nothing. Both renderers now run the `content` payload with `now` past the served
  `staleAfterMs` and produce the view from that same clock — so the readings age with the fixture rather
  than diverging from a golden taken at another moment, and the state asserts what it claims: the same
  cards, the same figures, plus one sentence.
- **Decision 6 — `apps/mobile` is wired too, and its `online` is `true` on native by design.**
  ADR-113 keeps the RN app as the reference and says *"the specs still bind it"* — a sentence a rider
  reads is content, which ADR-075 puts on the identity side of the line, so a spec that projects it
  binds both renderers. `navigator.onLine` and the `online`/`offline` events are web APIs: under
  `react-native-web` (the Expo PWA, and both suites' environment) they are the browser's own, and on
  iOS/Android there is no `onLine` property at all. The hook type-guards the read and answers `true`
  there, which is **exactly what the field means** — `false` is evidence of no network, `true` is merely
  the absence of evidence — so a native build never says *"You're offline"* and reaches the
  `unreachable` arm instead, which is the same arm that covers a captive portal on the web. A real
  native signal needs `expo-network` or NetInfo and is a `docs/07` row.
- **What writing it found.**
  - 🔴 **A corpus case pinned impossible data, and only the notice could read it.**
    `placeDetailView`'s `a-served-policy-moves-the-imminence-band` had its arrival re-timed to five
    minutes after `now` and its board left at the capture date **two days earlier** — a feed that had
    published on the 27th predicting a bus on the 29th — with `stale: true` pinned beside a live
    reading. Nothing had ever read the board's clock at screen level, so nothing could see it. Repaired
    in place; the eight other cases with 47-hour-old boards are *coherent* snapshots (board and arrival
    both from the 27th, viewed on the 29th) and are left alone.
  - 🟠 **Route detail's spec forbade what Route detail does.** Its `stale` state still read *"the
    readings we have, dimmed"* and its `mustNot` banned *"a whole-screen staleness banner"* — three days
    after ADR-133 put exactly that banner on that screen. It is the drift
    [ADR-134](#adr-134--the-design-system-gallery-is-enumerated-from-the-published-specs-not-hand-listed)'s
    gallery caught in three other specs, in the one file the gallery does not surface as prose.
  - 🟠 **A fixture had been running for nothing since ADR-123.** `apps/mobile`'s Favourites driver kept
    a `stale` fixture with a shifted clock — written for the `~` mark — while the spec state was
    `unenforced`, so `conformStates` skipped it every run. A projected state with no fixture is a loud
    finding; a fixture with no projected state is a silent one, and the format has no guard for that
    direction.
- **Consequences:**
  - 🟢 Every screen that draws a reading says once, in the rider's own language, that it has stopped
    being fed — and says nothing at all while it is being fed, which is the ordinary case.
  - 🟢 `stale` and `offline` are enforced on eight surfaces (four screens × two renderers) rather than
    declared on none.
  - 🟠 **The `unreachable` arm cannot be a spec state**, because `trouble` is a property of the last
    *request* rather than of any fixture. It has a suite of its own — `apps/web/test/feed-notice.test.tsx`
    — which seeds the query cache and lets the refetch fail, the state a rider is in when our Worker is
    down and their cards are still on screen. Watched failing on an injected `trouble: 'none'`.
  - 🟠 **A two-day-old board still reads as this morning**, and now it is visible in the goldens: several
    Place-detail states project *"Last updated 11:59"* from a payload captured two days before their
    `now`. That is ADR-133's recorded limitation — `formatClock` has no date — pinned rather than
    papered over. `docs/07` carries the fix.
  - 🟡 **The spec format still cannot say "enforced here, absent there".** It did not have to this time
    because both renderers were wired, but ADR-113 makes one-renderer features a standing possibility,
    and the only vocabulary for it today is an `unenforced` string that reads exactly like a gap.
  - 🟢 **The gallery got live samples, asked for while reviewing this row** — *"ideally the same component
    in multiple states so I can review it quickly."* `apps/web/lab/samples.tsx` draws `FeedNotice`,
    `StopRow` and `PlaceRow` several states at a time at a phone's width, every panel a **corpus golden**
    or the kernel call the screen makes, with the inputs written beside it;
    `test/gallery-samples.test.tsx` renders every one, so a renamed corpus case is a red build rather than
    a blank panel somebody meets months later. It pays ADR-134's *"still owed"* for the leaf components and
    it is what makes the `docs/07` row about reviewing the app's error and placeholder texts **as a set**
    possible at all: four sentences met one screen at a time can each read fine and still not belong
    together.
  - 🟠 **The lab has no Tailwind of its own, and nobody had noticed.** `tailwind.config.cjs` scans
    `./src/**` and not `./lab/**`, so any utility the app does not already use is never generated: `w-96`
    computed to 1016 px — the width of its own caption — and the panels came out three different sizes.
    Adding `lab/` to the content glob would fix it by letting lab-only classes into the **shipped**
    stylesheet, which is exactly what [ADR-112](#adr-112--a-dev-page-lives-in-the-app-and-a-gate-keeps-it-out-of-the-app)'s
    three assertions exist to prevent, so lab-only geometry is an inline style and everything semantic
    stays a token class. The same trap is waiting for the next person who adds a panel.

---

## ADR-151 — The route line geometry we said Hong Kong did not publish has existed since 2021

- **Status:** **Accepted** — **2026-08-22.** Corrects a factual claim carried in four docs since the
  2026-06-09 research. It **amends nothing**: neither
  [ADR-011](#adr-011--maps-via-maplibre-tentative-phase-15) nor
  [ADR-049](#adr-049--the-basemap-is-the-hk-lands-departments-self-cached-with-labels-as-a-per-locale-overlay)
  said anything false — the basemap decision and the tile seam stand exactly as written. What changes is
  a *research* premise those decisions were taken alongside, which is why this is a new entry rather
  than an amendment to either. No code yet: the plan that acts on it is
  [`proposals/06`](./proposals/06-maps-and-route-geometry.md), and the measured detail is
  [`research/07`](./research/07-route-geometry-and-maps.md).
- **Context.** The owner noticed that Transit shows minibus route paths and asked where that data comes
  from. Our own record said it did not exist. [`research/01 §0`](./research/01-open-data-catalog.md)
  listed *"no official route line geometry/polylines anywhere"* as one of **five facts that shape
  everything we can build**; [`research/02 §4`](./research/02-data-availability-matrix.md) said *"stops
  yes, lines no"*; [`research/README`](./research/README.md) made it headline takeaway #2; and
  [`docs/07`](./07-backlog.md)'s hkbus map study concluded *"a real route line is a data question before
  it is a rendering one."* Feature gap B2 sat 🟥 on that basis. All of it traced to one measurement.
- **Decision 1 — record that the claim was false, and how it got that way.** The Transport Department
  publishes road-following route lines on the **CSDI portal**: **Bus Route**
  (`td_rcd_1638844988873_41214`, 2,255 `MultiLineString` features, since 2021-12-07) and **Green Minibus
  Route** (`td_rcd_1697082463580_57453`, 1,161 `LineString` features, since 2023-10-12), under the same
  DATA.GOV.HK terms as everything else we use. The original measurement was not sloppy and was not
  wrong: `data.gov.hk`'s routes-and-fares GeoJSON really is 56,048 `Point` features with zero
  `LineString`. It was the right department's **wrong file** — the lines live in a different catalogue,
  on a different site, that the survey never opened. The bus dataset predates this project.
- **Decision 2 — the failure mode is worth more than the fact.** A verified-by-download negative got
  promoted to a headline takeaway, and then three downstream docs cited it instead of re-checking, so
  one file-level miss hardened into an architectural constraint that shaped a backlog item and a feature
  gap. **A negative result is only as broad as the thing actually searched**, and ours recorded the
  conclusion (*"no polylines anywhere"*) rather than the evidence (*"this file has no polylines"*).
  Where a research doc says a thing does not exist, it should now say **what was opened to find out** —
  the corrected §4 does, which is why it is longer than the line it replaces.
- **Decision 3 — `gtfsId` stops being GMB-only.** CSDI `ROUTE_ID` **is** the consolidated dataset's
  `gtfsId`, verified on `ROUTE_ID 2000410` → GMB 69 Cyberport↔Quarry Bay with terminus names matching.
  We already carry it, but only for GMB, where ADR-047 needed it to disambiguate a canonical id. It is
  dropped for KMB/CTB/NLB, and **retaining it there is the one prerequisite** for everything in
  `proposals/06`. That is a `dataset.ts` change, not an architecture: the field already exists on the
  entry type.
- **Decision 4 — coverage is stated as a measurement, and the gap is structural.** GMB **100%**, KMB
  **91%**, CTB **86%**, NLB **83%** — **93%** of 3,625 bus + minibus route-directions; MTR feeder, Light
  Rail and ferries **0%**. Of the 269 uncovered, **258 have no `gtfsId` at all**: they are racecourse
  (`R`), school (`S`), peak-hour (`P`) and `serviceType ≠ 1` variants that TD does not separately
  register. So the gap will not close by waiting, and any plan must answer for it rather than round it
  up. Nothing here should be quoted as "~95%" — the number is per-operator and it moves.
- **Decision 5 — do not copy hkbus's fallback without deciding to.** When a route has no geometry,
  hkbus.app silently draws a `LineString` through the ordered stop coordinates — no marker, no styling
  change, no message (`useRoutePath.tsx`). For CTB `20R` that is four stops over 7.6 km, so the "route"
  is three chords straight across Kowloon Bay; for KMB `101R` the line crosses the harbour through the
  water. Under rule 3 — *never fake precision* ([ADR-008](#adr-008--etas-are-approximations-no-client-side-fake-countdown))
  — an unmarked crow-flies line is the cartographic twin of a client-side per-second countdown: it draws
  a confidence the data does not have. This ADR does not settle what we draw instead; it settles that
  **the fallback is a decision to be made, not a default to inherit**, and `proposals/06 §5` is where it
  gets made.
- **Consequences.**
  - ✅ Feature gap **B2** is unblocked, and re-filed as a rendering problem.
  - ✅ `research/01 §0`, `research/02 §4` (+ the matrix row), `research/README` takeaway #2,
    `research/04` B2 and `docs/07`'s hkbus study are all corrected in place, each pointing here.
  - ✅ `research/07` is new and carries the resolver, the coverage table and the sample routes.
  - 🟠 **`ROUTE_SEQ` is not a direction and must never be treated as one.** hkbus flag this (their issue
    #14) and it silently swaps routes. Measured alternative: mean distance from the rider's ordered
    stops to the candidate line separates KMB 101's two directions **8.6 m vs 41.9 m**, and rejects a
    wrong route at **432 m**. Whatever ships must resolve direction geometrically.
  - 🟠 **That same metric cannot tell a short-working from its parent** — KMB 101's two variants both
    score 8.6 m, because a short-working's stops all lie on the parent's line. The answer is not a
    better score but a different step: pick by coverage, then **trim the line to the rider's own first
    and last stop**. Recorded here because it is the kind of thing a later implementer will otherwise
    rediscover by shipping a line with a tail past the terminus.
  - 🟠 **The bulk archive is a real cost and option A may make it unnecessary.** The bus GeoJSON is
    **1.53 GB** unzipped (240 MB zipped; the FGDB is 38 MB but needs GDAL). Per-route on demand via the
    ArcGIS FeatureServer is 33 KB for both directions and needs no pipeline. Deliberately not decided
    here.

---

## ADR-152 — Route geometry comes from CSDI, joined on `gtfsId`, resolved at the edge

- **Status:** **Accepted** — **2026-08-24.** Built: `gtfsId` retained for every operator in
  `packages/data-normalize/src/dataset.ts` and carried on `RouteDoc`; `GET /v1/route/:id/path` in
  `apps/edge/src/route-path.ts`. Acts on
  [ADR-151](#adr-151--the-route-line-geometry-we-said-hong-kong-did-not-publish-has-existed-since-2021);
  the rule it delegates to is [ADR-153](#adr-153--a-route-line-is-chosen-by-coverage-oriented-by-the-riders-stops-and-trimmed-to-their-terminals).
  Plan: [`proposals/06`](./proposals/06-maps-and-route-geometry.md) rows M0 and M2.
- **Context.** ADR-151 established that the Transport Department publishes road-following route lines
  on CSDI, keyed on `ROUTE_ID` — which **is** the consolidated dataset's `gtfsId`. We already carried
  `gtfsId`, but only for GMB, where ADR-047 needs it to disambiguate a canonical id. Everything else
  dropped it, so the join key for 93% of route-directions was being discarded one line before it
  became useful.
- **Decision 1 — `gtfsId` is retained for every operator, and absence is not an error.** It moves onto
  `IndexRouteMeta` for all of KMB/CTB/NLB/GMB. About 7% of route-directions have none — they are the
  racecourse (`R`), school (`S`), peak-hour (`P`) and `serviceType ≠ 1` variants the TD does not
  separately register — so the field is optional and a consumer must treat missing as ordinary. Coded
  as a doc comment on the field rather than a runtime check, because there is nothing to check: the
  gap is a property of the upstream register, not a fault.
- **Decision 2 — it goes on the shard, not on the wire.** `RouteDoc.gtfsId`, not `Route.gtfsId`. Only
  the edge needs it — to look a line up on the rider's behalf — so putting it on the contract would be
  a **breaking** wire change (`CONTRACT_VERSION`) that bought a client nothing. A client asks for a
  *path*; it never asks for a *key*.
- **Decision 3 — the fetch is the edge's, not the client's.** `GET /v1/route/:id/path`. Three existing
  rules already require it and none of them is new: rule 2 forbids a view knowing a URL, and
  `check-view-transport-free` scans the *source* for the literal because an import graph cannot see
  one; the caching that makes this affordable and the attribution that makes it permitted both already
  live in the Worker for tiles (ADR-049), and CSDI is the same licence family. Measured: **444 ms and
  7.9 KB** for KMB 1 outbound against a warm dataset, 371 vertices at 5 dp.
- **Decision 4 — a route with no line is a 200, not a 404.** The response carries
  `available: false` with an empty `path`. A 404 would make *"no geometry"* indistinguishable from
  *"no such route"*, which is a different fact and already answered by `/v1/route/:id`. What a screen
  draws for `available: false` is a design decision, deliberately not settled here
  (`proposals/06 §5`).
- **Decision 5 — cached for a day, and the fallback query is second, not merged.** `ROUTE_PATH_TTL_SEC`
  is 86 400: CSDI republishes on the order of a fortnight and an alignment is the most static thing we
  serve. The operator+number fallback runs **only** when the `gtfsId` query returns nothing, so the
  common path costs one upstream call and the ambiguous path is never entered for the 93% that do not
  need it.
- **Consequences.**
  - ✅ Coverage measured end to end through the real endpoint: KMB 1 both bounds, KMB 101, CTB 11 and
    GMB 69 all resolve, with fits of **3.1–10.1 m** against a 100 m reject threshold.
  - ✅ `packages/core` stays at 100% statement, branch, function and line coverage with the new module.
  - 🟠 **A long route is a large-ish payload.** CTB 11 outbound is 2,747 vertices. 5 dp plus gzip keeps
    it reasonable, but nothing yet simplifies geometry to the zoom being drawn, and at some point
    something should — Douglas–Peucker per zoom band is the obvious answer and is not built.
  - 🟠 **The first request against a cold isolate pays the inline dataset build**, which is ADR-055's
    known `"dataset":"inline"` tier and not this endpoint's doing — but it is how a 60 s first response
    is explained, and it is worth knowing before anyone benchmarks this.

---

## ADR-153 — A route line is chosen by coverage, oriented by the rider's stops, and trimmed to their terminals

- **Status:** **Accepted** — **2026-08-24.** Built: `packages/core/src/route-path.ts`, pinned by
  `packages/core/spec/route-path.spec.json` (5 groups, 20 rows). Consumed by
  [ADR-152](#adr-152--route-geometry-comes-from-csdi-joined-on-gtfsid-resolved-at-the-edge).
- **Context.** Fetching a line by id is the easy half. Two traps sit behind it, both measured on the
  real data ([`research/07 §4`](./research/07-route-geometry-and-maps.md)), and both produce a map that
  looks plausible and is wrong — the worst failure mode a map has, because nobody reports it.
- **Decision 1 — `ROUTE_SEQ` is never read as a direction.** CSDI's `ROUTE_SEQ` (1/2) does not track
  the operator's outbound/inbound; hkbus record the same finding as their issue #14 and it silently
  swaps some routes. It is carried in our types for diagnostics and **the resolver never branches on
  it**. Stated as a decision rather than an omission because the field is right there, it is named
  like a direction, and the next person to touch this will be tempted.
- **Decision 2 — direction comes from the rider's own stop order.** `orientToStops` compares where the
  **first** and **last** stop project along the line, and reverses the vertices when the first lands
  later. hkbus's fix compares a single point — the line's first vertex against the direction's first
  stop, with a 500 m guard — which decides a whole route at a terminus where both directions commonly
  board within 40 m of each other. Two points cost nothing more and cannot be fooled that way.
- **Decision 3 — candidates are scored by mean stop-to-line distance, and the metric's blind spot is
  the reason for decision 4.** Measured on KMB 101: the correct direction scores **8.6 m**, the
  opposite **41.9 m**, an outright wrong route **432 m**. It is decisive for those. It **cannot**
  separate a short-working from the parent route it runs along — both score 8.6 m, because the
  short-working's stops all lie on the parent's line. That is not a tuning problem, and no scoring
  function fixes it; it is the wrong question.
- **Decision 4 — so the line is trimmed to the rider's own first and last stop.** After trimming it
  stops mattering which variant won: the drawn line is the road the rider's stops sit on, bounded by
  the rider's own terminals. One code path handles short-workings, variants **and** circulars, and no
  line can draw a tail past the terminus on screen. A circular — where both terminals project to
  nearly the same place — is explicitly excluded from trimming so it does not collapse to nothing.
- **Decision 5 — `ROUTE_PATH_REJECT_METRES` is 100, and it is calibrated, not chosen.** It sits in the
  wide gap between the observed good fits (8–10 m) and the observed wrong route (432 m), and exists to
  guard the operator+number fallback, which is ambiguous by construction — NLB route 1 matches five
  `ROUTE_ID`s. It is **not** a direction test: a route whose only available line runs the other way
  scores well under it and is then reversed by decision 2.
- **Decision 6 — the corpus's expected values come from a second implementation.** They were generated
  by an independent Python implementation of the same rule and are asserted against the TypeScript, so
  a passing row is *agreement between two readings* rather than a recording of whatever the code
  happened to do. A corpus that only records the implementation cannot catch the implementation being
  wrong, which is most of what a corpus is for.
- **Consequences.**
  - ✅ 20 rows across 5 groups; `packages/core` holds 100% coverage.
  - ✅ The rule is `packages/core`, so iOS and Android hand-port it from the corpus rather than each
    re-deriving a direction test — which, on this evidence, they would get wrong.
  - 🟠 **Mean stop-to-line distance is direction-agnostic**, and one corpus row exists specifically to
    say so (`reversed-line-scores-identically`). Anyone who later tries to use the fit as a direction
    signal will find that row already disagreeing with them.
  - 🟠 **Distances use a local equirectangular projection, not `geo.ts`'s haversine.** Deliberate: the
    trim step needs a planar projection, and over a Hong Kong route the difference is far below the
    corpus tolerances. Two rounding conventions in one package is a real hazard — the module doc says
    which is which and why, exactly as `mercator.ts` does against `geo.ts`.

---

## ADR-154 — `MapProvider` widens the basemap seam, and the base/label zoom split becomes a kernel rule

- **Status:** **Accepted** — **2026-08-25.** Built: `packages/ports/src/map-provider.ts`,
  `tileZoomPlan` in `packages/core/src/mercator.ts` (9 corpus rows),
  `apps/web/src/adapters/mapProvider.ts` and `apps/web/src/components/MapView.tsx`, exercised by a
  `#map` lab page. Amends
  [ADR-049](#adr-049--the-basemap-is-the-hk-lands-departments-self-cached-with-labels-as-a-per-locale-overlay)
  — the basemap stays LandsD, but *how a renderer asks for it* is no longer one shape. Row M3 of
  [`proposals/06`](./proposals/06-maps-and-route-geometry.md).
- **Context.** `TileSource` answers *"what is the URL of one tile"*, which is the right question only
  while the renderer composites tiles itself — which is what `MiniMap` does. An interactive map hands
  compositing to an engine, and an engine wants a **style**, not a tile URL. ADR-049 decision 6 said
  vector was "a later upgrade" and decision 7 kept Protomaps as the documented fallback; neither can be
  taken up while the only seam we have describes a raster tile.
- **Decision 1 — the new port is a superset, not a replacement.** `MapProvider`'s `raster-xyz` arm
  **is** `TileSource` structurally, so the LandsD implementation is re-used rather than rewritten and
  `MiniMap` keeps working untouched. The `vector-style` arm exists with no implementation, on purpose:
  the shape of the seam is cheaper to decide now than during a swap. Nothing has to move in one commit,
  and the two ports coexist until `MiniMap` retires.
- **Decision 2 — which zoom levels to request is a KERNEL rule, not a renderer's business.**
  `tileZoomPlan(zoom, devicePixelRatio, zooms)` returns `{ base, label, scale }`. LandsD serves 256 px
  tiles only — no `@2x` — so on a DPR-2 screen the base must be fetched one level deeper and drawn at
  half size to reach true 2× density. The **labels must not** get the same treatment: LandsD bakes
  label size into the raster, so a deeper level arrives with a denser, half-size label set — sharper
  text that is harder to read, a worse trade than the softness it cures. Splitting them is only
  possible because the labels are a separate service, which is the same property ADR-049 relies on for
  per-locale labels; the overlay now earns its keep twice.
- **Decision 3 — the engine is told in its own vocabulary, and the rule stays one declaration.**
  MapLibre's lever is `tileSize`: declaring a source's tiles as 128 px makes it fetch one level deeper.
  So the component asks the kernel *whether* to overzoom and configures the engine to match, rather
  than re-deriving the policy. A native renderer will express it differently again and must still make
  the same call, which is exactly why the numbers are in `packages/core` and pinned by a corpus.
- **Decision 4 — attribution stays a required member.** Carried over verbatim. LandsD's licence
  requires the mark on the map face, so a provider usable without it is a provider usable out of
  compliance. `attributionControl: false` on the map is deliberate: MapLibre's own control is not the
  one the licence asks for.
- **Decision 5 — it lands in the lab, not on a screen.** `MiniMap` still owns every shipping screen.
  Proving the seam and the engine on a `#map` lab page is ADR-112's argument applied: a spec'd screen
  is an expensive place to try something, and moving one is M4's job, alongside the route line that
  gives it a reason.
- **Consequences.**
  - ✅ `packages/ports` stays type-only (9 modules, no runtime emit) with the new port in it.
  - ✅ `packages/core` keeps 100% coverage; `mercator.spec.json` gains a `tileZoomPlan` group whose
    rows include the rounding convention (JS `Math.round` is half-**up**, which Python's `round` is
    not) and the DPR boundary (1.5× overzooms, 1.25× does not) — both things a hand-port gets wrong
    silently.
  - 🟠 **The interactive map is NOT visually verified.** It typechecks, the style is constructed and
    the component mounts, but MapLibre's whole lifecycle — style load, tile selection, the `load`
    event — is driven by its `requestAnimationFrame` render loop, and the automated browser used here
    runs a **backgrounded** tab where rAF never fires. The map therefore constructs and does nothing,
  ...
  - 🟠 **Native is not started.** `@maplibre/maplibre-react-native` needs a config plugin and a
    **dev-client build**, so it cannot be added and verified in the same motion; `apps/mobile` still
    ...
    than in what any spec asserts, which ADR-075 permits — but it is drift in the ordinary sense and
    should not sit unclosed for long.
  - 🟠 **A lab-only utility class is never generated.** `h-[520px]` computed to 0, the map got a
    zero-height viewport, and it took a while to see because nothing errors. ADR-112 already records
    this trap; this is its second occurrence, so the rule is worth restating: **lab-only geometry is
    an inline style.**

---

## ADR-155 — Route detail draws its line, and the spec's subject is the sentence under it

- **Status:** **Accepted** — **2026-08-26.** Built: `routePathView` and `boundsOf`/`centreOf` in
  `packages/core` (7 + 11 corpus rows), `apps/web/src/components/RouteMap.tsx`,
  `apps/web/src/hooks/useRoutePath.ts`, four new states and a shared `ROUTE_PATH` slot node in
  `route-detail.spec.json`, dark-mode raster paint in `MapView`, and four `color.map.route*` tokens.
  Row **M4** of [`proposals/06`](./proposals/06-maps-and-route-geometry.md), which this closes.
  Follows [ADR-152](#adr-152--route-geometry-comes-from-csdi-joined-on-gtfsid-resolved-at-the-edge) (the
  endpoint) and [ADR-154](#adr-154--mapprovider-widens-the-basemap-seam-and-the-baselabel-zoom-split-becomes-a-kernel-rule)
  (the engine).
- **Context.** The line existed on the wire and nothing drew it. What made this row expensive was not
  the drawing: it touches a spec'd screen, so the spec, both conformance drivers and the coverage
  control move together (ADR-083/084).

- **Decision 1 — what to draw is a kernel rule with three answers, and "no answer yet" is not one of
  them.** `routePathView(available, line, stops)` returns `surveyed`, `approximate` or `none`.
  hkbus.app's answer to a missing line is to join the ordered stops with straight lines, unmarked; for
  KMB 101R that draws a bus crossing Victoria Harbour through the water. Under rule 3 (ADR-008) an
  unmarked crow-flies line is the cartographic twin of a client-side per-second countdown, so a sketch
  is dashed *and* captioned, and where the stops are too far apart for a chord to mean anything there
  is no line at all.

  A **pending query is the absence of a model**, not a fourth arm. `available: false` means *asked and
  told no*, which for a dense urban route is a legitimate `approximate` — feeding an unresolved query
  into the same function would flash a dashed sketch on the way to nearly every route on the network.
  The spec expresses that as `when: 'path'`, so the slot is gated off rather than given a case no
  corpus row could ever produce.

- **Decision 2 — the thresholds are measured, not chosen, and they discriminate on real routes.**
  `APPROXIMATION_MAX_MEAN_GAP_M = 500` and `APPROXIMATION_MIN_STOPS = 4`. KMB 1 is 25 stops over 8 km
  — a 333 m mean gap — where CTB 20R is 4 stops over 7.6 km, a 2.5 km gap. The rule therefore asks
  *"are these stops close enough together that joining them describes a road"*, which is the question,
  rather than *"is this an express"*, which is a guess about it.

  Sampled over 120 real route-directions on 2026-08-26, **116 resolved to a surveyed line** and the
  four that did not split across both fallback arms: `KMB R215` (34 stops, 433 m) sketches, while
  `KMB R298` (1 209 m), `KMB SP10` (901 m) and `CTB NB3` (1 224 m) correctly draw nothing. KMB 101R —
  the route the whole argument is about — measures 607 m and is refused, narrowly and correctly.

- **Decision 3 — line coverage is ~97%, not 93%, and it includes green minibuses.** §7's figure came
  from `gtfsId` matching alone. Measured per operator over 40 route-directions each: KMB 37/40, CTB
  39/40, **GMB 40/40**. The CSDI dataset covers green minibus routes, which the record did not say and
  which matters more than the percentage point — GMB is the operator with no route-level ETA board, so
  it is the one where a map earns the most.

- **Decision 4 — a bounding box is a kernel function, because it is a number a renderer would
  otherwise compute.** `boundsOf` and `centreOf` in `mercator.ts`, corpus-pinned. `check-no-derivation`
  flagged both when they lived in the component and it was right to: ADR-068's rule is that two
  renderers computing a number separately is how they drift. `centreOf` halves in **degrees**, not in
  Mercator, and the corpus has a 50°N–70°N row where those two answers are 175 km apart — the mistake
  is easy to make with `latToWorldY` sitting two functions away. `boundsOf` is deliberately **not**
  antimeridian-aware, with a row pinning the long-way-round answer so a port does not silently "fix" it
  into a different function.

- **Decision 5 — dark mode is raster paint, not a CSS filter, because MapLibre draws the line on the
  same canvas.** `MiniMap` inverts with `filter: invert(1) hue-rotate(180deg) brightness(1.2)` and can,
  because its tiles are `<img>` elements with the overlay in separate DOM. Here the basemap and the
  route line share one canvas, so that filter would invert the line along with the map and hand back
  exactly the colour the light theme rejected. The style spec expresses it per source instead —
  `raster-brightness-min: 1, brightness-max: 0` is the inversion — and the overlay keeps its own
  light/dark token pair. This is ADR-049's *"the overlay needs its own pair"* trap arriving one layer
  down, where the mechanism that made it easy no longer exists.

- **Decision 6 — the native renderer has no map, and the spec says so rather than a test file.** The
  four map states are declared, measured on `apps/web`, and explicitly excused on `apps/mobile` through
  a named `NO_MAP_ON_NATIVE` set — which is itself checked, so a state excused there that the spec has
  dropped fails rather than quietly excusing nothing. The reason is real: MapLibre GL JS is WebGL, the
  RN equivalent is a native module, and adding it ends this app's ability to run in Expo Go. ADR-069's
...
- 🔴 **The spec passed while the caption could be deleted.** `pathApproximate` was declared, driven and
  ...
  found it — removing the caption altogether *passed*, while captioning every line failed. This is
  WP6-3b's finding recurring exactly: **an arm nothing drives is a specification looking at nothing**,
  and the only reliable check is to watch the suite fail on purpose. There is now a control asserting
  the three arms are reached, and it fires on the original bug.
- 🔴 **Every `routeDetailView` corpus route is a *sample* of a route, and a sampled route is a
  different shape.** The coordinates are real — the `content` case runs Tuen Mun to Tsim Sha Tsui — but
  five stops stand in for forty, so consecutive stops sit 6.9 km apart. That was free for two years
...
## ADR-156 — Route detail becomes a map with a sheet over it, assembled from parts

- **Status:** **Accepted** — **2026-08-26.** Built: `apps/web/src/components/sheet/` (`detents.ts` +
  `DraggableSheet.tsx`), `apps/web/src/screens/route/RouteContextCard.tsx`, and the assembly in
  `RouteDetail.tsx`. Rounds 3 and 4 of the mockups
  ([`proposals/06 §8`](./proposals/06-maps-and-route-geometry.md)), settled by the owner. Follows
  [ADR-155](#adr-155--route-detail-draws-its-line-and-the-specs-subject-is-the-sentence-under-it).
- **Context.** M4–M7 put a map on Route detail as a **strip above a scrolling list**, and every
  interaction §8d specified fought that layout: a marker tap scrolled its row into view and took the
  map off screen, which needed `position: sticky` to survive at all (ADR-155). The mockups had never
  met the problem, because they had two independently scrolling panes. The strip was the compromise;
  this removes the thing being compromised with.

- **Decision 1 — the map is the base layer and the only thing that scrolls is the sheet.** `main` is
  `fixed inset-0`; the map fills it; the stop list lives in a sheet over it. The sticky workaround is
  gone, because there is no page scroll left to be sticky against.

- **Decision 2 — every camera move is inset by what is covering the map.** A full-bleed map under a
  sheet has a *visible* rectangle that is not its bounding box, and `fitBounds` framing the whole
  viewport centres a route **behind** the sheet — a rider looking at the visible half sees the top of
  their route and nothing else. `visibleInset` carries the sheet's detent and the floating chrome's
  height as fractions, and `cameraPadding` turns them into MapLibre's `padding`, on `flyTo` as well as
  `fitBounds`. The first constant over-stated the chrome (0.22 against a true 0.14) and the result was
  a route zoomed out to a thumbnail: padding is subtracted from the space available, so over-stating
  it is not the safe direction to err in.

- **Decision 3 — three detents, `half` by default, and the arithmetic is not in the component.**
  `detents.ts` holds where a sheet may rest, what a release settles into and what an over-drag does —
  no React, no DOM, no NextBus vocabulary. That is what makes it testable: a drag cannot be exercised
  in jsdom, so the alternative is behaviour only ever checked by hand, and gestures are exactly where
  *"it felt right when I tried it"* stops being evidence. Twelve tests, and the ones that matter are
  about feel — **velocity beats proximity**, so a flick moves one detent in the direction of the throw
  even when the nearer detent is behind; one detent per flick, never two; over-drag still moves, at a
  third, because a sheet that stops dead under a moving finger reads as broken rather than as ended.

  `check-no-derivation` flagged four sites in the component and the fix was to **move** the logic, not
  to excuse it. The allowlist entry is on `detents.ts` alone, whole-file, on the stated ground that if
  anything in it ever reads a domain quantity the entry is wrong and should be deleted.

- **Decision 4 — the context card steps aside; it does not shrink in place.** Expanded, it runs under
  the floating back lens with a **centred** badge; collapsed, its left edge moves past the lens so the
  pill is a clean rectangle rather than a shape with a bite out of it. The badge is centred rather than
  leading because the card runs under a 48 px circle and the first build put the badge exactly there,
  where it vanished.

  Collapsed is a `<button>` and expanded is a `<div>`: the expanded card **contains** controls — the
  swap link, the fact pills — and a button cannot contain a button (ADR-024's rule again).

- **Decision 5 — only the *rider* collapses the chrome, and `originalEvent` is what proves it.** The
  card collapses on a map drag or zoom and on the sheet leaving its opening detent. It must not
  collapse on a camera **we** drove — and it did: `fitBounds` fires `zoomstart`, so the opening frame
  dismissed the header before anybody had touched anything. MapLibre attaches the DOM event that caused
  a camera move and attaches nothing when the app caused it, so the guard is exact rather than a
  heuristic. It also covers the flight a stop tap starts, which must not read as a dismissal either.

  Scrolling the list deliberately does **not** collapse it: a list flick is the most common gesture on
  this screen, and round 4 tried it and made the card flicker.

### What this cost, and what it found

- ✅ **The published spec did not move at all**, which is the strongest evidence the restructure was a
  layout change rather than a redesign. All 25 states still project; every slot is where it was.
- 🟠 **One line had to be dropped from the projection**: the sheet's `sr-only` heading. The walker sees
  presence rather than visibility (ADR-097), so an accessible name invisible to a rider is still a text
...
- 🟠 **Scroll chaining is not built.** Round 3 had it as a toggle — raise the sheet, then scroll the
  list. It is off, deliberately: this list is 40 rows and chaining makes every flick a potential
  resize. Cheap to add behind a flag if it is wanted.
- 🟠 **`CollapsingHeader` is now Place detail's alone.** The two screens stop being one family here, on
  purpose: only this one has a map underneath to get out of the way of.
